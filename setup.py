...
KOZMODB_PIP_INSTALL_DEFAULT_EXTRAS = (
    True if os.getenv("KOZMODB_PIP_INSTALL_DEFAULT_EXTRAS", "true").lower() == "true" else False
)
DEFAULT_PIP_EXTRAS = [
    line.split("#")[0].rstrip()
...
about = {}
with open("kozmodb/__about__.py") as fp:
    exec(fp.read(), about)
...
def define_deps():
    ...
    handlers_dir_path = os.path.normpath("./kozmodb/integrations/handlers")
    for fn in os.listdir(handlers_dir_path):
        if os.path.isdir(os.path.join(handlers_dir_path, fn)) and fn.endswith("_handler"):
            ...
            for req_file_path in glob.glob(os.path.join(handlers_dir_path, fn, "requirements*.txt")):
                ...
                if KOZMODB_PIP_INSTALL_DEFAULT_EXTRAS and extra_name in DEFAULT_PIP_EXTRAS and extra:
                    requirements += extra
