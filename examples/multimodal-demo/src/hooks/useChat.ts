...
import { MemoryClient, Memory as KozmodbMemory } from 'kozmodb';
...
export const useChat = ({ user, mem0ApiKey, openaiApiKey }: UseChatProps): UseChatReturn => {
  ...
  const updateMemories = async (messages: PromptMessage[]) => {
    ...
    try {
      ...
      const newMemories = response.map((memory: KozmodbMemory) => ({
        id: memory.id || '',
      ...
      let relevantMemories = '';
        try {
          ...
          relevantMemories = searchResponse
            .map((memory: KozmodbMemory) => `Previous context: ${memory.memory}`)
            .join('\n');
        } catch (error) {
