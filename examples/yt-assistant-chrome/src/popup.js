...
async function init() {
  try {
    ...
    document
      .getElementById("save-kozmodb-api-key")
      .addEventListener("click", saveKozmodbApiKey);
    ...
    document
      .getElementById("toggle-kozmodb-key")
      .addEventListener("click", () =>
        togglePasswordVisibility("kozmodb-api-key")
      );
...
// Save kozmodb API key to storage
async function saveKozmodbApiKey() {
  const apiKeyInput = document.getElementById("kozmodb-api-key");
  const apiKey = apiKeyInput.value.trim();

  // Show loading status
  showStatus("Saving Kozmodb API key...", "warning");
  ...
  try {
    ...
    if (response.error) {
      ...
      showStatus("Kozmodb API key saved successfully", "success");
      loadConfig(); // Refresh the UI
    }
  } catch (error) {
    showStatus(`Error: ${error.message}`, "error");
