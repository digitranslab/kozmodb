...
Remember: The goal is to build a comprehensive understanding of both the user's knowledge and their learning journey through YouTube.`,
        ...
        console.error("Error initializing KozmodbAI:", error);
        return false;
      }
    } else {
      console.log("No KozmodbAI API key found in storage");
      return false;
    }
  } catch (error) {
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  await initializeKozmodbAI(); // Initialize KozmodbAI
});
  ...
  try {
    ...
    if (mem0client) {
      try {
        ...
        console.error("Error with KozmodbAI operations:", memoryError);
...
async function loadMemories() {
  try {
    ...
    if (!mem0client) {
      ...
      if (!initialized) {
        memoriesContainer.innerHTML =
          '<div class="error">Please set your Kozmodb API key in the extension options.</div>';
        return;
      }
    }
