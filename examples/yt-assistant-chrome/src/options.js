...
async function init() {
  ...
  await loadConfig();
  // Initialize KozmodbAI and load memories
  await initializeKozmodbAI();
  await fetchMemories();
}

// Initialize KozmodbAI with API key from storage
async function initializeKozmodbAI() {
  try {
    ...
    showMemoriesError("Failed to initialize KozmodbAI");
    return false;
  }
}
