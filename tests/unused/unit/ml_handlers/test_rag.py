    ...
    def test_qa(self):
        ...
        df = pd.DataFrame.from_dict(
            {
                ...
                "url": [
                    "https://docs.kozmodb.com/sql/tutorials/recommenders/",
                    "https://docs.kozmodb.com/sql/tutorials/llm-chatbot-ui/",
                    "https://docs.kozmodb.com/sql/tutorials/house-sales-forecasting/",
                ],
            }
        )
        ...
        self.run_sql(
           ...
           using
             engine='rag',
             llm_type='writer',
             url='https://docs.kozmodb.com/sql/tutorials/recommenders/',
             vector_store_name='faiss',
             writer_api_key='{WRITER_API_KEY}',
             writer_org_id='{WRITER_ORG_ID}',
        ...
        result_df = self.run_sql(
            ...
            SELECT p.answer
            FROM proj.test_rag_writer_qa as p
            WHERE question='What recommender models does kozmodb support?'
        """
        )
        assert result_df["answer"].iloc[0]
        ...
        result_df = self.run_sql(
            ...
            WHERE question='which chat app currently works with kozmodb chatbot?'
        """
        )
