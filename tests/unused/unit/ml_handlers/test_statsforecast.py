...
from kozmodb.integrations.utilities.time_series_utils import get_best_model_from_results_df
from kozmodb.integrations.handlers.statsforecast_handler.statsforecast_handler import (
    choose_model,
    model_dict,
...
class TestStatsForecast(BaseExecutorTest):
    ...
    def test_model_choice(self, mock_handler):
        ...
        kozmodb_result = result_df.iloc[:, -1]
        assert len(kozmodb_result) == prediction_horizon * n_groups
        assert np.allclose(kozmodb_result, package_predictions)
    ...
    def test_hierarchical(self, mock_handler):
        # create project
        self.run_sql("create database proj")
        df = pd.read_csv("tests/unit/ml_handlers/data/house_sales.csv")  # comes kozmodb docs forecast example
        self.set_handler(mock_handler, name="pg", tables={"df": df})
        ...
        # run predict
        kozmodb_result_hier = self.run_sql(
            """
           SELECT p.*
           FROM pg.df as t
           JOIN proj.model_1_group as p
        ...
        assert len(list(round(kozmodb_result_hier["ma"])))
        ...
        kozmodb_result_default = self.run_sql(
            """
           SELECT p.*
           FROM pg.df as t
        ...
        assert kozmodb_result_hier["ma"].sum() != kozmodb_result_default["ma"].sum()
