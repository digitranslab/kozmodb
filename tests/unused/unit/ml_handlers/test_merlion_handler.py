...
from kozmodb.integrations.handlers.merlion_handler.adapters import DefaultForecasterAdapter, SarimaForecasterAdapter, \
...
class TestMerlion(BaseExecutorTest):
    def set_project(self):
        r = self.db.Project.query.filter_by(name='kozmodb').first()
        if r is not None:
    ...
    def exec_train_and_forecast(self, mock_handler, model_name, using):
        # create predictor
        create_sql = f'''
                    CREATE PREDICTOR kozmodb.{model_name}_forecaster
                    FROM pg
                    (select t, H1 from m4 where train = 1)
                    PREDICT H1
                    USING engine='merlion'{using}
                '''
        ret = self.run_kozmodb_sql(sql=create_sql)
        ...
        predict_sql = f'''
                    ...
                    from kozmodb.{model_name}_forecaster t
                    inner join pg.m4 p on t.t = p.t
                    where p.train = 0
                '''
        ret = self.run_kozmodb_sql(sql=predict_sql)
    ...
    def exec_train_and_detect(self, mock_handler, model_name, using_model):
        # create predictor
        create_sql = f'''
                    CREATE PREDICTOR kozmodb.{model_name}_detector
                    FROM pg
        ...
        predict_sql = f'''
                    ...
                    from kozmodb.{model_name}_detector d
    ...
    def wait_training(self, model_name):
        ...
        for attempt in range(900):
            ret = self.run_kozmodb_sql(
                f"select status from kozmodb.predictors where name='{model_name}'"
            )
            if len(ret.data) > 0:
