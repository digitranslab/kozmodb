...
class TestHuggingface(BaseExecutorTest):
    ...
    def hf_test_run(self, mock_handler, model_name, create_sql, predict_sql):
        ...
        for attempt in range(900):
            ret = self.run_sql(
                f"select status from kozmodb.models where name='{model_name}'"
            )
    ...
    def test_hf_classification_bin(self, mock_handler):
        # create predictor
        create_sql = """
            CREATE PREDICTOR kozmodb.spam_classifier
            predict PRED
            USING
                engine='huggingface',
        ...
        # one line prediction
        predict_sql = """
            SELECT * from kozmodb.spam_classifier
    ...
    def test_hf_classification_multy(self, mock_handler):
        # create predictor
        create_sql = """
           CREATE PREDICTOR kozmodb.sentiment_classifier
           predict PRED
           USING
                engine='huggingface',
                join_learn_process=true,
        ...
        predict_sql = """
            SELECT h.*
            FROM pg.df as t
            JOIN kozmodb.sentiment_classifier as h
        """
        self.hf_test_run(mock_handler, model_name, create_sql, predict_sql)
    ...
    def test_hf_zero_shot(self, mock_handler):
        # create predictor
        create_sql = """
         CREATE PREDICTOR kozmodb.zero_shot_tcd
            predict PREDZS
         USING
            engine='huggingface',
            join_learn_process=true,
        ...
        predict_sql = """
            SELECT h.*
            FROM pg.df as t
            JOIN kozmodb.zero_shot_tcd as h
        """
        self.hf_test_run(mock_handler, model_name, create_sql, predict_sql)
    ...
    def test_summarization(self, mock_handler):
        # create predictor
        create_sql = """
        CREATE MODEL kozmodb.hf_summarization
        PREDICT summary
        USING
         engine = 'huggingface',
         task = 'summarization',
        ...
        predict_sql = """
                    SELECT h.*
                    FROM pg.df as t
                    JOIN kozmodb.hf_summarization as h
                """
    ...
    def test_hf_translation(self, mock_handler):
        # create predictor
        create_sql = """
         CREATE PREDICTOR kozmodb.translator_en_fr
            predict TRANSLATION
        USING
            engine='huggingface',
            join_learn_process=true,
        ...
        predict_sql = """
            SELECT h.*
            FROM pg.df as t
            JOIN kozmodb.translator_en_fr as h
        """
        self.hf_test_run(mock_handler, model_name, create_sql, predict_sql)
    ...
    def test_hf_text2text(self, mock_handler):
        # create predictor
        create_sql = """
        CREATE MODEL kozmodb.text_generator
        predict PREDICTION
        USING
            engine='huggingface',
            join_learn_process=true,
    ...
    def test_hf_text_classification_finetune(self, mock_handler):
        ...
        fine_tune_sql = """
        FINETUNE kozmodb.spam_classifier
        FROM pg (
    ...
    def test_hf_zero_shot_classification_finetune(self, mock_handler):
        ...
        fine_tune_sql = """
        FINETUNE kozmodb.zero_shot_tcd
    ...
    def test_hf_translation_finetune(self, mock_handler):
        ...
        fine_tune_sql = """
        FINETUNE kozmodb.translator_en_fr
    ...
    def test_hf_summarization_finetune(self, mock_handler):
        ...
        # fine tune
        fine_tune_sql = """
        FINETUNE kozmodb.hf_summarization
        FROM pg (
