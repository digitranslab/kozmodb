...
from kozmodb.integrations.utilities.time_series_utils import (
...
def test_statsforecast_df_transformations():
    ...
    kozmodb_results_df = get_results_from_nixtla_df(nixtla_results_df, settings_dict)
    pd.testing.assert_frame_equal(kozmodb_results_df, df[["time_col", "target_col", "group_col"]])
    ...
    nixtla_results_df = nixtla_df.rename({"y": model_name}, axis=1).set_index("unique_id")
    kozmodb_results_df = get_results_from_nixtla_df(nixtla_results_df, settings_dict)
    pd.testing.assert_frame_equal(kozmodb_results_df, df)
