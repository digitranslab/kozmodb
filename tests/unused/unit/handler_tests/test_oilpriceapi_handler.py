...
class TestOilPriceAPIHandler(BaseExecutorTest):
    ...
    def test_basic_select_from(self):
        sql = "SELECT * FROM kozmodb_oilpriceapi.latest_price;"
        assert self.run_sql(sql).shape[0] == 1

        sql = "SELECT * FROM kozmodb_oilpriceapi.past_day_price;"
        assert self.run_sql(sql).shape[0] == 20

    def test_complex_select(self):
        sql = 'SELECT price FROM kozmodb_oilpriceapi.latest_price where by_type="daily_average_price" and by_code="WTI_USD";'
