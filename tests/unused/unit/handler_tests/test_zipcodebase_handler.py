...
class TestZipCodeBaseHandler(BaseExecutorTest):
    ...
    def test_basic_select_from(self):
        sql = "SELECT * FROM kozmodb_zipcodebase.code_to_location where codes='10005';"
        self.run_sql(sql)

        sql = 'SELECT * FROM kozmodb_zipcodebase.codes_within_radius WHERE code="10005" AND radius="100" AND country="us";'
        self.run_sql(sql)

    def test_complex_select(self):
        sql = 'SELECT state FROM kozmodb_zipcodebase.codes_within_radius WHERE code="10005" AND radius="100" AND country="us";'
        assert self.run_sql(sql).shape[1] == 6

        sql = "SELECT * FROM kozmodb_zipcodebase.code_to_location where codes='10005'; LIMIT 1;"
