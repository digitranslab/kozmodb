...
class TestSymblAPIHandler(BaseExecutorTest):
    ...
    def test_basic_select_from(self):
        ...
        sql = f'SELECT * FROM kozmodb_symbl.get_messages where conversation_id="{conversation_id}"'
        self.run_sql(sql)

        sql = f'SELECT * FROM kozmodb_symbl.get_topics where conversation_id="{conversation_id}"'
        self.run_sql(sql)

        sql = f'SELECT * FROM kozmodb_symbl.get_questions where conversation_id="{conversation_id}"'
        self.run_sql(sql)

        sql = f'SELECT * FROM kozmodb_symbl.get_analytics where conversation_id="{conversation_id}"'
        self.run_sql(sql)

        sql = f'SELECT * FROM kozmodb_symbl.get_action_items where conversation_id="{conversation_id}"'
        self.run_sql(sql)

        sql = f'SELECT * FROM kozmodb_symbl.get_follow_ups where conversation_id="{conversation_id}"'
        self.run_sql(sql)
