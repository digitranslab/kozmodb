import unittest
from kozmodb.integrations.handlers.instatus_handler.instatus_handler import InstatusHandler
...
class InstatusHandlerTest(unittest.TestCase):
    ...
    @classmethod
    def setUpClass(cls):
        cls.handler = InstatusHandler(name='kozmodb_instatus', connection_data={'api_key': os.environ.get('INSTATUS_API_KEY')})
    ...
    def test_6_insert_status_pages(self):
        query = f'''INSERT INTO kozmodb_instatus.status_pages (email, name, subdomain, components, logoUrl) VALUES ('{os.environ.get('EMAIL_ID')}', 'kozmodb', 'somtirtha-roy', '["Website", "App", "API"]', 'https://instatus.com/sample.png')'''
        try:
            self.assertTrue(self.handler.native_query(query))
    ...
    def test_7_update_status_pages(self):
        ...
        query = f'''UPDATE kozmodb_instatus.status_pages
    ...
    def test_10_insert_components(self):
        query = f'''INSERT INTO kozmodb_instatus.components (page_id, name, description, status, order, showUptime, grouped, translations_name_in_fr, translations_desc_in_fr)
                    VALUES (
                        '{self.pageId}',
                        'Test component',
                        'Testing',
    ...
    def test_11_update_components(self):
        query = f'''UPDATE kozmodb_instatus.components
                    SET
