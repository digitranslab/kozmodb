...
class TestAQICNHandler(BaseExecutorTest):
    ...
    def test_basic_select_from(self):
        sql = 'SELECT * FROM kozmodb_aqicn.air_quality_city where city="Bangalore";'
        assert self.run_sql(sql).shape[0] == 1

        sql = 'SELECT * FROM kozmodb_aqicn.air_quality_lat_lng where lat="12.938539" AND lng="77.5901";'
        assert self.run_sql(sql).shape[0] == 1

        sql = 'SELECT * FROM kozmodb_aqicn.air_quality_user_location;'
        assert self.run_sql(sql).shape[0] == 1

    def test_complex_select(self):
        sql = 'SELECT data.city.url, data.city.name FROM kozmodb_aqicn.air_quality_lat_lng where lat="12.938539" AND lng="77.5901";'
        assert self.run_sql(sql).shape[1] == 2

        sql = 'SELECT * FROM kozmodb_aqicn.air_quality_user_location LIMIT 1;'
        assert self.run_sql(sql).shape[0] == 1
