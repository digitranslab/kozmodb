...
from kozmodb.integrations.handlers.gmail_handler.gmail_handler import GmailHandler
from kozmodb.integrations.handlers.gmail_handler.gmail_handler import EmailsTable
...
class GmailHandlerTest(unittest.TestCase):
    ...
    @patch('kozmodb.integrations.handlers.gmail_handler.gmail_handler.requests.get')  # Patching the requests.get method
    ...
    def test_has_creds_file_with_invalid_s3_link(self, mock_get):
        ...
        with patch('kozmodb.utilities.log.logger') as mock_logger:
            result = self.handler._has_creds_file(self.credentials_file)
            # Assert that the requests.get method was called with the correct URL
    ...
    def test_create_connection_with_mocked_credentials_file_and_s3(self):
        with mock.patch('google.oauth2.credentials.Credentials.from_authorized_user_file') as mock_credentials:
            ...
            with mock.patch('os.path.isfile') as mock_isfile:
                mock_isfile.return_value = True
                with mock.patch(
                        'kozmodb.integrations.handlers.gmail_handler.gmail_handler.GmailHandler._has_creds_file') \
