...
from kozmodb.utilities.ml_task_queue.const import TASKS_STREAM_NAME
...
REDIS_HOST = os.environ.get("INTERNAL_URL", "").replace("kozmodb", "redis-master")
...
class TestMLTaskQueue(HTTPHelperMixin):
    ...
    def test_predict(self):
        ...
        query = """
            ...
            FROM kozmodb.p_test_queue_async
            WHERE b = 10;
        """
        response = self.sql_via_http(query, RESPONSE_TYPE.TABLE)
        ...
        query = """
            ...
            JOIN kozmodb.p_test_queue_async
            LIMIT 3;
        """
        response = self.sql_via_http(query, RESPONSE_TYPE.TABLE)
