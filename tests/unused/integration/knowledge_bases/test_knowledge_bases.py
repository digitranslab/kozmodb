...
def app():
    ...
    with TemporaryDirectory(prefix='knowledge_bases_integration_test_') as temp_dir:
        ...
        response = test_client.post('/api/projects/kozmodb/models', json=train_data, follow_redirects=True)
        ...
        while not model_complete:
            ...
            response = test_client.get('/api/projects/kozmodb/models/test_embedding_model')
        ...
        yield app
    os.environ['KOZMODB_DB_CON'] = old_kozmo_db_con
...
class TestKnowledgeBaseCompletion:
    def test_chat_completion(self, client, pgvector_database_name):
        ...
        create_kb_response = client.post('/api/projects/kozmodb/knowledge_bases', json=create_request, follow_redirects=True)
        assert create_kb_response.status_code == HTTPStatus.CREATED

        # Insert documents to help answer the question.
        rows_to_insert = [
            {'content': 'The capital of Tyler Fantasy RAG Land is KozmoDB'}
        ]
        update_request = {
            'knowledge_base': {
                'rows': rows_to_insert
        ...
        update_kb_response = client.put(f'/api/projects/kozmodb/knowledge_bases/{test_kb_name}',
        ...
        response = client.post(f'/api/projects/kozmodb/knowledge_bases/{test_kb_name}/completions',
        ...
        assert 'kozmodb' in response_data['message']['content'].lower()
    ...
    def test_context_completion_with_metadata(self, client, pgvector_database_name):
        ...
        # Insert documents for context.
        rows_to_insert = [
            {'content': 'The capital of Tyler Fantasy RAG Land is KozmoDB', 'author': 'Danya'},
