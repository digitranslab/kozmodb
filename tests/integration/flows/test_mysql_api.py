...
from kozmodb.api.mysql.mysql_proxy.libs.constants.mysql import DATA_C_TYPE_MAP, MYSQL_DATA_TYPE
...
class BaseStuff:
    ...
    def check_predictor_readiness(self, predictor_name):
        ...
        res = ""
        while time.time() < threshold:
            _query = "SELECT status, error FROM kozmodb.models WHERE name='{}';".format(predictor_name)
...
class TestMySqlApi(BaseStuff):
    ...
    def test_describe_predictor_attrs(self, describe_attr, use_binary):
        self.query(f"describe kozmodb.{self.predictor_name}.{describe_attr};")
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            "show schemas;",
            "show tables;",
            "show tables from kozmodb;",
            "show tables in kozmodb;",
            "show full tables from kozmodb;",
            "show full tables in kozmodb;",
            "show variables;",
            "show session status;",
    ...
    def test_train_model_from_files(self, use_binary):
        ...
        self.query(f"DROP MODEL IF EXISTS kozmodb.{file_predictor_name};")
        # add file lock here
        _query = f"""
            CREATE MODEL kozmodb.{file_predictor_name}
    ...
    def test_ts_train_and_predict(self, subtests, use_binary):
        ...
        params = [
            (
                "with_group_by_hor1",
                f"CREATE MODEL kozmodb.%s from files (select * from {train_ds_name}) PREDICT y ORDER BY oby GROUP BY gby WINDOW 10 HORIZON 1;",
                f"SELECT res.gby, res.y as PREDICTED_RESULT FROM files.{test_ds_name} as source JOIN kozmodb.%s as res WHERE source.gby= 'A' LIMIT 1;",
                1,
            ),
            (
                "no_group_by_hor1",
                f"CREATE MODEL kozmodb.%s from files (select * from {train_ds_name}) PREDICT y ORDER BY oby WINDOW 10 HORIZON 1;",
                f"SELECT res.gby, res.y as PREDICTED_RESULT FROM files.{test_ds_name} as source JOIN kozmodb.%s as res LIMIT 1;",
                1,
            ),
            (
                "with_group_by_hor2",
                f"CREATE MODEL kozmodb.%s from files (select * from {train_ds_name}) PREDICT y ORDER BY oby GROUP BY gby WINDOW 10 HORIZON 2;",
                f"SELECT res.gby, res.y as PREDICTED_RESULT FROM files.{test_ds_name} as source JOIN kozmodb.%s as res WHERE source.gby= 'A' LIMIT 2;",
                2,
            ),
            (
                "no_group_by_hor2",
                f"CREATE MODEL kozmodb.%s from files (select * from {train_ds_name}) PREDICT y ORDER BY oby WINDOW 10 HORIZON 2;",
                f"SELECT res.gby, res.y as PREDICTED_RESULT FROM files.{test_ds_name} as source JOIN kozmodb.%s as res LIMIT 2;",
                2,
            ),
        ]
        for predictor_name, create_query, select_query, res_len in params:
            ...
            with subtests.test(
                ...
                self.query(f"DROP MODEL IF EXISTS kozmodb.{predictor_name};")
    ...
    def test_tableau_queries(self, subtests, use_binary):
        ...
        queries = [
            ...
            f"""
                ...
                FROM (
                  SELECT res.x1, res.y
                   ...
                   JOIN kozmodb.{predictor_name} as res
                ) `Custom SQL Query`
                HAVING (COUNT(1) > 0)
            """,
            f"""
            ...
            """,
            """
                SHOW KEYS FROM `kozmodb`.`predictors`
            """,
            """
            ...
            """
                SELECT `table_name`, `column_name` FROM `information_schema`.`columns`
                 WHERE `data_type`='enum' AND `table_schema`='kozmodb'
            """,
            f"""
