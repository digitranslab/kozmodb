...
class TestCompanyIndependent(HTTPHelperMixin):
    ...
    def test_add_data_db_http(self):
        self.sql_via_http("CREATE PROJECT kozmodb;", company_id=CID_A)
        self.sql_via_http("CREATE PROJECT kozmodb;", company_id=CID_B)
        db_details = {
            "type": "postgres",
            "connection_data": {
                "type": "postgres",
    ...
    def test_views(self):
        ...
        for cid, char in [(CID_A, 'a'), (CID_B, 'b')]:
            ...
            tables = self.get_tables_in('kozmodb', cid)
            self.assert_list(
                tables, {
                    'models',
                    f'test_view_{char}'
