...
class TestHTTP(HTTPHelperMixin):
    ...
    def test_auth(self):
        ...
        response = session.put(
            ...
            json={"auth": {"http_auth_enabled": True, "username": "kozmodb", "password": "kozmodb"}},
        )
        assert response.status_code == 200
        ...
        response = session.post(f"{HTTP_API_ROOT}/login", json={"username": "kozmodb", "password": "kozmodb"})
        assert response.status_code == 200
        ...
        response = session.put(
            f"{HTTP_API_ROOT}/config/",
            json={"auth": {"http_auth_enabled": False, "username": "kozmodb", "password": ""}},
        )
    ...
    def test_sql_select_from_file(self):
        self.sql_via_http("use kozmodb", RESPONSE_TYPE.OK)
    ...
    def test_sql_general_syntax(self):
        ...
        select_const_int_alias = [
            "select 1 as `2`",
            # "select 1 as '2'",     https://github.com/digitranslab/kozmodb_sql/issues/198
            'select 1 as "2"',
            "select 1 `2`",
            # "select 1 '2'",     https://github.com/digitranslab/kozmodb_sql/issues/198
            'select 1 "2"',
        ]
    ...
    def test_context_changing(self):
        resp = self.sql_via_http("use kozmodb", RESPONSE_TYPE.OK)
        assert resp["context"]["db"] == "kozmodb"
    ...
    def test_special_queries_show_databases(self):
        ...
        assert "kozmodb" in db_names
        assert "files" in db_names

    def test_show_tables(self):
        self.sql_via_http("use kozmodb", RESPONSE_TYPE.OK)
        resp_1 = self.sql_via_http("show tables", RESPONSE_TYPE.TABLE)
        resp_2 = self.sql_via_http("show tables from kozmodb", RESPONSE_TYPE.TABLE)
        resp_3 = self.sql_via_http("show full tables from kozmodb", RESPONSE_TYPE.TABLE)
    ...
    def test_sql_create_predictor(self, train_finetune_lock):
        ...
        resp = self.sql_via_http(
            """
            select * from kozmodb.p_test_http_1 where sqft = 1000
        """,
            RESPONSE_TYPE.TABLE,
        )
