...
def mock_text_embedding_model():
    with patch("kozmodb.embeddings.vertexai.TextEmbeddingModel") as mock_model:
        mock_instance = Mock()
...
@pytest.fixture
def mock_os_environ():
    with patch("kozmodb.embeddings.vertexai.os.environ", {}) as mock_environ:
        yield mock_environ


@pytest.fixture
def mock_config():
    with patch("kozmodb.configs.embeddings.base.BaseEmbedderConfig") as mock_config:
...
def mock_text_embedding_input():
    with patch("kozmodb.embeddings.vertexai.TextEmbeddingInput") as mock_input:
        yield mock_input


@patch("kozmodb.embeddings.vertexai.TextEmbeddingModel")
...
@patch("kozmodb.embeddings.vertexai.os")
