...
UTILITIES_REQS_PATHS = [
    "kozmodb/integrations/utilities/handlers/auth_utilities/microsoft/requirements.txt",
    "kozmodb/integrations/utilities/handlers/auth_utilities/google/requirements.txt",
]
...
MAIN_EXCLUDE_PATHS = ["kozmodb/integrations/handlers/.*_handler", "pryproject.toml"]
...
# and not explicitly imported in kozmodb.
# transformers is required for langchain_core and not explicitly imported by kozmodb.
...
HANDLER_RULE_IGNORES = {
    ...
    "DEP001": [
        ...
        "IfxPyDbi",
        "ingres_sa_dialect",
    ],  # 'tests' is the kozmodb tests folder in the repo root, 'pyarrow' used in snowflake handler
...
def run_deptry(reqs, rule_ignores, path, extra_args=""):
    ...
    try:
        result = subprocess.run(
            f'deptry -o deptry.json --no-ansi --known-first-party kozmodb --requirements-files "{reqs}" --per-rule-ignores "{rule_ignores}" --package-module-name-map "{get_ignores_str(PACKAGE_NAME_MAP)}" {extra_args} {path}',
            shell=True,
...
def check_relative_reqs():
    ...
    -R kozmodb/integrations/handlers/child_handler/requirements.txt

    This is important to ensure that "pip install kozmodb[parent_handler]" works correctly.
        ...
        for file in files:
            with open(file, "r") as fh:
                for line in fh.readlines():
                    ...
                    if line.startswith("-r kozmodb/integrations/handlers/"):
                        entries[line.split("kozmodb/integrations/handlers/")[1].split("/")[0]] = line

        return entries

    for handler_dir in glob.glob("kozmodb/integrations/handlers/*/"):
        handler_name = handler_dir.split("/")[-2].split("_handler")[0]

        # regex for finding imports of other handlers like "from kozmodb.integrations.handlers.file_handler import FileHandler"
        # excludes the current handler importing parts of itself
        import_pattern = re.compile(
            f"(?:\s|^)(?:from|import) kozmodb\.integrations\.handlers\.(?!{handler_name}_handler)\w+_handler"
        )  # noqa: W605
        ...
        for file in glob.glob(f"{handler_dir}/**/*.py", recursive=True):
            ...
            for line, imported_handler_name in imported_handlers.items():
                ...
                imported_handler_req_file = f"kozmodb/integrations/handlers/{imported_handler_name}/requirements.txt"
