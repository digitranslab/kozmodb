...
def test_get_tables(client):
    # Get default kozmodb tables.
    response = client.get('/api/databases/kozmodb/tables', follow_redirects=True)
    all_tables = response.get_json()
    assert any(db['name'] == 'models' for db in all_tables)


def test_get_table(client):
    # Get default kozmodb models.
    response = client.get('/api/databases/kozmodb/tables/models', follow_redirects=True)
...
def test_get_table_not_found(client):
    # Get default kozmodb models.
    response = client.get('/api/databases/kozmodb/tables/bloop', follow_redirects=True)
...
def test_create_table_in_project_aborts(client):
    ...
    response = client.post('/api/databases/kozmodb/tables', json=table_data, follow_redirects=True)
    assert response.status_code == HTTPStatus.BAD_REQUEST
...
def test_delete_project_table_aborts(client):
    response = client.delete('/api/databases/kozmodb/tables/models', follow_redirects=True)
    assert response.status_code == HTTPStatus.BAD_REQUEST
