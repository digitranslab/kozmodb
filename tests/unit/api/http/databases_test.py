...
def test_get_databases(client):
    ...
    assert any(db["name"] == "kozmodb" for db in all_databases)
    assert any(db["name"] == "log" for db in all_databases)


def test_get_database(client):
    # Get default kozmodb project.
    response = client.get("/api/databases/kozmodb", follow_redirects=True)
    kozmodb_database = response.get_json()
    expected_db = {"name": "kozmodb", "engine": None, "type": "project", "id": kozmodb_database["id"]}

    assert kozmodb_database == expected_db

    response = client.get("/api/databases/KozmoDB", follow_redirects=True)
    kozmodb_database = response.get_json()
    kozmodb_database["name"] = kozmodb_database["name"].lower()
    assert kozmodb_database == expected_db
...
def test_create_database(client):
    kozmodb_data = {"database": {"name": "test_postgres", "engine": "postgres", "parameters": {}}}
    response = client.post("/api/databases", json=kozmodb_data, follow_redirects=True)
...
def test_create_database_already_exists_abort(client):
    kozmodb_data = {"database": {"name": "test_duplicate", "engine": "postgres", "parameters": {}}}
    response = client.post("/api/databases", json=kozmodb_data, follow_redirects=True)
    assert response.status_code == HTTPStatus.CREATED
    create_duplicate_response = client.post("/api/databases", json=kozmodb_data, follow_redirects=True)
...
def test_create_database_no_database_aborts(client):
    kozmodb_data = {"name": "test_postgres", "engine": "postgres", "parameters": {}}
    response = client.post("/api/databases", json=kozmodb_data, follow_redirects=True)
    assert response.status_code == HTTPStatus.BAD_REQUEST


def test_create_database_no_name_aborts(client):
    kozmodb_data = {"database": {"engine": "postgres", "parameters": {}}}
    response = client.post("/api/databases", json=kozmodb_data, follow_redirects=True)
    assert response.status_code == HTTPStatus.BAD_REQUEST


def test_create_database_no_engine_aborts(client):
    kozmodb_data = {"database": {"name": "test_postgres", "parameters": {}}}
    response = client.post("/api/databases", json=kozmodb_data, follow_redirects=True)
    assert response.status_code == HTTPStatus.BAD_REQUEST
...
def test_update_database_no_database_aborts(client):
    kozmodb_data = {"name": "test_postgres", "engine": "postgres", "parameters": {}}
    response = client.put("/api/databases/test_postgres", json=kozmodb_data, follow_redirects=True)
    assert response.status_code == HTTPStatus.BAD_REQUEST


def test_delete_database(client):
    kozmodb_data = {"database": {"name": "test_delete", "engine": "postgres", "parameters": {}}}
    # Delete newly created DB.
    client.post("/api/databases", json=kozmodb_data, follow_redirects=True)
