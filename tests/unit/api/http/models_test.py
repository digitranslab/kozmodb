...
def test_train_model(client):
    ...
    create_query = """
        CREATE MODEL kozmodb.home_rentals_model
    ...
    expected_model = {
        ...
        "kozmodb_version": created_model["kozmodb_version"],
...
def test_train_model_no_query_aborts(client):
    response = client.post("/api/projects/kozmodb/models", json={}, follow_redirects=True)
...
def test_train_model_invalid_query_aborts(client):
    invalid_create_query = """
        CREATE MAWDOL kozmodb.home_rentals_model
...
def test_train_model_already_exists_aborts(client):
    # Learning Hub home rentals model.
    create_query = """
        CREATE MODEL kozmodb.home_rentals_model_duplicate
        FROM example_db (SELECT * FROM demo_data.home_rentals limit 10)
        PREDICT rental_price
        USING engine = 'dummy_ml', join_learn_process = true
    """
...
def test_train_model_no_ml_handler_aborts(client):
    # Learning Hub home rentals model.
    create_query = """
        CREATE MODEL kozmodb.home_rentals_model_no_handler
...
def test_get_model_by_version(client):
    response = client.get("/api/projects/kozmodb/models/home_rentals_model.1", follow_redirects=True)
    model_ver_1 = response.get_json()
    response = client.get("/api/projects/kozmodb/models/home_rentals_model", follow_redirects=True)
...
def test_predict_model_no_model_aborts(client):
    response = client.post("/api/projects/kozmodb/models/plumbus/predict", follow_redirects=True)
    assert response.status_code == HTTPStatus.NOT_FOUND
...
def test_describe_model_no_model_aborts(client):
    response = client.get("/api/projects/kozmodb/models/plumbus/describe", follow_redirects=True)
    assert response.status_code == HTTPStatus.NOT_FOUND
...
def test_delete_model(client):
    # Learning Hub home rentals model.
    create_query = """
        CREATE MODEL kozmodb.home_rentals_model_delete
        FROM example_db (SELECT * FROM demo_data.home_rentals limit 10)
        PREDICT rental_price
        USING engine = 'dummy_ml', join_learn_process = true
    """
    ...
    response = client.delete("/api/projects/kozmodb/models/home_rentals_model_delete", follow_redirects=True)
    assert response.status_code == HTTPStatus.NO_CONTENT

    response = client.get("/api/projects/kozmodb/models/home_rentals_model_delete", follow_redirects=True)
    assert response.status_code == HTTPStatus.NOT_FOUND
...
def test_delete_model_no_model_aborts(client):
    response = client.delete("/api/projects/kozmodb/models/meeseeks", follow_redirects=True)
    assert response.status_code == HTTPStatus.NOT_FOUND
