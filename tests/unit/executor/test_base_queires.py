...
class TestSelect(BaseExecutorDummyML):
    def test_view(self):
        ...
        self.run_sql(
            """
                CREATE model kozmodb.task_model
                from kozmodb (select * from Vtasks)
                PREDICT a
                using engine='dummy_ml'
            """
        )
        self.wait_predictor("kozmodb", "task_model")

        # use model
        ret = self.run_sql("""
            SELECT m.*
            FROM kozmodb.vtasks as t
            JOIN kozmodb.task_model as m
        """)
    ...
    def test_complex_joins(self, data_handler):
        ...
        ret = self.run_sql("""
            SELECT t1.a t1a, t3.a t3a, m.*
              FROM dummy_data.tbl1 as t1
              JOIN kozmodb.pred m
              LEFT JOIN kozmodb.view2 as t3 on t1.c=t3.c
              where t1.a>1
        """)
    ...
    def test_complex_queries(self):
        ...
        # -- create view --
        self.run_sql("""
            create view kozmodb.stores_view (
        ...
        self.wait_predictor("kozmodb", "model1")
        ...
        self.wait_predictor("kozmodb", "model2")
    ...
    def test_mysql_queries(self):
        self.run_sql("SHOW KEYS FROM `kozmodb`.`predictors`")
    ...
    def test_select_without_table(self):
        test_data = (("session_user", None), ("version()", "8.0.17"), ("@@version_comment", "(KozmoDB)"), ("1", 1))
