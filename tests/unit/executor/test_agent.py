...
from kozmodb.interfaces.agents.langchain_agent import SkillData
...
class TestAgent(BaseExecutorDummyML):
    @pytest.mark.slow
    def test_kozmodb_provider(self):
    ...
    @patch("kozmodb.utilities.config.Config.get")
    @patch("openai.OpenAI")
    def test_agent_with_default_llm_params(self, mock_openai, mock_config_get):
        ...
        with (
            ...
            patch("kozmodb.interfaces.agents.langchain_agent.LangchainAgent._initialize_args") as mock_initialize_args,
        ):
    ...
    def test_agent_retrieval(self, mock_openai, mock_litellm_embedding):
        ...
        with patch("kozmodb.interfaces.knowledge_base.controller.KnowledgeBaseTable.select_query") as kb_select:
            # kb response
    ...
    @patch("openai.OpenAI")
    @patch("kozmodb.integrations.handlers.litellm_handler.litellm_handler.embedding")
    ...
    def test_agent_accept_wrong_quoting(self, mock_litellm_embedding, mock_openai):
        ...
        set_openai_completion(
            mock_openai,
            [
                self._action("kb_query_tool", "SELECT * FROM `kozmodb.kb1` WHERE id = '1001'"),
    ...
    @patch("openai.OpenAI")
    @patch(
        "kozmodb.interfaces.agents.langchain_agent.LangchainAgent.run_agent",
