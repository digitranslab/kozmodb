...
from kozmodb.api.executor.utilities.sql import query_df
from kozmodb.api.mysql.mysql_proxy.utilities.lightwood_dtype import dtype
...
class Test(BaseExecutorMockPredictor):
    ...
    def test_describe(self):
        ...
        self.execute("""
            CREATE MODEL kozmodb.test_predictor
            PREDICT target
            USING
                engine = 'dummy_ml'
        """)
        ...
        assert ret.data.records[0]["PROJECT"] == "kozmodb"
    ...
    @patch("kozmodb.integrations.handlers.postgres_handler.Handler")
    ...
    def test_predictor_1_row(self):
        ...
        ret = self.execute("""
             select p, a from kozmodb.task_model where a = 2
        """)
    ...
    def test_dates(self, mock_handler):
        ...
        ret = self.execute("""
            ...
            FROM (
               ...
               JOIN kozmodb.task_model as res
            )
            group by 1
            order by a
        """)
    ...
    def test_ts_predictor(self, mock_handler):
        ...
        predict_result = [
            # window
            {"a": 2, "t": dt.datetime(2020, 1, 2), "g": "x", "__kozmodb_row_id": 2},
            {"a": 3, "t": dt.datetime(2020, 1, 3), "g": "x", "__kozmodb_row_id": 3},
            # horizon
            {"a": 4, "t": dt.datetime(2020, 1, 4), "g": "x", "__kozmodb_row_id": None},
            {"a": 5, "t": dt.datetime(2020, 1, 5), "g": "x", "__kozmodb_row_id": None},
            {"a": 6, "t": dt.datetime(2020, 1, 6), "g": "x", "__kozmodb_row_id": None},
            # window
            {"a": 12, "t": dt.datetime(2021, 1, 2), "g": "y", "__kozmodb_row_id": 2},
            {"a": 13, "t": dt.datetime(2021, 1, 3), "g": "y", "__kozmodb_row_id": 3},
            # horizon
            {"a": 14, "t": dt.datetime(2021, 1, 4), "g": "y", "__kozmodb_row_id": None},
            {"a": 15, "t": dt.datetime(2021, 1, 5), "g": "y", "__kozmodb_row_id": None},
            {"a": 16, "t": dt.datetime(2021, 1, 6), "g": "y", "__kozmodb_row_id": None},
        ]
        ...
        predict_result = [
            # window
            {"a": 1, "t": dt.datetime(2020, 1, 1), "g": "x", "__kozmodb_row_id": 1},
            {"a": 2, "t": dt.datetime(2020, 1, 2), "g": "x", "__kozmodb_row_id": 2},
            {"a": 3, "t": dt.datetime(2020, 1, 3), "g": "x", "__kozmodb_row_id": 3},
            # horizon
            {"a": 1, "t": dt.datetime(2020, 1, 4), "g": "x", "__kozmodb_row_id": None},
            {"a": 1, "t": dt.datetime(2020, 1, 5), "g": "x", "__kozmodb_row_id": None},
            {"a": 1, "t": dt.datetime(2020, 1, 6), "g": "x", "__kozmodb_row_id": None},
        ]
        predict_result = pd.DataFrame(predict_result)
        self.mock_predict.side_effect = lambda *a, **b: predict_result
    ...
    def test_ts_predictor_fix_order_by_modification(self, mock_handler):
        ...
        predict_result = [
            # window
            {"a": 2, "t": dt.datetime(2020, 1, 1, 10, 1, 0), "g": "x", "__kozmodb_row_id": 2},
            {"a": 3, "t": dt.datetime(2020, 1, 1, 10, 2, 0), "g": "x", "__kozmodb_row_id": 3},
            # horizon
            {"a": 1, "t": dt.datetime(2020, 1, 1, 10, 3, 0), "g": "x", "__kozmodb_row_id": None},
            {"a": 1, "t": dt.datetime(2020, 1, 1, 10, 4, 0), "g": "x", "__kozmodb_row_id": None},
            {"a": 1, "t": dt.datetime(2020, 1, 1, 10, 5, 0), "g": "x", "__kozmodb_row_id": None},
        ]
        predict_result = pd.DataFrame(predict_result)
        self.mock_predict.side_effect = lambda *a, **b: predict_result
    ...
    def test_ts_predictor_file(self):
        ...
        predict_result = [
            # window
            {"a": 2, "t": np.float64(2022.0), "g": "x", "__kozmodb_row_id": 2},
            {"a": 3, "t": np.float64(2023.0), "g": "x", "__kozmodb_row_id": 3},
            # horizon
            {"a": 1, "t": np.float64(2024.0), "g": "x", "__kozmodb_row_id": None},
            {"a": 1, "t": np.float64(2024.0), "g": "x", "__kozmodb_row_id": None},
            {"a": 1, "t": np.float64(2025.0), "g": "x", "__kozmodb_row_id": None},
        ]
        predict_result = pd.DataFrame(predict_result)
        self.mock_predict.side_effect = lambda *a, **b: predict_result
    ...
    def test_drop_database(self, mock_handler):
        from kozmodb.utilities.exception import EntityNotExistsError
...
class TestComplexQueries(BaseExecutorMockPredictor):
    ...
    def test_union(self, mock_handler):
        ...
        sql = """
            ...
            FROM pg.tasks as t
            JOIN kozmodb.task_model as model
            WHERE t.a=1
        """
        # union all
    ...
    @patch("kozmodb.integrations.handlers.mysql_handler.Handler")
    ...
    #     '''
    #     from kozmodb.api.mysql.mysql_proxy.utilities import ErSqlWrongArguments
...
class TestWithNativeQuery(BaseExecutorMockPredictor):
    ...
    def test_view_native_query(self, mock_handler):
        ...
        # --- create view ---
        self.execute("create view kozmodb.vtasks (select * from pg (select * from tasks))")

        # --- select from view ---
        ret = self.execute("select * from kozmodb.vtasks")
        ...
        self.execute(f"alter view kozmodb.vtasks as ({view_query})")
    ...
    def test_use_predictor_with_view(self, mock_handler):
        ...
        view_name = "vtasks"
        # --- create view ---
        self.execute(f"create view kozmodb.{view_name} (select * from pg (select * from tasks))")
        ...
        ret = self.execute(f"""
           select m.p, v.a
           from kozmodb.{view_name} v
           join kozmodb.task_model m
           where v.a = 2
        """)
    ...
    def test_use_ts_predictor_with_view(self, mock_handler):
        ...
        ret = self.execute(f"""
           select task_model.*
           from kozmodb.{view_name}
           join kozmodb.task_model
...
class TestIfExistsIfNotExists(BaseExecutorMockPredictor):
    ...
    def test_ml_engine(self):
        from kozmodb.utilities.exception import EntityExistsError, EntityNotExistsError
    ...
    def test_project(self):
        from kozmodb.utilities.exception import EntityExistsError
    ...
    def test_view(self, mock_handler):
        ...
        ret = self.execute("""
            SHOW FULL TABLES
            WHERE tables_in_kozmodb = 'test_view2'
        """)
        assert len(ret.data) == 1
