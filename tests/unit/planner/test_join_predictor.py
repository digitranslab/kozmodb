...
from kozmodb_sql_parser.ast import Identifier, Select, Join, Constant, Star, Parameter, BinaryOperation
from kozmodb_sql_parser.utils import JoinType

from kozmodb.api.executor.planner.exceptions import PlanningException
from kozmodb.api.executor.planner import plan_query
from kozmodb.api.executor.planner.query_plan import QueryPlan
from kozmodb.api.executor.planner.step_result import Result
    ...
    def test_join_predictor_plan_aliases(self):
        ...
        expected_plan = QueryPlan(
            steps=[
                ...
                ApplyPredictorStep(
                    namespace="kozmodb", dataframe=Result(0), predictor=Identifier("pred", alias=Identifier("tb"))
                ),
                JoinStep(
                    left=Result(0),
                    right=Result(1),
    ...
    def test_join_predictor_plan_complex_query(self):
        ...
        expected_plan = QueryPlan(
            steps=[
                ...
                ApplyPredictorStep(
                    namespace="kozmodb", dataframe=Result(0), predictor=Identifier("pred", alias=Identifier("m"))
                ),
                JoinStep(
                    left=Result(0),
                    right=Result(1),
...
class TestPredictorWithUsing:
    def test_using_join(self):
        ...
        expected_plan = QueryPlan(
            steps=[
                ...
                ApplyPredictorStep(
                    namespace="kozmodb", dataframe=Result(0), predictor=Identifier("pred"), params={"a": 1}
                ),
                JoinStep(
                    left=Result(0),
                    right=Result(1),
        ...
        expected_plan = QueryPlan(
            steps=[
                ...
                ApplyPredictorStep(
                    namespace="kozmodb", dataframe=Result(1), predictor=Identifier("pred"), params={"a": 1}
                ),
                JoinStep(
                    left=Result(1),
                    right=Result(2),
    ...
    def test_using_one_line(self):
        ...
        expected_plan = QueryPlan(
            steps=[
                ApplyPredictorRowStep(
                    namespace="kozmodb", predictor=Identifier("pred"), row_dict={"x": 2}, params={"a": 1}
                ),
