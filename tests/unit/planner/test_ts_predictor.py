...
from kozmodb_sql_parser import parse_sql, NativeQuery, OrderBy, NullConstant
from kozmodb_sql_parser.ast import Select, Star, Identifier, Join, Constant, BinaryOperation, Update, BetweenOperation
from kozmodb_sql_parser.ast.kozmodb import Latest
...
class TestJoinTimeseriesPredictor:
    def test_join_predictor_timeseries(self):
        ...
        query = Select(
            ...
            from_table=Join(
                ...
                right=Identifier("kozmodb.tp3", alias=Identifier("tb")),
                join_type="left join",
            ),
        )
        ...
        expected_plan = QueryPlan(
            steps=[
                ...
                ApplyTimeseriesPredictorStep(
                    namespace="kozmodb", predictor=Identifier("tp3", alias=Identifier("tb")), dataframe=Result(1)
                ),
                JoinStep(
                    left=Result(1),
                    right=Result(2),
    ...
    def test_join_predictor_timeseries_concrete_date_greater(self):
        ...
        sql = "select * from mysql.data.ny_output as ta left join kozmodb.tp3 as tb where ta.pickup_hour > 10 and ta.vendor_id = 1"
    ...
    def test_join_predictor_timeseries_concrete_date_greater_2_group_fields(self):
        ...
        sql = "select * from mysql.data.ny_output as ta left join kozmodb.tp3 as tb\
    ...
    def test_join_predictor_timeseries_concrete_date_less(self):
        ...
        sql = "select * from mysql.data.ny_output as ta join kozmodb.tp3 as tb where ta.pickup_hour < 10 and ta.vendor_id = 1"
    ...
    def test_join_predictor_timeseries_error_on_nested_where(self):
        query = Select(
            ...
            from_table=Join(
                left=Identifier("int.tab1"), right=Identifier("kozmodb.pred"), join_type=None, implicit=True
            ),
            where=BinaryOperation(
                "and",
                args=[
    ...
    def test_timeseries_planner_not_changes_query(self):
        sql = "select * from ds.data as ta left join kozmodb.pr as tb where ta.f2 in ('a') and ta.f1 > LATEST"
        query = parse_sql(sql)
    ...
    def test_timeseries_without_group(self):
        sql = "select * from ds.data.ny_output as ta join kozmodb.pr as tb where ta.f1 > LATEST"
        query = parse_sql(sql)
