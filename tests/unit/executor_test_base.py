...
class BaseUnitTest:
    """
    kozmodb instance with temporal database and config
    """

    @staticmethod
    def setup_class(cls):
        # remove imports of kozmodb in previous tests
        unload_module("kozmodb")

        # database temp file

        cls.storage_dir = tempfile.mkdtemp(prefix="kozmodb_db_")

        cls.db_file = os.path.join(cls.storage_dir, "kozmodb.db")
        ...
        os.environ["KOZMODB_STORAGE_DIR"] = cls.storage_dir
        os.environ["KOZMODB_CONFIG_PATH"] = cfg_file
    ...
    def teardown_class(cls):
        ...
        if 'KOZMODB_DB_CON' in os.environ:
            del os.environ["KOZMODB_DB_CON"]

        # remove import of kozmodb for next tests
        unload_module("kozmodb")

    def setup_method(self):
        self._dummy_db_path = os.path.join(tempfile.mkdtemp(), '_kozmodb_duck_db')
...
class BaseExecutorTest(BaseUnitTest):
    ...
    def set_executor(
        ...
        from kozmodb.api.executor.controllers.session_controller import (
            SessionController,
        )
        from kozmodb.api.executor.command_executor import (
            ExecuteCommands,
        )
        # clear cache of previous test case to apply mocks of current test case
        from kozmodb.integrations.libs.process_cache import process_cache
        process_cache.cache = {}
        from kozmodb.interfaces.database.integrations import integration_controller
        from kozmodb.interfaces.file.file_controller import FileController
        from kozmodb.interfaces.model.model_controller import ModelController
        ...
        if mock_lightwood:
            predict_patcher = mock.patch("kozmodb.integrations.libs.ml_exec_base.BaseMLEngineExec.predict")
            self.mock_predict = predict_patcher.__enter__()

            create_patcher = mock.patch("kozmodb.integrations.handlers.lightwood_handler.Handler.create")
            self.mock_create = create_patcher.__enter__()

        ctx.set_default()
        sql_session = SessionController()
        sql_session.database = "kozmodb"
        ...
        config_patch = mock.patch("kozmodb.utilities.cache.FileCache.get")
    ...
    def save_file(self, name, df):
        file_path = tempfile.mktemp(prefix="kozmodb_file_")
    ...
    def set_handler(self, mock_handler, name, tables, engine="postgres"):
        ...
        from kozmodb.integrations.libs.response import RESPONSE_TYPE
        from kozmodb.integrations.libs.response import HandlerResponse as Response
...
class BaseExecutorDummyML(BaseExecutorTest):
    ...
    def run_sql(self, sql, throw_error=True, database='kozmodb'):
