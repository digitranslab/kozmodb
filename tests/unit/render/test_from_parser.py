...
from kozmodb_sql_parser.ast import Select, Constant, WindowFunction, Function
...
def parse_sql2(sql, dialect="kozmodb"):
    # convert to ast
...
class TestFromParser:
    def test_from_parser(self, pytestconfig):
        try:
            ...
            print(
                "Unable to import render's tests. Make sure they are in the kozmodb folder. It can be done by:"
                "- git clone https://github.com/digitranslab/kozmodb_sql_parser.git parser_tests"
                f"\nError: {e}"
            )
