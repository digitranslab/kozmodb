...
Integrate Kozmodb’s memory capabilities with OpenAI’s Inbuilt Tools to create AI agents with persistent memory.
...
```bash
npm install kozmodb openai zod
```

## Environment Setup

Save your Kozmodb and OpenAI API keys in a `.env` file:
...
Get your Kozmodb API key from the [Kozmodb Dashboard](https://app.kozmodb.ai/dashboard/api-keys).
...
```javascript
const relevantMemories = await kozmodbClient.search(userInput, kozmodbConfig);
```
