---
title: Kozmodb as an Agentic Tool
---
...
```bash
pip install kozmodb pydantic openai-agents
```
...
Store your Kozmodb API key as an environment variable:
...
```python
import os
os.environ["KOZMODB_API_KEY"] = "your_kozmodb_api_key"
```
...
try:
    ...
    raise ImportError("kozmodb is not installed. Please install it using 'pip install kozmodb'.")
from agents import (
    Agent,
    ItemHelpers,
    MessageOutputItem,
...
```python
class KozmodbContext(BaseModel):
    user_id: str | None = None
```

### 3. Initialize the Kozmodb Client

```python
client = AsyncMemoryClient(api_key=os.getenv("KOZMODB_API_KEY"))
```
...
async def add_to_memory(
    context: RunContextWrapper[KozmodbContext],
    content: str,
) -> str:
    """
    Add a message to Kozmodb
    Args:
...
async def search_memory(
    ...
    """
    Search for memories in Kozmodb
    Args:
...
async def get_all_memory(
    ...
    """Retrieve all memories from Kozmodb"""
    user_id = context.context.user_id or "default_user"
...
```python
memory_agent = Agent[KozmodbContext](
...
```python
async def main():
    current_agent: Agent[KozmodbContext] = memory_agent
    input_items: list[TResponseInputItem] = []
    context = KozmodbContext()
    while True:
...
```python
context = KozmodbContext(user_id="user123")
```
...
- [Kozmodb Dashboard](https://app.kozmodb.ai/dashboard)
- [API Reference](https://docs.kozmodb.ai/api-reference)
