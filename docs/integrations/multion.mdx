...
Build a personal browser agent that remembers user preferences and automates web tasks. It integrates Kozmodb for memory management with MultiOn for executing browser actions, enabling personalized and efficient web interactions.
...
2. A travel agent that provides personalized travel information based on user preferences. Refer to the [notebook](https://github.com/MULTI-ON/cookbook/blob/main/personalized-travel-agent/kozmodb_travel_agent.ipynb) for detailed code.
...
```bash
pip install kozmodb multion openai
```
...
KOZMODB_API_KEY = 'your-kozmodb-key'  # Replace with your actual Kozmodb API key
USER_ID = "your-user-id"

# Set up OpenAI API key
os.environ['OPENAI_API_KEY'] = OPENAI_API_KEY
os.environ['KOZMODB_API_KEY'] = KOZMODB_API_KEY

# Initialize Kozmodb and MultiOn
...
### Add memories to Kozmodb

Define user data and add it to Kozmodb.

```python
USER_DATA = """
About me
- I'm Mohamed Ben Chaliah, Co-founder and CTO at Kozmodb, interested in AI and ML Infrastructure.
...
Define search command and retrieve relevant memories from Kozmodb.
...
Add conversation to Kozmodb and create a function to get travel information based on user's question and optionally their preferences from memory.
...
By integrating Kozmodb with MultiOn, you've created personalized browser agents that remember user preferences and automate web tasks. The first example demonstrates a research-focused agent, while the second example shows a travel agent capable of providing personalized recommendations.
...
- For more details and advanced usage, refer to the full [cookbooks here](https://github.com/digitranslab/kozmodb/blob/main/cookbooks).
- Feel free to visit our [Github](https://github.com/digitranslab/kozmodb) or [Kozmodb Platform](https://app.kozmodb.ai/).
- For any questions or assistance, please reach out to `mobenchio` on [Discord](https://kozmodb.dev/DiD).
