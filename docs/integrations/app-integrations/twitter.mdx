...
In this section, we present how to connect Twitter accounts to KozmoDB.
...
The Twitter handler enables you to fetch tweets and create replies utilizing AI models wthin KozmoDB. Furthermore, you can automate the process of fetching tweets, preparing replies, and sending replies to Twitter.
...
2. To connect Twitter to KozmoDB, install the required dependencies following [this instruction](/setup/self-hosted/docker#install-dependencies).
3. Install or ensure access to Twitter.

## Connection

To connect a Twitter account to KozmoDB, you need a Twitter developer account.
...
<Accordion title="How to Apply for a Twitter Developer Account" icon="twitter" iconType="thin">
  ...
  *I have a blog and want to educate users how to use the Twitter API with KozmoDB.*

  *I will read tweets that mention me and use them with KozmoDB machine learning to generate responses. I plan to post tweets 2-3 times a day and keep using Twitter like I normally would.*
...
Once you have all the tokens and keys, here is how to connect your Twitter account to KozmoDB:
...
Here is how to search tweets containing `kozmodb` keyword:

```sql
SELECT id, created_at, author_username, text 
FROM my_twitter.tweets 
WHERE query = '(kozmodb OR #kozmodb) -is:retweet -is:reply'
...
<Tip>
To learn more about native queries in KozmoDB, visit our docs [here](/sql/native-queries).
</Tip>
...
<Info>
For more information about available actions and development plans, visit [this page](https://github.com/digitranslab/kozmodb/blob/main/kozmodb/integrations/handlers/twitter_handler/README.md).
</Info>
