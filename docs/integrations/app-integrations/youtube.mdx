...
In this section, we present how to connect YouTube to KozmoDB.
...
Before proceeding, ensure the following prerequisites are met:
 ...
 1. Install KozmoDB on your system or obtain access to cloud options.
 2. To use YouTube with KozmoDB, install the required dependencies following [this instruction](/setup/self-hosted/docker#install-dependencies).

## Connection

There are two ways you can connect YouTube to KozmoDB:

1. Limited permissions: This option provides KozmoDB with read-only access to YouTube, including viewing comments data.
2. Elevated permissions: This option provides KozmoDB with full access to YouTube, including viewing comments data and posting replies to comments.

### Option 1: Limited permissions

Establish a connection to YouTube from KozmoDB by executing the below SQL command and following the Google authorization link provided as output:
...
<Tip>
Alternatively, you can connect YouTube to KozmoDB via the form.
...
<Accordion title="How to Generate the Credentials File" icon="youtube" iconType="thin">
  ...
  3. Inside this project, go to APIs & Services:
      ...
      - Go to Credentials:
        ...
        - Choose type as `Web application` and provide a name. Under Authorized redirect URIs, enter URL where KozmoDB has been deployed followed by `/verify-auth`. For example, if you are running KozmoDB locally (on `https://localhost:47334`), enter `https://localhost:47334/verify-auth`.
        - Click on CREATE.
        - Download the JSON file that is required to connect YouTube to KozmoDB.
</Accordion>
...
```sql
INSERT INTO kozmodb_youtube.comments (comment_id, reply)
