...
In this section, we present how to connect Gmail accounts to KozmoDB.

[Gmail](https://gmail.com/) is a widely used and popular email service developed by Google.

By connecting your Gmail account to KozmoDB, you can utilize various AI models available within KozmoDB to summarize emails, detect spam, or even automate email replies.

<Warning>
Please note that currently you can connect your Gmail account to local KozmoDB installation by providing a path to the credentials file stored locally.

If you want to connect your Gmail account to KozmoDB Cloud, you can upload the credentials file, for instance, to your S3 bucket and provide a link to it as a parameter.
</Warning>
...
2. To connect Gmail to KozmoDB, install the required dependencies following [this instruction](/setup/self-hosted/docker#install-dependencies).
...
In order to make use of this handler and connect the Google Calendar app to KozmoDB, the following syntax can be used:
...
3. Create credentials for the Gmail API:
    ...
    3.4 Add the following KozmoDB URL to `Authorized redirect URIs`:
        - For local installation, add `http://localhost/verify-auth`
        - For Cloud, add `http://cloud.kozmodb.com/verify-auth`.
...
This creates a database called kozmodb_gmail. This database ships with a table called emails that we can use to search for emails as well as to write emails.
...
<Info>
For more information about available actions and development plans, visit [this page](https://github.com/digitranslab/kozmodb/blob/main/kozmodb/integrations/handlers/gmail_handler/README.md).
</Info>
