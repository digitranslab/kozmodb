...
The [**Kozmodb AI SDK Provider**](https://www.npmjs.com/package/@kozmodb/vercel-ai-provider) is a library developed by **Kozmodb** to integrate with the Vercel AI SDK. This library brings enhanced AI interaction capabilities to your applications by introducing persistent memory functionality.

<Note type="info">
  🎉 Exciting news! Kozmodb AI SDK now supports <strong>Graph Memory</strong>.
</Note>
...
1. Get your **Kozmodb API Key** from the [Kozmodb Dashboard](https://app.kozmodb.ai/dashboard/api-keys).

2. Initialize the Kozmodb Client in your application:
    ...
    const kozmodb = createKozmodb({
      ...
      // Optional Kozmodb Global Config
      kozmodbConfig: {
        user_id: "kozmodb-user-id",
        org_id: "kozmodb-org-id",
        project_id: "kozmodb-project-id",
      },
    });
    ```

    > **Note**: The `openai` provider is set as default. Consider using `KOZMODB_API_KEY` and `OPENAI_API_KEY` as environment variables for security.

    > **Note**: The `kozmodbConfig` is optional. It is used to set the global config for the Kozmodb Client (eg. `user_id`, `agent_id`, `app_id`, `run_id`, `org_id`, `project_id` etc).
...
### Standalone Features:
    ...
    ```
     > For standalone features, such as `addMemories`, `retrieveMemories`, and `getMemories`, you must either set `KOZMODB_API_KEY` as an environment variable or pass it directly in the function call.
...
### 1. Basic Text Generation with Memory Context
    ...
    const { text } = await generateText({
      model: kozmodb("gpt-4-turbo", { user_id: "borat" }),
...
### 4. Generate Responses with Tools Call
    ...
    const result = await generateText({
      model: kozmodb('claude-3-5-sonnet-20240620'),
      tools: {
        weather: tool({
...
Kozmodb AI SDK now supports Graph Memory. You can enable it by setting `enable_graph` to `true` in the `kozmodbConfig` object.

```typescript
const kozmodb = createKozmodb({
  kozmodbConfig: { enable_graph: true },
});
```
...
```typescript
const memories = await getMemories(prompt, { user_id: "borat", kozmodbApiKey: "m0-xxx", enable_graph: true });
```
...
- `createKozmodb()`: Initializes a new Kozmodb provider instance.
...
Kozmodb’s Vercel AI SDK enables the creation of intelligent, context-aware applications with persistent memory and seamless integration.
