...
2. Initialize the Kozmodb Client in your application:
    ...
    > **Note**: The `mem0Config` is optional. It is used to set the global config for the Kozmodb Client (eg. `user_id`, `agent_id`, `app_id`, `run_id`, `org_id`, `project_id` etc).
...
Kozmodb AI SDK now supports Graph Memory. You can enable it by setting `enable_graph` to `true` in the `mem0Config` object.
