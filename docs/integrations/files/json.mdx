---
title: Upload JSON files to KozmoDB
sidebarTitle: JSON
---

You can upload JSON files of any size to KozmoDB that runs locally via [Docker](/setup/self-hosted/docker) or [pip](/contribute/install).
...
Here is the sample format of a JSON file that can be uploaded to KozmoDB:
...
KozmoDB converts it into a table where each row stores the high-level object.
