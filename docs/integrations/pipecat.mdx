---
title: 'Pipecat'
description: 'Integrate Kozmodb with Pipecat for conversational memory in AI agents'
---
...
Kozmodb seamlessly integrates with [Pipecat](https://pipecat.ai), providing long-term memory capabilities for conversational AI agents. This integration allows your Pipecat-powered applications to remember past conversations and provide personalized responses based on user history.

## Installation

To use Kozmodb with Pipecat, install the required dependencies:

```bash
pip install "pipecat-ai[kozmodb]"
```

You'll also need to set up your Kozmodb API key as an environment variable:

```bash
export KOZMODB_API_KEY=your_kozmodb_api_key
```

You can obtain a Kozmodb API key by signing up at [kozmodb.ai](https://kozmodb.ai).

## Configuration

Kozmodb integration is provided through the `KozmodbMemoryService` class in Pipecat. Here's how to configure it:

```python
from pipecat.services.kozmodb import KozmodbMemoryService

memory = KozmodbMemoryService(
    api_key=os.getenv("KOZMODB_API_KEY"),  # Your Kozmodb API key
...
The `KozmodbMemoryService` should be positioned between your context aggregator and LLM service in the Pipecat pipeline:
...
pipeline = Pipeline([
    ...
    memory,             # Kozmodb Memory service enhances context here
...
Here's a complete example of a Pipecat voice agent with Kozmodb memory integration:
...
When integrated with Pipecat, Kozmodb provides two key functionalities:

### 1. Message Storage

All conversation messages are automatically stored in Kozmodb for future reference:
...
2. Relevant memories are retrieved from Kozmodb's database
...
memory = KozmodbMemoryService(
    ...
    params={
        ...
        "api_version": "v2",          # Kozmodb API version
    }
)
```
...
- [Kozmodb Pipecat Integration](https://docs.pipecat.ai/server/services/memory/kozmodb)
