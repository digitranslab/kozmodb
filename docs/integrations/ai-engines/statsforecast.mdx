---
title: Nixtla's StatsForecast Integration with KozmoDB
...
## How to bring StatsForecast Models to KozmoDB
...
Once the ML engine is created, we use the `CREATE MODEL` statement to create the StatsForecast model in KozmoDB.
...
Let's go through an example of how to use Nixtla's StatsForecast with KozmoDB to forecast monthly expenditures.

Please note that before using the StatsForecast engine, you should create it from the KozmoDB editor, or other clients through which you interact with KozmoDB, with the below command:
...
We use a table from our MySQL public demo database, so let’s start by connecting KozmoDB to it:
...
FROM kozmodb.quarterly_expenditure_forecaster as m
...
KozmoDB provides the `LATEST` keyword that marks the latest training data point. In the `WHERE` clause, we specify the `month > LATEST` condition to ensure the predictions are made for data after the latest training data point.
...
Before we start, visit our docs to learn how to connect [Mongo Compass](https://docs.kozmodb.com/connect/mongo-compass) and [Mongo Shell](https://docs.kozmodb.com/connect/mongo-shell) to KozmoDB.

We use a collection from our Mongo public demo database, so let’s start by connecting KozmoDB to it from Mongo Compass or Mongo Shell:
