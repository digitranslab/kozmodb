...
### Bringing the Ray Serve Model to KozmoDB
...
Now, you can fetch predictions using the standard KozmoDB syntax. Follow the
...
JOIN kozmodb.byom_ray_serve AS tb
...
training data. KozmoDB can send as much as you'd like - it has been
...
Now, we can create and train this custom model in KozmoDB.
...
predictions using the standard KozmoDB syntax. Follow the guide on the
[`SELECT`](/sql/api/select/) statement to learn more.

```sql
SELECT *
FROM kozmodb.byom_ray_serve_nlp
...
Check out the article on [How to bring your own machine learning model to databases](https://medium.com/kozmodb/how-to-bring-your-own-machine-learning-model-to-databases-47a188d6db00) by [Patricio Cerda Mardini](https://medium.com/@paxcema) to learn more.
</Tip>
