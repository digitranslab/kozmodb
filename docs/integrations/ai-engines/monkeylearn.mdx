...
MonkeyLearn is a No-code text analysis tool. KozmoDB allows you to use pre-built & custom MonkeyLearn models to use its features like classifying text according to user needs and fields of interest like business, reviews, comments, and customer feedback.

## How to bring MonkeyLearn Models to KozmoDB
...
Once the ML_ENGINE is created, we use the `CREATE MODEL` statement to bring MonkeyLearn models to KozmoDB.

For this example, you will make use of MonkeyLearn's pre-made model `E-commerce Support Ticket Classifier`.

```sql
CREATE MODEL kozmodb.ecommerce_ticket_classifier
PREDICT tag
USING
...
|----------------------------------------|----------------------------------------------------------|
| `ecommerce_ticket_classifier`          | The model name provided to the model created in KozmoDB. |
...
You can also create a model with a dataset. For this example, we will be using a dataset consisting of messages for E-commerce support tickets. The dataset will be [uploaded as a file](https://docs.kozmodb.com/sql/create/file)  onto the GUI.

Use the `CREATE MODEL` syntax:

```sql
CREATE MODEL kozmodb.ecommerce_ticket_classifier2
...
The KozmoDB model created with the MonkeyLearn model successfully predicted the tag of an E-commerce support ticket according to the text input.
