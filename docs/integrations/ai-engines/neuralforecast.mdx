---
title: Nixtla's NeuralForecast Integration with KozmoDB
...
## How to bring NeuralForecast Models to KozmoDB
...
Once the ML engine is created, we use the `CREATE MODEL` statement to create the NeuralForecast model in KozmoDB.
...
Let's go through an example of how to use Nixtla's NeuralForecast with KozmoDB to forecast monthly expenditures based on historical data.

Please note that before using the NeuralForecast engine, you should create it from the KozmoDB editor, or other clients through which you interact with KozmoDB, with the below command:
