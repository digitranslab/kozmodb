...
Build a personalized Customer Support AI Agent using LangGraph for conversation flow and Kozmodb for memory retention. This integration enables context-aware and efficient support experiences.

## Overview

In this guide, we'll create a Customer Support AI Agent that:
1. Uses LangGraph to manage conversation flow
2. Leverages Kozmodb to store and retrieve relevant information from past interactions
...
```bash
pip install langgraph langchain-openai kozmodb
```
...
# Initialize LangChain and Kozmodb
llm = ChatOpenAI(model="gpt-4", api_key=OPENAI_API_KEY)
kozmodb = MemoryClient(api_key=KOZMODB_API_KEY)
```
...
def chatbot(state: State):
    ...
    user_id = state["kozmodb_user_id"]

    # Retrieve relevant memories
    memories = kozmodb.search(messages[-1].content, user_id=user_id)
...
{context}""")
    ...
    # Store the interaction in Kozmodb
    kozmodb.add(f"User: {messages[-1].content}\nAssistant: {response.content}", user_id=user_id)
...
```python
def run_conversation(user_input: str, kozmodb_user_id: str):
    config = {"configurable": {"thread_id": kozmodb_user_id}}
    state = {"messages": [HumanMessage(content=user_input)], "kozmodb_user_id": kozmodb_user_id}
...
if __name__ == "__main__":
    ...
    kozmodb_user_id = "customer_123"  # You can generate or retrieve this based on your user management system
    while True:
        user_input = input("You: ")
        if user_input.lower() in ['quit', 'exit', 'bye']:
            print("Customer Support: Thank you for contacting us. Have a great day!")
            break
        run_conversation(user_input, kozmodb_user_id)
```
...
By integrating LangGraph with Kozmodb, you can build a personalized Customer Support AI Agent that can maintain context across interactions and provide personalized assistance.
