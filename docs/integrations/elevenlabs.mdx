...
Create voice-based conversational AI agents with memory capabilities by integrating ElevenLabs and Kozmodb. This integration enables persistent, context-aware voice interactions that remember past conversations.

## Overview

In this guide, we'll build a voice agent that:
1. Uses ElevenLabs Conversational AI for voice interaction
2. Leverages Kozmodb to store and retrieve memories from past conversations
...
```bash
pip install elevenlabs kozmodb python-dotenv
```

Configure your environment variables:

<Note>You'll need both an ElevenLabs API key and a Kozmodb API key to use this integration.</Note>
...
- `AsyncMemoryClient` from Kozmodb for memory operations
...
def main():
    ...
    KOZMODB_API_KEY = os.environ.get('KOZMODB_API_KEY')
    ...
    if not KOZMODB_API_KEY:
        sys.stderr.write("KOZMODB_API_KEY environment variable must be set\n")
        sys.exit(1)

    # Set up Kozmodb API key in the environment
...
- Sets the Kozmodb API key in the environment for the Kozmodb client to use

### 3. Client Initialization

Initialize both the ElevenLabs and Kozmodb clients:
...
```python
    ...
    async def retrieve_memories(parameters):
        ...
        results = await kozmodb_client.search(
            query=message,
            version="v2", 
            filters=filters
        )
...
- Stores the message in Kozmodb using the `add` method
...
When the agent identifies information worth remembering, it calls this function to store it in the Kozmodb database with the appropriate user ID.
...
4. The function stores the memory in Kozmodb linked to the user's ID
...
3. The function performs a semantic search in Kozmodb
...
By integrating ElevenLabs Conversational AI with Kozmodb, you can create voice agents that maintain context across conversations and provide personalized responses based on user history. This powerful combination enables:
