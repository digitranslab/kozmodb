...
Integrate [**Kozmodb**](https://github.com/digitranslab/kozmodb) with [Agno](https://github.com/agno-agi/agno), a Python framework for building autonomous agents. This integration enables Agno agents to access persistent memory across conversations, enhancing context retention and personalization.

## Overview

1. 🧠 Store and retrieve memories from Kozmodb within Agno agents
...
Before setting up Kozmodb with Agno, ensure you have:

1. Installed the required packages:
```bash
pip install agno-ai kozmodb
```

2. Valid API keys:
   - [Kozmodb API Key](https://app.kozmodb.ai/dashboard/api-keys)
   - OpenAI API Key (for the agent model)

## Integration Example

The following example demonstrates how to create an Agno agent with Kozmodb memory integration, including support for image processing:
