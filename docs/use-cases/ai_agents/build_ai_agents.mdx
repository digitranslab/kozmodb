---
title: Build an AI Agent with KozmoDB
sidebarTitle: Build an AI Agent
---

KozmoDB provides a custom syntax to build AI agents that comprises an AI model augmented with users' data access. AI agents can be connected to a chat interface, like Slack or MS Teams, to create chatbots.
...
This tutorial demonstrates how to build AI agents with KozmoDB using KozmoDB SQL editor. This can be also accomplished with [APIs](/rest/agents/create-agent) and [Python SDK](/sdks/python/agents).
...
<Steps>
  ...
  <Step title="Create skills">
    ...
    *Note that skills store data to be passed to an agent, so it is required to connect users' data to KozmoDB before creating skills.*
  </Step>
...
First, connect your database to KozmoDB. Here the sample database is used.
...
Note that there are two types of skills: text-to-SQL and knowledge bases. Learn more about [skills here](/kozmodb_sql/agents/agent#create-skills).
...
First connect a chat interface to KozmoDB. Here the Slack connection is made.
...
Follow the instructions on how to [connect Slack to KozmoDB](/integrations/app-integrations/slack#method-1-chatbot-responds-in-direct-messages-to-a-slack-app) for this use case.
...
USING
    database = 'kozmodb_slack', -- connect a chat interface with CREATE DATABASE
