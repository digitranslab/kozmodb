...
This tutorial presents how to chain OpenAI models within KozmoDB to analyze text sentiment and generate responses, which will be sent in the form of Slack notifications.

## Data setup

Connect your database to KozmoDB.
...
The alert system will send notification to Slack. Here is how to [connect Slack to KozmoDB](/integrations/app-integrations/slack#method-2-chatbot-responds-on-a-defined-slack-channel).
...
[Create a job](/kozmodb_sql/sql/create/jobs) to send notification every time a negative review is received.
...
SELECT * FROM kozmodb.jobs WHERE name = 'customer_reviews_notifications';

SELECT * FROM log.jobs_history WHERE project = 'kozmodb' AND name = 'customer_reviews_notifications';
```
...
[Create a job](/kozmodb_sql/sql/create/jobs) to send notification, including a sample response, every time a positive review is received.
...
```sql
SELECT * FROM kozmodb.jobs WHERE name = 'customer_reviews_and_responses_notifications';

SELECT * FROM log.jobs_history WHERE project = 'kozmodb' AND name = 'customer_reviews_and_responses_notifications';
```
