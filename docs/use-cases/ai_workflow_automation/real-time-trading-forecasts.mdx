...
KozmoDB enables you to automate AI workflows between any source of data and any AI/ML Model. The core building block of this automation is a job that allows anything in KozmoDB to be run either on a timer (e.g. every day) or based on a trigger (e.g. when a new row is added to the database).
...
KozmoDB can connect to any source of data - a database, warehouse, stream or app. Here, we’ll connect to Binance API to get a feed of real-time price information.
...
- `CREATE MODEL`: It is used to create, train, and deploy an ML model. By default, KozmoDB’s AutoML will automatically choose the best model for your data but this can be overridden (docs).
...
The following elements are unique to forecasting models in KozmoDB:
...
The time it takes to train the model depends on the amount of training data. In this case, it takes about 2 minutes on KozmoDB Cloud. Once the status reads complete, we’re ready to make some forecasts!
...
The next thing we can do is automate price alerts. Here we’ll choose Slack as our preferred place to receive the alerts but this could be any other system that KozmoDB integrates with.

## Connect Slack

Follow these instructions to set up your Slack app and generate a Slack bot token. Once you get the Slack bot token and integrate your Slack app into one of the Slack channels, you can connect it to KozmoDB.
...
By now you have connected Binance to KozmoDB, deployed and trained a time-series model, and set up the Slack connection. Let's create a job that will retrain your model periodically using the latest Binance data (which allows us to keep improving the accuracy and performance of the model) and send real-time forecasts of the BTC/USDT trading pair for the next 10 minutes as Slack notifications.
