...
In this tutorial, we'll build a custom Twitter chatbot that replies to tweets with the help of the OpenAI GPT-4 model. The workflow will be automated using Jobs - a KozmoDB feature that enables you to schedule execution of tasks.
...
## Connect Twitter to KozmoDB

Follow the [docs](/integrations/app-integrations/twitter) to connect your Twitter account to KozmoDB.
...
This job is executed every minute. It fetches all recently added tweets with the help of the [`LAST` keyword](/kozmodb_sql/sql/create/jobs#last). Then, it prepares and posts the replies.
...
WHERE project = 'kozmodb' AND name = 'twitter_chatbot';
```
