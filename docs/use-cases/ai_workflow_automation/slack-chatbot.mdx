---
title: Build a Slack Chatbot with KozmoDB and OpenAI
sidebarTitle: Slack Chatbot
---

The objective of this tutorial is to create an AI-powered personalized chatbot by utilizing the KozmoDB's Slack connector, and combining it with OpenAI's GPT-4 Model.
...
- [Create a Slack Account](https://slack.com/get-started#/createnew) and follow [this instruction](/integrations/app-integrations/slack) to connect Slack to KozmoDB.
- Go to your KozmoDB Editor

## Usage

This query will create a database called `kozmodb_slack` that comes with the `channels` table.

```sql
CREATE DATABASE kozmodb_slack
WITH
  ENGINE = 'slack',
  PARAMETERS = {
      "token": "xoxb-..."
...
```sql
INSERT INTO kozmodb_slack.messages (channel_id, text)
VALUES
    ("<channel-id>", "Hey KozmoDB, Thanks to you! Now I can respond to my Slack messages through SQL Queries. 🚀 "),
    ("<channel-id>", "It's never been that easy to build ML apps using KozmoDB!");
```

Whoops! Sent it by mistake? No worries! Use this to delete a specific message:

```sql
DELETE FROM kozmodb_slack.messages
...
_Generating a [Machine Learning model](https://docs.kozmodb.com/nlp/nlp-kozmodb-openai) with KozmoDB feels like taking a thrilling elevator ride in Burj Khalifa (You don't realize, that you made it)!_
...
SELECT
  text, response
FROM kozmodb.gpt_model
WHERE text = 'Hi, can you please explain me more about KozmoDB?';
```
...
```sql
CREATE MODEL kozmodb.whizfizz_model
PREDICT response
USING
engine = 'openai_engine',
max_tokens = 300,
...
FROM kozmodb_slack.messages as t
JOIN kozmodb.whizfizz_model as r
...
We will `CREATE JOB` to schedule periodical execution of SQL statements. The job will execute every hour and do the following:
    1. Check for new messages using the [`LAST` keyword](/kozmodb_sql/sql/create/jobs#last).
...
```sql
CREATE JOB kozmodb.gpt4_slack_job AS (
...
The `LAST` keyword is used to ensure the query fetches only the newly added messages. Learn more [here](/kozmodb_sql/sql/create/jobs#last).
...
SELECT * FROM kozmodb.jobs WHERE name = 'gpt4_slack_job';

SELECT * FROM log.jobs_history WHERE project = 'kozmodb' AND name = 'gpt4_slack_job';
```
...
```sql
CREATE TRIGGER slack_trigger ON kozmodb_slack.messages (
...
Check out [How to Generate Images using OpenAI with KozmoDB](/sql/tutorials/image-generator) to see another interesting use case of OpenAI integration.
</Tip>
