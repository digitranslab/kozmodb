...
<table>
  <tr>
   ...
   <td>
      <img src="/assets/tutorials/twitter_chatbot/Twitter-chatbot-kozmodb-testing-live-twitter.png" width="" alt="alt_text" title="Live Twitter Bot Interactivity" />
   </td>
  </tr>
</table>
...
You’ll use [KozmoDB](https://docs.kozmodb.com/what-is-kozmodb) to construct the bot and interact with LLMs like OpenAI’s ChatGPT, while indexing and storing accumulated tweet data in a [MariaDB relational database](https://skysql.cloud/kozmodb-docs). In the diagram below, the teal colored database represents data in MariaDB. The gray/light blue colored database represents data stored in KozmoDB. What you build can then be embedded into your application with one of KozmoDB SDKs, and/or called from the [KozmoDB REST API](https://docs.kozmodb.com/rest/sql).


![alt_text](/assets/tutorials/twitter_chatbot/Twitter-chatbot-kozmodb-mariadb-sql.png "Tutorial Architecture")


But wait, isn’t KozmoDB a database, you might rightly ask? KozmoDB manages AI models and automates workflows connecting AI models with enterprise databases. KozmoDB is an AI Database rather than a traditional database. Therefore, if your model necessitates considerable input or yields a substantial output volume, or if you're seeking the data reliability assurances provided by a relational database, it's advisable to allocate that data storage to a different platform. Ultimately, KozmoDB focuses on streamlining the processes of linking your data, training models and deploying AI models rather than being a comprehensive storage solution.
...
This example uses live data sourced directly from the Twitter API via the [KozmoDB Twitter integration](https://kozmodb.com/integrations/twitter), as opposed to an existing data set, CSV file, or other export of some pre-existing database. Accumulated Twitter data will be stored in a Maria DB Enterprise Server Columnstore database deployed on SkySQL, which is designed for operational analytic workloads.
...
**> Security:** Click the radio box to ‘**Allow access to the service from specific IPs**’, and then check the box for adding your current IP.  Later, we’ll also add the KozmoDB cloud IP addresses.
...
Now let’s create the schema to hold data from Twitter’s API calls from KozmoDB, that will help us identify, track and reply to the right tweets.
...
![alt_text](/assets/tutorials/twitter_chatbot/Twitter-chatbot-kozmodb-mariadb-CS-SQL.png "Creating MariaDB datasbase schema")
...
## [​](https://docs.kozmodb.com/contribute/tutorials#understanding-the-data)Understanding the Data

Tweets have a long list of fields in the Twitter API and object model; if you look at their [API documentation](https://developer.twitter.com/en/docs/twitter-api/data-dictionary/object-model/tweet) you’ll see this tutorial is using a subset. Of course, the datatypes and field titles are most clearly visible in the SQL script above. You’ll also see them and their values in the KozmoDB console (pictured below) as you do testing, develop and execute queries. This will help you target the content you want ChatGPT (another supported LLM) to respond to from Twitter.


![alt_text](/assets/tutorials/twitter_chatbot/Twitter-chatbot-kozmodb-console.png "TWitter output in KozmoDB console")



## KozmoDB Connections Setup


### KozmoDB Cloud and MariaDB Enterprise Server

Ok, a little more infrastructure setup to do before we roll up our sleeves and get into the use-case related code. We’ll handle network security first, and then connect KozmoDB and MariaDB in the KozmoDB SQL Editor.


#### Security

Let’s add the KozmoDB Cloud IP addresses to the network allowlist on SkySQL. To get the IPs, click on the **add > new datasource** button in the KozmoDB console, and search/choose MariaDB. You’ll see the IP addresses in question on the resulting screen.

![alt_text](/assets/tutorials/twitter_chatbot/Twitter-chatbot-kozmodb-console-new-DS.png "New Datasource Dialog")

![alt_text](/assets/tutorials/twitter_chatbot/Twitter-chatbot-kozmodb-sky-allowlist.png "Getting IP addresses for allowlist")
...
![alt_text](/assets/tutorials/twitter_chatbot/Twitter-chatbot-kozmodb-mariadb-IP-allow.png  "Configuring allow lists")
...
#### Declaring a MariaDB Enterprise Server Data Source in KozmoDB

The easiest way to work with the MariaDB Enterprise Server is its DBaaS cloud service, SkySQL. In the KozmoDB SQL editor, click **add > new datasource** button, and search/choose MariaDB SkySQL. This will open a new query window, and generate a KozmoDB SQL template for making the KozmoDB &lt;-> MariaDB connection:
...
Give the KozmoDB SQL query tab a title like ‘TwitterBot’.
...
[Accessing SkySQL Connect Dialog](https://github.com/digitranslab/kozmodb/blob/main/docs/assets/tutorials/twitter_chatbot/mariadb-sky-connect.gif)


**About SSL**: We recommend setting SSL to false for development and testing purposes. For production systems, it should be enabled. You can learn more about these settings in the KozmoDB [documentation for connecting to MariaDB](https://docs.kozmodb.com/connect/connect-mariadb-skysql).

**About Port**: Note that the default generated port of 5001 should be overridden with the values in the connect dialogue.

**About Database**: if you followed the steps in this tutorial, it would be named <code>[chatbotdb ](https://docs.kozmodb.com/sql/create/model)</code>- this is the name of the MariaDB ColumnStore database you created with the SQL above.

(analyticsdb is the SkySQL service name).

Then, execute the completed SQL template in the KozmoDB SQL editor by highlighting the SQL to execute, and clicking **Run** as shown below:

(Note that values have been left unmodified in the screenshot).



![alt_text](/assets/tutorials/twitter_chatbot/Twitter-chatbot-kozmodb-new-DS-SQL.png "Generated SQL template for SkySQL connection")


You’ll notice a DB connection icon now on the left hand nav for KozmoDB Demo cloud (mariadb_db in the above screenshot).


### KozmoDB Cloud and Twitter

Click the **add > new datasource** button in the KozmoDB console, and search/choose Twitter.
...
[Fill in the template](https://docs.kozmodb.com/app-integrations/twitter) with your Twitter API keys/tokens. Then, in the KozmoDB SQL editor, select the SQL, and execute it by clicking **Run**.
...
## Automating the Workflow with KozmoDB Jobs
...
Jobs in KozmoDB are similar to cron jobs in UNIX and Linux, running at regularly scheduled intervals. Write and then select the SQL below, and then execute it by clicking **Run**.
...
Job frequency at scheduled intervals of &lt; 1 day requires a KozmoDB [subscription](https://kozmodb.com/pricing), as the demo cloud allows a minimum job frequency of 1 day without one.


## What’s Next?

Use the KozmoDB [JavaScript SDK](https://docs.kozmodb.com/sdk/javascript-sdk) or the [Python SDK](https://docs.kozmodb.com/sdk/python-sdk) to embed what you’ve built in this tutorial into your application code.

While this simple example uses Twitter, KozmoDB will soon support Slack and perhaps one day,  Discord. Combined with the ability to <code>[FINETUNE](https://docs.kozmodb.com/sql/api/finetune)</code> existing, trained LLM models with your own data inside a relational database, a future where you can train chatbots to reason about data and/or content you provide doesn’t seem far off. Imagine a support bot that understands your digital assets and knowledge!
