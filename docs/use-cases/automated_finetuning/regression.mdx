...
```sql
CREATE MODEL kozmodb.adjust_home_rentals_model
FROM example_db
    (SELECT * 
...
FROM kozmodb.adjust_home_rentals_model
WHERE sqft = 1000
AND location = 'great'
...
```sql
FINETUNE kozmodb.adjust_home_rentals_model
FROM example_db
    (SELECT * 
    FROM demo_data.home_rentals 
    WHERE days_on_market >= 10);
...
+---------------------------+-----------+---------+--------+---------+----------+
| adjust_home_rentals_model | lightwood | kozmodb | false  | 1       | complete |
| adjust_home_rentals_model | lightwood | kozmodb | true   | 2       | complete |
+---------------------------+-----------+---------+--------+---------+----------+
```
