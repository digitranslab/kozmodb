...
```sql
CREATE MODEL kozmodb.adjust_customer_churn_model
FROM example_db
    (SELECT *
...
FROM kozmodb.adjust_customer_churn_model
...
```sql
FINETUNE kozmodb.adjust_customer_churn_model
FROM example_db
    (SELECT *
    FROM demo_data.customer_churn
    WHERE gender = 'Male');
...
+-----------------------------+-----------+---------+--------+---------+----------+
| adjust_customer_churn_model | lightwood | kozmodb | false  | 1       | complete |
| adjust_customer_churn_model | lightwood | kozmodb | true   | 2       | complete |
+-----------------------------+-----------+---------+--------+---------+----------+
```
