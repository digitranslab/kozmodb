...
This document presents various use cases of Hugging Face models from KozmoDB.
...
```sql
+---------------+-------+--------+--------+-------+-------------+---------------+------+-----------------+---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
|NAME           |PROJECT|STATUS  |ACCURACY|PREDICT|UPDATE_STATUS|KOZMODB_VERSION|ERROR |SELECT_DATA_QUERY|TRAINING_OPTIONS                                                                                                                                                                                               |
+---------------+-------+--------+--------+-------+-------------+---------------+------+-----------------+---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
|spam_classifier|kozmodb|complete|[NULL]  |PRED   |up_to_date   |22.10.2.1      |[NULL]|[NULL]           |{'target': 'PRED', 'using': {'engine': 'huggingface', 'task': 'text-classification', 'model_name': 'mrm8488/bert-tiny-finetuned-sms-spam-detection', 'input_column': 'text_spammy', 'labels': ['ham', 'spam']}}|
+---------------+-------+--------+--------+-------+-------------+---------------+------+-----------------+---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
```
...
```sql
CREATE MODEL kozmodb.sentiment_classifier
PREDICT sentiment
USING
  engine = 'huggingface',
  task = 'text-classification',
...
|NAME                |PROJECT|STATUS  |ACCURACY|PREDICT  |UPDATE_STATUS|KOZMODB_VERSION|ERROR |SELECT_DATA_QUERY|TRAINING_OPTIONS                                                                                                                                                                                                                  |
+--------------------+-------+--------+--------+---------+-------------+---------------+------+-----------------+----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
|sentiment_classifier|kozmodb|complete|[NULL]  |sentiment|up_to_date   |22.10.2.1      |[NULL]|[NULL]           |{'target': 'sentiment', 'using': {'engine': 'huggingface', 'task': 'text-classification', 'model_name': 'cardiffnlp/twitter-roberta-base-sentiment', 'input_column': 'text_short', 'labels': ['negative', 'neutral', 'positive']}}|
+--------------------+-------+--------+--------+---------+-------------+---------------+------+-----------------+----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
```
...
```sql
CREATE MODEL kozmodb.zero_shot_tcd
PREDICT topic
USING
  engine = 'huggingface',
...
|NAME         |PROJECT|STATUS  |ACCURACY|PREDICT |UPDATE_STATUS|KOZMODB_VERSION|ERROR |SELECT_DATA_QUERY|TRAINING_OPTIONS                                                                                                                                                                                                         |
+-------------+-------+--------+--------+--------+-------------+---------------+------+-----------------+-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
|zero_shot_tcd|kozmodb|complete|[NULL]  |topic   |up_to_date   |22.10.2.1      |[NULL]|[NULL]           |{'target': 'topic', 'using': {'engine': 'huggingface', 'task': 'zero-shot-classification', 'model_name': 'facebook/bart-large-mnli', 'input_column': 'text_short', 'candidate_labels': ['travel', 'cooking', 'dancing']}}|
+-------------+-------+--------+--------+--------+-------------+---------------+------+-----------------+-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
```
...
```sql
CREATE MODEL kozmodb.translator_en_fr
PREDICT translated
USING
  engine = 'huggingface',
  task = 'translation',
...
|NAME            |PROJECT|STATUS  |ACCURACY|PREDICT   |UPDATE_STATUS|KOZMODB_VERSION|ERROR |SELECT_DATA_QUERY|TRAINING_OPTIONS                                                                                                                                                                   |
+----------------+-------+--------+--------+----------+-------------+---------------+------+-----------------+-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
|translator_en_fr|kozmodb|complete|[NULL]  |translated|up_to_date   |22.10.2.1      |[NULL]|[NULL]           |{'target': 'translated', 'using': {'engine': 'huggingface', 'task': 'translation', 'model_name': 't5-base', 'input_column': 'text_short', 'lang_input': 'en', 'lang_output': 'fr'}}|
+----------------+-------+--------+--------+----------+-------------+---------------+------+-----------------+-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
```
...
```sql
CREATE MODEL kozmodb.summarizer_10_20
...
|NAME            |PROJECT|STATUS  |ACCURACY|PREDICT     |UPDATE_STATUS|KOZMODB_VERSION|ERROR |SELECT_DATA_QUERY|TRAINING_OPTIONS                                                                                                                                                                                                     |
+----------------+-------+--------+--------+------------+-------------+---------------+------+-----------------+---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
|summarizer_10_20|kozmodb|complete|[NULL]  |text_summary|up_to_date   |22.10.2.1      |[NULL]|[NULL]           |{'target': 'text_summary', 'using': {'engine': 'huggingface', 'task': 'summarization', 'model_name': 'sshleifer/distilbart-cnn-12-6', 'input_column': 'text_long', 'min_output_length': 10, 'max_output_length': 20}}|
+----------------+-------+--------+--------+------------+-------------+---------------+------+-----------------+---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
```
...
FROM example_db.demo_data.hf_test AS t
JOIN kozmodb.summarizer_10_20 AS h;
```
...
```sql
CREATE MODEL kozmodb.fill_mask
PREDICT text_filled
USING
  engine = 'huggingface',
  task = 'fill-mask',
...
|NAME            |PROJECT|STATUS  |ACCURACY|PREDICT     |UPDATE_STATUS|KOZMODB_VERSION|ERROR |SELECT_DATA_QUERY|TRAINING_OPTIONS                                                                                                    |
+----------------+-------+--------+--------+------------+-------------+---------------+------+-----------------+--------------------------------------------------------------------------------------------------------------------+
|fill_mask       |kozmodb|complete|[NULL]  |text_filled |up_to_date   |23.3.5.0       |[NULL]|[NULL]           |{'target': 'text_filled', 'using': {'task': 'fill-mask', 'model_name': 'bert-base-uncased', 'input_column': 'text'}}|
+----------------+-------+--------+--------+------------+-------------+---------------+------+-----------------+--------------------------------------------------------------------------------------------------------------------+
```
...
## Hugging Face + KozmoDB Models Library
...
```sql
CREATE MODEL kozmodb.hf_sentiment
PREDICT PRED
USING
 engine = 'huggingface',
 task = 'text-classification',
...
```sql
CREATE MODEL kozmodb.hf_sentiment_finance
PREDICT PRED
USING
 engine = 'huggingface',
 task = 'text-classification',
...
```sql
SELECT *
FROM kozmodb.hf_sentiment_finance
...
```sql
CREATE MODEL kozmodb.hf_emotions_6
PREDICT PRED
USING
 engine = 'huggingface',
 task = 'text-classification',
...
```sql
CREATE MODEL kozmodb.hf_toxicity
PREDICT PRED
USING
 engine = 'huggingface',
 task = 'text-classification',
...
```sql
CREATE MODEL kozmodb.hf_esg_26
PREDICT PRED
USING
 engine = 'huggingface',
 task = 'text-classification',
...
```sql
CREATE MODEL kozmodb.hf_crypto
PREDICT PRED
USING
 engine = 'huggingface',
 task = 'text-classification',
...
```sql
CREATE MODEL kozmodb.hf_us_party
PREDICT PRED
USING
 engine = 'huggingface',
 task = 'text-classification',
...
```sql
CREATE MODEL kozmodb.hf_question
PREDICT PRED
USING
 engine = 'huggingface',
 task = 'text-classification',
...
```sql
CREATE MODEL kozmodb.hf_industry
PREDICT PRED
USING
 engine = 'huggingface',
 task = 'text-classification',
...
```sql
CREATE MODEL kozmodb.hf_zs_bart
PREDICT PRED
USING
 engine = 'huggingface',
 task = 'zero-shot-classification',
...
```sql
CREATE MODEL kozmodb.hf_t5_en_fr
PREDICT PRED
USING
 engine = 'huggingface',
 task = 'translation',
...
```sql
CREATE MODEL kozmodb.hf_bart_sum_20
PREDICT PRED
USING
 engine = 'huggingface',
 task = 'summarization',
...
```sql
CREATE MODEL kozmodb.hf_peg_sum_20
PREDICT PRED
USING
 engine = 'huggingface',
 task = 'summarization',
