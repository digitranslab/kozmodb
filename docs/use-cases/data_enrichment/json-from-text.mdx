...
When you create an OpenAI model in KozmoDB, it uses the `gpt-3.5-turbo` model by default. But you can use the `gpt-4` model as well by passing it to the `model-name` parameter.
</Info>
...
<Tabs>
    ...
    ```
    ```sql
    CREATE MODEL kozmodb.nlp_model
    PREDICT json
    USING
