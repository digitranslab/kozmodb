---
title: Predict Text Sentiment with Hugging Face and KozmoDB
...
To create a model in KozmoDB, we use the `CREATE MODEL` statement. Next, we define the target column using the `PREDICT` clause. Finally, we specify all required parameters in the `USING` clause.
