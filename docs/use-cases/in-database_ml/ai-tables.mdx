...
```sql
CREATE MODEL kozmodb.debt_model
FROM income_table
PREDICT debt;
```
...
KozmoDB provides the [`CREATE MODEL`](/sql/create/model/)
...
approximated `debt` value, we query the `kozmodb.debt_model` model instead
of the `income_table` table.

```sql
SELECT income, debt
FROM kozmodb.debt_model
WHERE income = 90000;
```
