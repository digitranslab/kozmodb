---
title: Predict Home Rental Prices with KozmoDB
...
We will start by connecting a demo database to KozmoDB using the `CREATE DATABASE` statement.
...
This job will execute every 2 days only if there is new data available in the `home_rentals` table. Learn more about the [`LAST` keyword](/kozmodb_sql/sql/create/jobs#last) here.
