---
title: Predict Customer Churn with KozmoDB
...
<Tabs>
  ...
  <Tab title="Connecting as a file">
    You can download [the `CSV` data file here](https://github.com/digitranslab/kozmodb-examples/blob/master/classics/customer_churn/raw_data/WA_Fn-UseC_-Telco-Customer-Churn.csv) and upload it via [KozmoDB SQL Editor](/connect/kozmodb_editor/).

    Follow [this guide](/sql/create/file/) to find out how to upload a file to KozmoDB.
...
```sql
CREATE MODEL kozmodb.customer_churn_predictor
FROM files
  (SELECT * FROM churn)
PREDICT Churn;
```
...
KozmoDB predicted the probability of this customer churning with confidence of
...
JOIN kozmodb.customer_churn_predictor AS m
LIMIT 100;
```
...
- Install KozmoDB locally via [Docker](/setup/self-hosted/docker) or [Docker Desktop](/setup/self-hosted/docker-desktop).
- Engage with the KozmoDB community on
  [Slack](https://kozmodb.com/joincommunity) or
  [GitHub](https://github.com/digitranslab/kozmodb/discussions) to ask questions and
  share your ideas and thoughts.

If this tutorial was helpful, please give us a GitHub star
[here](https://github.com/digitranslab/kozmodb).
