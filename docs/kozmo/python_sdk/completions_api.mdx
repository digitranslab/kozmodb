...
Once the Mind is created, you can chat with the Mind using the OpenAI-compatible Completions API. Therefore to follow this code, if you haven’t already, install the OpenAI SDK by running `pip install openai`, and use the Kozmo API key and name of the Mind you created.
...
your_kozmo_api_key = <YOUR API KEY>
mind_name = <NAME YOU GAVE TO YOUR MIND>

# point the Openai SDK to the Kozmo Cloud
client = OpenAI(
    api_key=your_kozmo_api_key,
...
Alternatively, continue using the Kozmo SDK to access the OpenAI-compatible Completions API by calling the `completion` function on the `mind` object.
