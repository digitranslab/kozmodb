...
Once the Mind is created, you can chat with the Mind using the OpenAI-compatible Assistants API. Therefore to follow this code, if you haven’t already, install the OpenAI SDK by running `pip install openai`, and use the Kozmo API key and name of the Mind you created.
...
Action: kozmodb_sql_parser_tool
Action Input: 
```sql
SELECT COUNT(*) 
