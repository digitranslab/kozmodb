...
KozmoDB provides a range of options for persisting predictions and forecasts. Let's explore all possibilities to save the prediction results.
...
KozmoDB enables you to save predictions into a view or a table or download as a CSV file.
</Note>
...
Here, the `local_postgres` database is a PostgreSQL database connected to KozmoDB with a user that has the write access.
