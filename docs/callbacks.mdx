...
<Note>
Please note that this feature works only in MinsdDB Cloud and only for [KozmoDB Starter](/setup/cloud-starter) users.
</Note>
...
con.api.session.post(
    'https://cloud.kozmodb.com/cloud/callback/model_status',
    json={
...
Below is an example of using callbacks with the `home_rentals` model. Be sure that the callback `HOSTNAME` is accessible from KozmoDB Cloud.

```js
import express from 'express';
import axios from 'axios';
import * as KozmoDB from 'kozmodb-js-sdk';
const mdb = KozmoDB.default.default;
...
// Connect to KozmoDB Cloud
try {
  await mdb.connect({
    host: 'https://cloud.kozmodb.com',
    user: 'name@email.com',
    password: 'password',
    httpClient: customAxios,
  });
} catch (error) {
  console.error('KozmoDB Cloud Connection Error:', error);
  process.exit(1);
}
...
app.post('/model-status', async (req, res) => {
  ...
  if (trained_model_version === 1) {
    ...
    let model = await mdb.Models.getModel(MODEL_NAME, 'kozmodb');
    model.retrain();
...
// Add a callback to notify KozmoDB Cloud about the endpoint
try {
  await customAxios.post('https://cloud.kozmodb.com/cloud/callback/model_status', {
...
// Train a model
try {
  await mdb.Models.trainModel(MODEL_NAME, 'rental_price', 'kozmodb', {
