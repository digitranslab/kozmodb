...
Kozmodb provides a powerful set of APIs that allow you to integrate advanced memory management capabilities into your applications. Our APIs are designed to be intuitive, efficient, and scalable, enabling you to create, retrieve, update, and delete memories across various entities such as users, agents, apps, and runs.
...
- **Multi-org/project Support**: Specify organization and project when initializing the Kozmodb client to attribute API usage appropriately
...
Example with the kozmodb Python package:
...
To begin using the Kozmodb API, you'll need to:

1. Sign up for a [Kozmodb account](https://app.kozmodb.ai) and obtain your API key.
