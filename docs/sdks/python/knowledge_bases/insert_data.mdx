...
* Inserting data from data sources connected to KozmoDB:
...
* Inserting data from files uploaded to KozmoDB:
...
* **Case 1: Inserting data into the knowledge base without the `id_column` defined.**
    ...
    When users do not define the `id_column` during the creation of a knowledge base, KozmoDB generates the ID for each row using a hash of the content columns, as [explained here](/kozmodb_sql/knowledge_bases/create#id-column).
