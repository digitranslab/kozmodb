...
try {
  ...
  let homeRentalPriceModel = await KozmoDB.Models.trainModel(
    'home_rentals_model',
    'rental_price',
    'kozmodb',
  ...
  while (homeRentalPriceModel.status !== 'complete' && homeRentalPriceModel.status !== 'error') {
    homeRentalPriceModel = await KozmoDB.Models.getModel('home_rentals_model', 'kozmodb');
  }
...
try {
  ...
  // Creating and training a model
  let houseSalesForecastModel = await KozmoDB.Models.trainModel(
    'house_sales_model',
    'ma',
    'kozmodb',
  ...
  while (houseSalesForecastModel.status !== 'complete' && houseSalesForecastModel.status !== 'error') {
    houseSalesForecastModel = await KozmoDB.Models.getModel('house_sales_model', 'kozmodb');
  }
...
try {
  ...
  // Creating and training a model
  let openai_js = await KozmoDB.Models.trainModel(
    'openai_js',
    'sentiment',
    'kozmodb',
    trainingOptions);
  ...
  while (openai_js.status !== 'complete' && openai_js.status !== 'error') {
    openai_js = await KozmoDB.Models.getModel('openai_js', 'kozmodb');
  }
