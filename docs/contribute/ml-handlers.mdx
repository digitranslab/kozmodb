...
In this section, you'll find how to create new machine learning (ML) handlers within KozmoDB.

<Note>
    **Prerequisite**

    You should have the latest version of the KozmoDB repository installed locally. Follow [this guide](/contribute/install/) to learn how to install KozmoDB for development.
</Note>
...
You can create your own ML handler within KozmoDB by inheriting from the [BaseMLEngine](https://github.com/digitranslab/kozmodb/blob/3d9090acb0b8b3b0e2a96e2c93dad436f5ebef90/kozmodb/integrations/libs/base.py#L123) class.
...
<Tip>
    **Other Common Methods**
    ...
    Under the `kozmodb.integrations.libs.utils` library, contributors can find various methods that may be useful while implementing new handlers.

    Also, there is a wrapper class for the `BaseMLEngine` instances called [BaseMLEngineExec](https://github.com/digitranslab/kozmodb/blob/main/kozmodb/integrations/libs/ml_exec_base.py#L157). It is automatically deployed to take care of modifying the data responses into something that can be used alongside data handlers.
</Tip>

### Implementation

Here are the methods that must be implemented while inheriting from the [BaseMLEngine](https://github.com/digitranslab/kozmodb/blob/3d9090acb0b8b3b0e2a96e2c93dad436f5ebef90/kozmodb/integrations/libs/base.py#L123) class:

* [The create() method](https://github.com/digitranslab/kozmodb/blob/3d9090acb0b8b3b0e2a96e2c93dad436f5ebef90/kozmodb/integrations/libs/base.py#L151) saves a model inside the engine registry for later usage.
...
* [The predict() method](https://github.com/digitranslab/kozmodb/blob/3d9090acb0b8b3b0e2a96e2c93dad436f5ebef90/kozmodb/integrations/libs/base.py#L162) calls a model with an input dataframe and optionally, arguments to modify model's behaviour. This method returns a dataframe with the predicted values.
...
* [The update() method](https://github.com/digitranslab/kozmodb/blob/3d9090acb0b8b3b0e2a96e2c93dad436f5ebef90/kozmodb/integrations/libs/base.py#L171) is used to update, fine-tune, or adjust an existing model without resetting its internal state.
...
* [The describe() method](https://github.com/digitranslab/kozmodb/blob/3d9090acb0b8b3b0e2a96e2c93dad436f5ebef90/kozmodb/integrations/libs/base.py#L181) provides global model insights, such as framework-level parameters used in training.
...
* [The create_engine() method](https://github.com/digitranslab/kozmodb/blob/3d9090acb0b8b3b0e2a96e2c93dad436f5ebef90/kozmodb/integrations/libs/base.py#L189) is used to connect with the external sources, such as REST API.
...
## KozmoDB ML Ecosystem

KozmoDB has recently decoupled some modules out of its AutoML package in order to leverage them in integrations with other ML engines. The three modules are as follows:

1. The [type_infer](https://github.com/digitranslab/type_infer) module that implements automated type inference for any dataset.<br></br>
...
We recommend that new contributors use [type_infer](https://github.com/digitranslab/type_infer) and [dataprep_ml](https://github.com/digitranslab/dataprep_ml) modules when writing ML handlers to avoid reimplementing thin AutoML layers over and over again; it is advised to focus on mapping input data and user parameters to the underlying framework’s API.

For now, using the [kozmodb_evaluator](https://github.com/digitranslab/kozmodb_evaluator) module is not required, but will be in the short to medium term, so it’s important to be aware of it while writing a new integration.
...
<AccordionGroup>
        ...
        <Accordion title="Step 1: Set up and run KozmoDB locally">

        1. Set up KozmoDB using the [self-hosted pip](/setup/self-hosted/pip/source) installation method.
        ...
        2. Copy the simple tests from a relevant handler. For regular data, use the [Ludwig](https://github.com/digitranslab/kozmodb/tree/main/kozmodb/integrations/handlers/ludwig_handler) handler. And for time series data, use the [StatsForecast](https://github.com/digitranslab/kozmodb/tree/main/kozmodb/integrations/handlers/statsforecast_handler) handler.
        ...
        1. Create a new directory in `kozmodb/integrations/handlers/`. You must name the new directory `{HandlerName}_handler/`.
        
        2. Copy the `.py` files from the [OpenAI handler folder](https://github.com/digitranslab/kozmodb/tree/main/kozmodb/integrations/handlers/openai_handler), including: `__about__.py`, `__init__.py`, `openai_handler.py`, `creation_args.py`, and `model_using_args.py`.
        ...
        1. Launch the KozmoDB server locally with `python -m kozmodb`. Again, any issues will appear in the terminal output.
        ...
        1. You need to fork the KozmoDB repository. Follow [this guide](https://github.com/digitranslab/kozmodb/blob/main/CONTRIBUTING.md) to start a PR.

        2. If relevant, add your tests and new dependencies to the CI config. This is at `.github/workflows/kozmodb.yml`.
...
If you implement a time-series ML handler, create your unit tests following the structure of the [StatsForecast unit tests](https://github.com/digitranslab/kozmodb/blob/main/tests/unit/ml_handlers/test_statsforecast.py).

If you implement an NLP ML handler, create your unit tests following the structure of the [Hugging Face unit tests](https://github.com/digitranslab/kozmodb/blob/main/tests/unit/ml_handlers/test_huggingface.py).
</Tip>


## Check out our Machine Learning Handlers!

To see some ML handlers that are currently in use, we encourage you to check out the following ML handlers inside the KozmoDB repository:

* [Lightwood](https://github.com/digitranslab/kozmodb/tree/main/kozmodb/integrations/handlers/lightwood_handler)
* [HuggingFace](https://github.com/digitranslab/kozmodb/tree/main/kozmodb/integrations/handlers/huggingface_handler)
* [Ludwig](https://github.com/digitranslab/kozmodb/tree/main/kozmodb/integrations/handlers/ludwig_handler)
* [OpenAI](https://github.com/digitranslab/kozmodb/blob/main/kozmodb/integrations/handlers/openai_handler)

And here are [all the handlers available in the KozmoDB repository](https://github.com/digitranslab/kozmodb/tree/main/kozmodb/integrations/handlers).
