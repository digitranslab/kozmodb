...
* `api_key`
It is an optional parameter (applicable to selected providers), which stores the API key to access the model. Users can provide it either in this `api_key` parameter, or using [environment variables](/kozmodb_sql/functions/from_env).
...
**Option 2.** Define the default model in the [KozmoDB configuration file](/setup/custom-config).
...
This parameter stores data connected to the agent, including knowledge bases and data sources connected to KozmoDB.

The following parameters store the list of connected data.

* `knowledge_bases` stores the list of [knowledge bases](/kozmodb_sql/knowledge_bases/overview) to be used by the agent.

* `tables` stores the list of tables from data sources connected to KozmoDB.
...
<Tip>
See detailed descriptions of parameters in the [`CREATE AGENT` section](/kozmodb_sql/agents/agent_syntax#create-agent-syntax).
</Tip>
...
USING
    data = {
         "knowledge_bases": ["kozmodb.sales_kb"],
