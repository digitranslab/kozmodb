---
title: MariaDB SkySQL Setup Guide with KozmoDB
sidebarTitle: MariaDB SkySQL
---

Find more information on MariaDB Sky SQL [here](https://cloud.MariaDB.com/)

## 1. Select your service for KozmoDB

If you haven't already, identify the service to be enabled with KozmoDB and make
sure it is running. Otherwise, skip to step 2.

## 2. Add KozmoDB to your service Allowlist

Access to MariaDB SkySQL services is [restricted on a per-service basis](https://mariadb.com/products/skysql/docs/security/firewalls/ip-allowlist-services/). Add the following IP addresses to allow KozmoDB to connect to your MariaDB service, do this by clicking on the cog icon and navigating to Security Access. In the dialog, input as prompted – one by one – the following IPs:
...
## 5. Link KozmoDB to your MariaDB SkySQL Service

To print the query template, go to KozmoDB Editor and add a new data source from the Connect tab, choose MariaDB SkySQL from the list. Fill in the values and run a query to complete the setup.
...
<CodeGroup>
    ...
    WITH ENGINE = 'MariaDB',                      --- name of the KozmoDB handler
    PARAMETERS = {
    ...
    WITH ENGINE = 'MariaDB',
    PARAMETERS = {
      "host": "kozmodbtest.mdb0002956.db1.skysql.net",
      "port": 5001,
      "database": "kozmodb_data",
      "user": "DB00007539",
      "password": "password",
      --- here, the SSL certificate is required
      "ssl-ca": {
        "url": "https://kozmodb-web-builds.s3.amazonaws.com/aws_skysql_chain.pem"
      }
    };
    ```
