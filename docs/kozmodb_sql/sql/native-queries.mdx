...
The underlying database engine of KozmoDB is MySQL. However, you can run queries native to your database engine within KozmoDB.

## Connect your Database to KozmoDB

To run queries native to your database, you must first connect your database to KozmoDB using the `CREATE DATABASE` statement.
...
The first line (`SELECT * FROM example_db`) informs KozmoDB that we select from a PostgreSQL database. After that, we nest a PostgreSQL code within brackets.
