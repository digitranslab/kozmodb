...
It is advised to `RETRAIN` the predictor whenever the `update_status` column value from the `kozmodb.models` table is set to `available`.

Here is when the `update_status` column value is set to `available`:

- When the new version of KozmoDB is available that causes the predictor to become obsolete.
- When the new data is available in the table that was used to train the predictor.

To find out whether you need to retrain your model, query the `kozmodb.models` table and look for the `update_status` column.
