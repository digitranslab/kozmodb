...
It can be useful in cases when integration engine doesn't support some functions, for example, grouping, as shown below. In this case, all data from raw select are passed to KozmoDB and then subselect performs operations on them inside KozmoDB.
...
It is possible to use `UNION` and `UNION ALL` operators. It this case, every subselect from union will be fetched and merged to one result-set on KozmoDB side.
