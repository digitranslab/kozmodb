...
<Note>
Note that the `integration_name` connection must be created with the [`CREATE DATABASE`](/kozmodb_sql/sql/create/database) statement and the user with write access.
</Note>
...
CREATE OR REPLACE TABLE int1.tbl1 (
    ...
    JOIN kozmodb.predictor_name AS tb
