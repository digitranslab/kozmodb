...
Upon execution, it registers `my_kb` and associates the specified models and storage. `my_kb` is a unique identifier of the knowledge base within KozmoDB.
...
<Tip>
Users can use the variables and the [`from_env()` function](/kozmodb_sql/functions/from_env) to pass parameters when creating knowledge bases.
</Tip>

<Note>
As KozmoDB stores objects, such as models or knowledge bases, inside [projects](/kozmodb_sql/sql/create/project), you can create a knowledge base inside a custom project.
...
<Note>
Users can define the default embedding and reranking models from OpenAI in Settings of the KozmoDB GUI.
...
Learn more about the [OpenAI integration with KozmoDB here](/integrations/ai-engines/openai).
...
<Note>
Users can define the default embedding and reranking models from Azure OpenAI in Settings of the KozmoDB GUI.
</Note>
...
<Note>
Users can define the default embedding and reranking models from Google in Settings of the KozmoDB GUI.
</Note>
...
Learn more about the [Google Gemini integration with KozmoDB here](/integrations/ai-engines/google_gemini).
...
**Option 2.** Define the default embedding model in the [KozmoDB configuration file](/setup/custom-config).

<Tip>
You can define the default models in the Settings of the KozmoDB Editor GUI.
</Tip>
...
**Option 2.** Define the default reranking model in the [KozmoDB configuration file](/setup/custom-config).
...
* `provider`
It is a required parameter. It defines the model provider as listed in [supported LLMs](/kozmodb_sql/knowledge_bases/create#supported-llms).
...
If the `storage` parameter is not provided, the system creates the default ChromaDB vector database called `<kb_name>_chromadb` with the default table called `default_collection` that stores the embedded data. This default ChromaDB vector database is stored in KozmoDB's storage.

In order to provide the storage vector database, it is required to connect it to KozmoDB beforehand.
...
+---------+---------+--------+----------------------------------------+----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+-------------------+--------------------+----------------+-------+----------+
| my_kb   | kozmodb | [NULL] | my_kb_chromadb.default_collection      | {"embedding_model": {"provider": "openai", "model_name": "text-embedding-ada-002", "api_key": "sk-xxx"}, "reranking_model": {"provider": "openai", "model_name": "gpt-4o", "api_key": "sk-xxx"}, "default_vector_storage": "my_kb_chromadb"} | [NULL]            | [NULL]             | [NULL]         | [NULL]| [NULL]   |
+---------+---------+--------+----------------------------------------+----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+-------------------+--------------------+----------------+-------+----------+
```
