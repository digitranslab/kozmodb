...
KozmoDB provides the `LLM()` function that lets users incorporate the LLM-generated output directly into the data queries.

## Prerequisites

The `LLM()` function requires a large language model, which can be defined in the following ways:

- By setting the `default_llm` parameter in the [KozmoDB configuration file](/setup/custom-config#default-llm).
- By saving the default model in the KozmoDB Editor under Settings.
