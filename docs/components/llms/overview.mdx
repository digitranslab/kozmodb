...
Kozmodb includes built-in support for various popular large language models. Memory can utilize the LLM provided by the user, ensuring efficient use for specific needs.
...
Kozmodb supports two types of OpenAI LLM formats, each with its own strengths and use cases:
