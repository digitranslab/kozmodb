---
title: Feature Engineering in KozmoDB
...
If you want to follow the examples, install KozmoDB locally via [Docker](/setup/self-hosted/docker) or [Docker Desktop](/setup/self-hosted/docker-desktop).

## Prerequisites

The base table is available in the `example_db` integration in the KozmoDB Editor. In order to be able to use it, you must first create a database like this:
...
<Tabs>
  ...
  <Tab title="Using the Base Table + 2 More Columns">
    ...
    ```sql
    SELECT *
    FROM kozmodb.used_car_price_plus_2_columns
    LIMIT 5;
    ```
  ...
  <Tab title="Using the Base Table + 4 More Columns">
    ...
    ```sql
    SELECT *
    FROM kozmodb.used_car_price_plus_another_2_columns
    LIMIT 5;
    ```
...
<Tabs>
  <Tab title="Using the Base Table">
    ...
    ```sql
    CREATE MODEL kozmodb.price_predictor
    FROM example_db
  ...
  <Tab title="Using the Base Table + 2 More Columns">
    ...
    CREATE MODEL kozmodb.price_predictor_plus_2_columns
    FROM kozmodb
  ...
  <Tab title="Using the Base Table + 4 More Columns">
    ...
    ```sql
    CREATE MODEL kozmodb.price_predictor_plus_another_2_columns
    FROM kozmodb
...
<Tabs>
  <Tab title="Using the Base Table">
    ...
    ```sql
    +---------------+--------+--------+---------+-------------+---------------+------+--------------------------------------+----------------+
    |name           |status  |accuracy|predict  |update_status|kozmodb_version|error |select_data_query                     |training_options|
    +---------------+--------+--------+---------+-------------+---------------+------+--------------------------------------+----------------+
...
<Note>
    **Nested `SELECT` Statements** Please note that we use the nested `SELECT` statement in order to trigger native query at the KozmoDB Cloud Editor. Here, the `example_db` database is a PostgreSQL database, so we trigger PostgreSQL-native syntax.
</Note>
...
```sql
CREATE MODEL kozmodb.price_predictor_car_sales
FROM example_db
...
```sql
CREATE MODEL kozmodb.price_predictor_car_sales_info
FROM kozmodb
