...
* `kozmodb/kozmodb:latest` (or `kozmodb/kozmodb`)
...
-   `kozmodb/kozmodb` is a Docker image provided by KozmoDB. You can choose a different one from the list above.
...
```bash
docker run --name kozmodb_container -e KOZMODB_APIS=http -d -p 47334:47334 kozmodb/kozmodb
```
</Tip>
...
```bash
mkdir mdb_data
docker run --name kozmodb_container -e KOZMODB_APIS=http -p 47334:47334 -v $(pwd)/mdb_data:/root/mdb_storage kozmodb/kozmodb
```
...
```bash
docker run --name kozmodb_container -e MKL_SERVICE_FORCE_INTEL=1 -e KOZMODB_APIS=http -p 47334:47334 kozmodb/kozmodb
```
</Warning>
...
```bash
docker run --name kozmodb_container -e KOZMODB_USERNAME='admin' -e KOZMODB_PASSWORD='password' -e KOZMODB_APIS=http -p 47334:47334 kozmodb/kozmodb
```
</Tip>
...
```bash
docker run --name kozmodb_container -e KOZMODB_APIS=http -d -p 47334:47334 -v $(pwd)/mdb_config.json:/root/kozmodb_config.json kozmodb/kozmodb
```
