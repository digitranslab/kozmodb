...
Install dependencies listed in `requirements/requirements-opentelemetry.txt`, or install `[opentelemetry]` extra installing kozmodb:

```bash
pip install kozmodb[opentelemetry]
```
...
### KozmoDB Service Configuration

```yaml
environment:
  OTEL_EXPORTER_TYPE: "otlp"
  OTEL_OTLP_ENDPOINT: "http://otel-collector:4317"
  OTEL_SERVICE_NAME: "kozmodb"
  OTEL_SERVICE_INSTANCE_ID: "kozmodb-instance"
