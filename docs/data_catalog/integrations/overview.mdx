...
To enable the Data Catalog feature in KozmoDB, update your `config.json` file by setting the `data_catalog` flag to `true`:
...
Follow this doc page to learn how to [start KozmoDB with custom configuration](/setup/custom-config).
...
When you create an [agent](/kozmodb_sql/agents/agent) in KozmoDB that connects to one of the supported integrations, the Data Catalog automatically:
