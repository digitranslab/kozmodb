---
title: Types of Models within KozmoDB
sidebarTitle: Model Types
---

In this section, we go through the types of ML and AI models available in KozmoDB. These include regression models, classification models, time series models, and large language models (LLMs).
...
In KozmoDB, we use the [CREATE MODEL](/sql/create/model/) statement to create, train, and validate a model.
...
<Note>
Please note that KozmoDB calculates the model's accuracy by default while running the `CREATE MODEL` statement. However, it is not guaranteed that all ML engines do this.
...
FROM kozmodb.customer_churn_predictor
...
```sql
CREATE MODEL kozmodb.house_sales_predictor
FROM files
...
FROM kozmodb.house_sales_predictor AS m
...
KozmoDB uses the Lightwood ML engine by default. This section takes a closer look at how this package automatically chooses what type of model to use.

Models in Lightwood follow an encoder-mixer-decoder pattern, where refined, or encoded, representations of all features are mixed to produce target predictions. Here are [the mixers used by Lightwood](https://kozmodb.github.io/lightwood/mixer.html).
...
To give you some details on how KozmoDB creates a model using different mixers, here is [the full code](https://github.com/digitranslab/lightwood/blob/9a48d5523d78111c77024e1983d281f37a9782ea/lightwood/api/json_ai.py).

And here comes the breakdown:
- This [piece of code](https://github.com/digitranslab/lightwood/blob/9a48d5523d78111c77024e1983d281f37a9782ea/lightwood/api/json_ai.py#L238,L351) adds mixers to the `submodels` array depending on the model type and the data type of the target variable.
- And [here](https://github.com/digitranslab/lightwood/blob/9a48d5523d78111c77024e1983d281f37a9782ea/lightwood/api/json_ai.py#L353,L358), we choose the best of submodels to be used to create, train, and validate our AI models.

Let's dive into the details of how KozmoDB picks the mixers.

<Tabs>

<Tab title="Case 1">

Here is the [piece of code](https://github.com/digitranslab/lightwood/blob/9a48d5523d78111c77024e1983d281f37a9782ea/lightwood/api/json_ai.py#L239,L277) being analyzed.

[If we deal with a simple encoder/decoder pair performing the task](https://github.com/digitranslab/lightwood/blob/9a48d5523d78111c77024e1983d281f37a9782ea/lightwood/api/json_ai.py#L239,L250), we use the [Unit](https://kozmodb.github.io/lightwood/mixer.html#mixer.Unit) mixer that can be thought of as a bypass mixer.
...
[Otherwise](https://github.com/digitranslab/lightwood/blob/9a48d5523d78111c77024e1983d281f37a9782ea/lightwood/api/json_ai.py#L251,L277), we choose from a range of other mixers depending on the following conditions:

- [If it is not a time series case](https://github.com/digitranslab/lightwood/blob/9a48d5523d78111c77024e1983d281f37a9782ea/lightwood/api/json_ai.py#L252,L264), we use the [Neural](https://kozmodb.github.io/lightwood/mixer.html#mixer.Neural) mixer.
A good example is the [Customer Churn model](/sql/tutorials/customer-churn#training-a-predictor/).

- [If it is a time series case](https://github.com/digitranslab/lightwood/blob/9a48d5523d78111c77024e1983d281f37a9782ea/lightwood/api/json_ai.py#L265,L277), we use the [NeuralTs](https://kozmodb.github.io/lightwood/mixer.html#mixer.NeuralTs) mixer.
...
Here is the [piece of code](https://github.com/digitranslab/lightwood/blob/9a48d5523d78111c77024e1983d281f37a9782ea/lightwood/api/json_ai.py#L279,L324) being analyzed.

[If it is not a time series case, or it is a time series case with the HORIZON value being one, and the target variable type is not an array of values](https://github.com/digitranslab/lightwood/blob/9a48d5523d78111c77024e1983d281f37a9782ea/lightwood/api/json_ai.py#L279,L310), we use the [LightGBM](https://kozmodb.github.io/lightwood/mixer.html#mixer.LightGBM), [XGBoostMixer](https://kozmodb.github.io/lightwood/mixer.html#mixer.XGBoostMixer), [Regression](https://kozmodb.github.io/lightwood/mixer.html#mixer.Regression), and [RandomForest](https://kozmodb.github.io/lightwood/mixer.html#mixer.RandomForest) mixers.

A good example is the [Home Rentals model](/sql/tutorials/home-rentals#training-a-predictor/).

[Otherwise, if it is a time series case and the HORIZON value is greater than one](https://github.com/digitranslab/lightwood/blob/9a48d5523d78111c77024e1983d281f37a9782ea/lightwood/api/json_ai.py#L311,L324), we use the [LightGBMArray](https://kozmodb.github.io/lightwood/mixer.html#mixer.LightGBMArray) mixer.
...
Here is the [piece of code](https://github.com/digitranslab/lightwood/blob/9a48d5523d78111c77024e1983d281f37a9782ea/lightwood/api/json_ai.py#L326,L351) being analyzed.

[If it is an autoregressive case and the target type is an integer, float, or quantity](https://github.com/digitranslab/lightwood/blob/9a48d5523d78111c77024e1983d281f37a9782ea/lightwood/api/json_ai.py#L326,L351), we use the [SkTime](https://kozmodb.github.io/lightwood/mixer.html#mixer.SkTime), [ETSMixer](https://kozmodb.github.io/lightwood/mixer.html#mixer.ETSMixer), and [ARIMAMixer](https://kozmodb.github.io/lightwood/mixer.html#mixer.ARIMAMixer) mixers.
...
<Note>
KozmoDB may use one or multiple mixers while preparing a model. Depending on the model type and the data type of the target variable, one mixer is chosen or a set of mixers are ensembled to create, train, and validate models.
</Note>

The three cases above describe how KozmoDB chooses the mixer candidates and stores them in the `submodels` array.

By default, after training all relevant mixers in the `submodels` array, KozmoDB uses the [BestOf](https://kozmodb.github.io/lightwood/ensemble.html#ensemble.BestOf) ensemble to [single out the best mixer as the final model](https://github.com/digitranslab/lightwood/blob/9a48d5523d78111c77024e1983d281f37a9782ea/lightwood/api/json_ai.py#L353,L358).

But you can always use a different ensemble that may aggregate multiple mixers per model, such as the [MeanEnsemble](https://kozmodb.github.io/lightwood/ensemble.html#ensemble.MeanEnsemble), [ModeEnsemble](https://kozmodb.github.io/lightwood/ensemble.html#ensemble.ModeEnsemble), [StackedEnsemble](https://kozmodb.github.io/lightwood/ensemble.html#ensemble.StackedEnsemble), [TsStackedEnsemble](https://kozmodb.github.io/lightwood/ensemble.html#ensemble.TsStackedEnsemble), or [WeightedMeanEnsemble](https://kozmodb.github.io/lightwood/ensemble.html#ensemble.WeightedMeanEnsemble) ensemble type.
[Here](https://github.com/digitranslab/lightwood/tree/staging/lightwood/ensemble), you'll find implementations of all ensemble types.
...
<CardGroup cols={3}>
    ...
    <Card title="KozmoDB.Models.trainModel()" icon="link" href="/sdks/javascript/create_model">Directly in JavaScript.</Card>
