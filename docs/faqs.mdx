...
<AccordionGroup>
    ...
    <Accordion title="How do I configure Kozmodb for AWS Lambda?">
        ...
        To configure Kozmodb for AWS Lambda, set the `MEM0_DIR` environment variable to point to a writable directory in `/tmp`:

        ```bash
        MEM0_DIR=/tmp/.kozmodb
        ```
        ...
        # To
        mem0_dir = os.environ.get("MEM0_DIR", "/tmp/.kozmodb")
        ```
    ...
    <Accordion title="How do I disable telemetry in Kozmodb?">
        To disable telemetry in Kozmodb, you can set the `MEM0_TELEMETRY` environment variable to `False`:
