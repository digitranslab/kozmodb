...
This motivates the concept of *handlers*, which is an abstraction for the two types of entities mentioned above: *data layers* and *ML frameworks*. Handlers are meant to enforce a common and sufficient set of behaviors that all KozmoDB-compatible entities should support. By creating a handler, the target system is effectively integrated into the wider KozmoDB ecosystem.

## Types of Handlers

### Data Handlers

Data handlers act as a bridge to any database. You use data handlers to connect data sources, including databases and applications, using [the CREATE DATABASE command](/sql/create/databases/). So you can reach data from any database that has its handler implemented within KozmoDB.

<Tip>
Go ahead and implement a handler for the database of your choice! <a href="https://github.com/digitranslab/kozmodb/blob/main/docs/contribute/data-handlers.mdx">Here</a> you'll find instructions on how to implement a data handler.
</Tip>
...
<Tip>
Go ahead and implement a handler for the ML library or framework of your choice! <a href="https://github.com/digitranslab/kozmodb/blob/main/docs/contribute/ml-handlers.mdx">Here</a> you'll find instructions on how to implement an ML handler.
</Tip>

## Handlers in the KozmoDB Repository

The source code for integrations is located in the main KozmoDB repository under the [/integrations](https://github.com/digitranslab/kozmodb/tree/main/kozmodb/integrations) directory.
...
Inherit from the [`DatabaseHandler`](https://github.com/digitranslab/kozmodb/blob/main/kozmodb/integrations/libs/base.py#L102) class when adding a new data handler. For more info, visit our [doc page here](/contribute/data-handlers/).

Inherit from the [`BaseMLEngine`](https://github.com/digitranslab/kozmodb/blob/main/kozmodb/integrations/libs/base.py#L122) class when adding a new ML handler. For more info, visit our [doc page here](/contribute/ml-handlers/).

### Parsing SQL

Whenever you want to parse a string that contains SQL, we strongly recommend using the `kozmodb_sql` package. It provides a parser that fully supports the KozmoDB SQL dialect and partially the standard SQL dialect. There is also a *render* feature to map other dialects into the already supported ones.

### Formatting Output

In the case of data handlers, when it comes to building the response of the public methods, the output should be wrapped by the [kozmodb.integrations.libs.response.HandlerResponse](https://github.com/digitranslab/kozmodb/blob/main/kozmodb/integrations/libs/response.py#L7) or [kozmodb.integrations.libs.response.HandlerStatusResponse](https://github.com/digitranslab/kozmodb/blob/main/kozmodb/integrations/libs/response.py#L32) class. These classes are used by the KozmoDB executioner to orchestrate and coordinate multiple handler instances in parallel.
