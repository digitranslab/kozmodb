...
class Server(Project):
    """
    Server instance allows to manipulate project and databases (integration) on kozmodb server

    Attributes for accessing to different objects:

        - projects, see :func:`~kozmodb_sdk.projects.Projects`
        - databases, see :func:`~kozmodb_sdk.databases.Databases`
        - ml_engines, see :func:`~kozmodb_sdk.ml_engines.MLEngines`
        - ml_handlers, see :func:`~kozmodb_sdk.handlers.MLHandlers`
        - data_handlers, see :func:`~kozmodb_sdk.handlers.DataHandlers`

    Server is also root(kozmodb) project and has attributes of project:
        - models, see :func:`~kozmodb_sdk.models.Models`
        - views, see :func:`~kozmodb_sdk.views.Views`
        - jobs, see :func:`~kozmodb_sdk.jobs.Jobs`

    """

    def __init__(self, api, skills: Skills = None, agents: Agents = None):
        # server is also kozmodb project
        self.databases = Databases(api)
        self.ml_engines = MLEngines(api)
        super().__init__(self, api, 'kozmodb')
    ...
    def status(self) -> dict:
        ...
        >>> print(server.status()['kozmodb_version'])
