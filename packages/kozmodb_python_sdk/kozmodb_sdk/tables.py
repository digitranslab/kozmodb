...
from kozmodb_sql_parser.ast import DropTables, CreateTable
from kozmodb_sql_parser.ast import Select, Star, Identifier, Constant, Delete, Insert, Update, Last, BinaryOperation

from kozmodb_sdk.utils.sql import dict_to_binary_op, add_condition, query_to_native_query
...
class Table(Query):
    ...
    def track(self, column):
        """
        Apply tracking column to table. ('LAST' keyword in kozmodb)
...
class Tables(CollectionBase):
  ...
  ` Droping table
    ...
    def create(self, name: str, query: Union[pd.DataFrame, Query], replace: bool = False) -> Union[Table, Query]:
        ...
        On kozmodb server it executes command:
