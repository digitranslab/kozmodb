...
from kozmodb_sql.planner.steps import (FetchDataframeStep, ProjectStep, JoinStep, ApplyPredictorStep,
...
class TestPlanJoinPredictor:
    def test_join_predictor_plan(self):
        ...
        expected_plan = QueryPlan(
            steps=[
                ...
                ApplyPredictorStep(namespace='kozmodb', dataframe=Result(0), predictor=Identifier('pred')),
        ...
        plan = plan_query(query, integrations=['int'], predictor_namespace='kozmodb', predictor_metadata={'pred': {}})
    ...
    def test_join_predictor_plan_aliases(self):
        ...
        sql = """
            ...
            from int.tab1 ta, kozmodb.pred tb
        """
        query = parse_sql(sql)
        ...
        expected_plan = QueryPlan(
            steps=[
                ...
                ApplyPredictorStep(namespace='kozmodb', dataframe=Result(0), predictor=Identifier('pred', alias=Identifier('tb'))),
                JoinStep(left=Result(0), right=Result(1),
                         query=Join(left=Identifier('tab1'),
                                    right=Identifier('tab2'),
                                    join_type=JoinType.INNER_JOIN)),
    ...
    #                                    right=Identifier('kozmodb.hrp3', alias=Identifier('tb')),
    #                                    join_type=JoinType.INNER_JOIN,
    #                                    implicit=True),
    ...
    #         plan_query(query, integrations=['postgres_90'], predictor_namespace='kozmodb', predictor_metadata={'hrp3': {}})

    def test_join_predictor_plan_complex_query(self):

        sql = """
            select t.asset, t.time, m.predicted
            from int.tab t, kozmodb.pred m
            where t.col1 = 'x'
            group by t.asset
        ...
        expected_plan = QueryPlan(
            steps=[
                ...
                ApplyPredictorStep(namespace='kozmodb', dataframe=Result(0), predictor=Identifier('pred', alias=Identifier('m'))),
                JoinStep(left=Result(0), right=Result(1),
                         query=Join(left=Identifier('tab1'),
                                    right=Identifier('tab2'),
                                    join_type=JoinType.INNER_JOIN)),
    ...
    def test_join_predictor_plan_default_namespace_integration(self):
        ...
        plan = plan_query(query, integrations=['int'], predictor_namespace='kozmodb', default_namespace='int', predictor_metadata={'pred': {}})
    ...
    def test_join_predictor_plan_default_namespace_predictor(self):
        ...
        plan = plan_query(query, integrations=['int'], predictor_namespace='kozmodb', default_namespace='kozmodb', predictor_metadata={'pred': {}})
    ...
    def test_subselect(self):
        ...
        expected_plan = QueryPlan(
            ...
            steps=[
                ...
                ApplyPredictorStep(namespace='kozmodb', dataframe=Result(1), predictor=Identifier('pred')),
...
class TestPredictorWithUsing:
    def test_using_join(self):
        ...
        expected_plan = QueryPlan(
            steps=[
                ...
                ApplyPredictorStep(namespace='kozmodb', dataframe=Result(0),
        ...
        expected_plan = QueryPlan(
            steps=[
                ...
                ApplyPredictorStep(namespace='kozmodb', dataframe=Result(1),
                                   predictor=Identifier('pred'), params={'a': 1}),
                JoinStep(left=Result(1), right=Result(2),
                         query=Join(left=Identifier('tab1'),
                                    right=Identifier('tab2'),
    ...
    def test_using_one_line(self):
        ...
        sql = '''
            select * from kozmodb.pred where x=2 using a=1
        '''
...
class TestPredictorVersion:
    def test_using_join(self):
        ...
        plan = plan_query(query, integrations=['int'], predictor_namespace='kozmodb',
            ...
            steps=[
                ...
                JoinStep(left=Result(0), right=Result(1),
                         query=Join(left=Identifier('tab1'),
                                    right=Identifier('tab2'),
                                    join_type=JoinType.JOIN)),
                ApplyPredictorStep(namespace='kozmodb', dataframe=Result(2),
                                   predictor=Identifier('pred', alias=Identifier('m')), row_dict={'a': 1}),
    ...
    def test_complex_subselect(self):
        ...
        expected_plan = QueryPlan(
            steps=[
                ...
                # model
                ApplyPredictorStep(namespace='kozmodb', dataframe=Result(6),
