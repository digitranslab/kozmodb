...
class TestDDL:
    ...
    def test_drop_predictor_table_syntax_ok(self, dialect):
        sql = "DROP TABLE kozmodb.tbl"
        ast = parse_sql(sql, dialect=dialect)
        expected_ast = DropTables(tables=[Identifier('kozmodb.tbl')])
        assert str(ast) == str(expected_ast)
        assert ast.to_tree() == expected_ast.to_tree()

        sql = "DROP TABLE if exists kozmodb.tbl"
        ast = parse_sql(sql, dialect=dialect)
        expected_ast = DropTables(tables=[Identifier('kozmodb.tbl')], if_exists=True)
        assert str(ast) == str(expected_ast)
        assert ast.to_tree() == expected_ast.to_tree()
