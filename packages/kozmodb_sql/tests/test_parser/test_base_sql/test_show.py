...
from kozmodb_sql.parser.ast.show import Show
...
class TestShow:
    ...
    def test_show_function_status(self, dialect):
        sql = "show function status where Db = 'KOZMODB' AND Name LIKE '%'"
        ast = parse_sql(sql, dialect=dialect)
        expected_ast = Show(category='function status',
                            where=BinaryOperation('and', args=[
                                BinaryOperation('=', args=[Identifier('Db'), Constant('KOZMODB')]),
...
class TestShowAdapted:
    ...
    def test_show_database_adapted(self):
        ...
        statement2 = parse_sql(sql, dialect='kozmodb')
...
class TestKozmodb:
    ...
    def test_show(self):
        sql = '''
           show full databases
        '''
        statement = parse_sql(sql, dialect='kozmodb')
        statement2 = Show(
            category='databases',
            modes=['full']
        )
