...
class TestSelectStructure:
    ...
    def test_select_order(self, dialect):
        ...
        for perm in itertools.permutations(components):
            ...
            if dialect == 'sqlite':
                # TODO fix message for kozmodb dialect
    ...
    def test_backticks(self, dialect):
        sql = "SELECT `name`, `status` FROM `kozmodb`.`wow stuff predictors`.`even-dashes-work`.`nice`"
        ast = parse_sql(sql, dialect=dialect)

        expected_ast = Select(targets=[Identifier(parts=['name']), Identifier(parts=['status'])],
                              from_table=Identifier(parts=['kozmodb', 'wow stuff predictors', 'even-dashes-work', 'nice']),
                              )
    ...
    def test_tricky_tables_case(self, dialect):
        sql = "SELECT TABLES.table_name AS Tables_in_kozmodb FROM TABLES WHERE TABLES.table_schema = 'KOZMODB' AND TABLES.table_type = 'BASE TABLE'"
        ast = parse_sql(sql, dialect=dialect)

        expected_ast = Select(targets=[Identifier('TABLES.table_name', alias=Identifier('Tables_in_kozmodb'))],
                              from_table=Identifier('TABLES'),
                              where=BinaryOperation('and', args=[
                                  BinaryOperation('=', args=[Identifier('TABLES.table_schema'), Constant('KOZMODB')]),
...
@pytest.mark.parametrize('dialect', ['mysql', 'kozmodb'])
...
class TestKozmodb:
    ...
    def test_window_function_kozmodb(self):
