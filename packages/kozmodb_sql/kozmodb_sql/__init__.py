...
def get_lexer_parser(dialect):
    if dialect == 'sqlite':
        from kozmodb_sql.parser.lexer import SQLLexer
        from kozmodb_sql.parser.parser import SQLParser
        lexer, parser = SQLLexer(), SQLParser()
    elif dialect == 'mysql':
        from kozmodb_sql.parser.dialects.mysql.lexer import MySQLLexer
        from kozmodb_sql.parser.dialects.mysql.parser import MySQLParser
        lexer, parser = MySQLLexer(), MySQLParser()
    elif dialect == 'kozmodb':
        from kozmodb_sql.parser.dialects.kozmodb.lexer import KozmoDBLexer
        from kozmodb_sql.parser.dialects.kozmodb.parser import KozmoDBParser
        lexer, parser = KozmoDBLexer(), KozmoDBParser()
    else:
        raise ParsingException(f'Unknown dialect {dialect}. Available options are: sqlite, mysql.')
    return lexer, parser


def parse_sql(sql, dialect='kozmodb'):
