...
from kozmodb_sql.parser import ast
from kozmodb_sql.parser.ast import (Select, Identifier, Join, Star, BinaryOperation, Constant, Union, CreateTable,
                                    Function, Insert, Except, Intersect,
                                    Update, NativeQuery, Parameter, Delete)
from kozmodb_sql.planner import utils
from kozmodb_sql.planner.query_plan import QueryPlan
from kozmodb_sql.planner.steps import (FetchDataframeStep, ProjectStep, ApplyPredictorStep,
...
from kozmodb_sql.planner.utils import (disambiguate_predictor_column_identifier,
                                       get_deepest_select,
                                       recursively_extract_column_values,
                                       query_traversal, filters_to_bin_op)
from kozmodb_sql.planner.plan_join import PlanJoin
from kozmodb_sql.planner.query_prepare import PreparedStatementPlanner
...
class QueryPlanner:
    ...
    def __init__(self,
        ...
        # allow to select from kozmodb namespace
        _projects.add('kozmodb')

        self.default_namespace = default_namespace

        # legacy parameter
        self.predictor_namespace = predictor_namespace.lower() if predictor_namespace else 'kozmodb'
    ...
    def check_single_integration(self, query):
        ...
        # one integration and not kozmodb objects in query
        if (
                len(query_info['mdb_entities']) == 0
                and len(query_info['integrations']) == 1
                and 'files' not in query_info['integrations']
