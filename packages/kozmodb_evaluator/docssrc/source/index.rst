...
:Date: |today|
|
Welcome to the ``kozmodb_evaluator`` documentation. ``kozmodb_evaluator`` is a Python package aimed at simplifying accuracy and calibration evaluations for machine learning models.
...
You can install ``kozmodb_evalutor`` as follows:

.. code-block:: bash

   pip install kozmodb_evaluator
...
- Run ``cd kozmodb_evaluator && pip install --editable .``
...
6. Submit a pull request from your repo to the ``staging`` branch of ``kozmodb/kozmodb_evaluator`` so that we can review your changes. Be sure to merge the latest from staging before making a pull request!
...
^^^^^^^^^^^^^^^^^^^^^^^^
We use GitHub issues to track bugs and features. Report them by opening a `new issue <https://github.com/digitranslab/kozmodb_evaluator/issues/new/choose>`_ and fill out all of the required inputs.
...
Community
^^^^^^^^^^^^^^^^^^^^^^^^^
If you have additional questions or you want to chat with KozmoDB core team, you can join our community:

.. raw:: html

    <embed>
    <a href="https://join.slack.com/t/kozmodbcommunity/shared_invite/zt-o8mrmx3l-5ai~5H66s6wlxFfBMVI6wQ" target="_blank"><img src="https://img.shields.io/badge/slack-@kozmodbcommunity-blueviolet.svg?logo=slack " alt="KozmoDB Community"></a>
    </embed>

To get updates on KozmoDB’s latest announcements, releases, and events, sign up for our `Monthly Community Newsletter <https://kozmodb.com/newsletter/?utm_medium=community&utm_source=github>`_.

Join our mission of democratizing machine learning and allowing developers to become data scientists!

Contributor Code of Conduct
^^^^^^^^^^^^^^^^^^^^^^^^^^^
Please note that this project is released with a `Contributor Code of Conduct <https://github.com/digitranslab/kozmodb_evaluator/blob/stable/CODE_OF_CONDUCT.md>`_. By participating in this project, you agree to abide by its terms.
...
| `License <https://github.com/digitranslab/kozmodb_evaluator/blob/stable/LICENSE>`_
