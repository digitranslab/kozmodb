...
from kozmodb_evaluator.helpers.general import Module, filter_fn_args
from kozmodb_evaluator.accuracy.forecasting import \
    evaluate_array_accuracy, \
    evaluate_num_array_accuracy, \
    evaluate_cat_array_accuracy, \
    complementary_smape_array_accuracy
from kozmodb_evaluator.accuracy.llm_generation import evaluate_rag, _setup_rag_eval
...
def evaluate_accuracy(data: pd.DataFrame,
    ...
    if accuracy_function in ['rag_faithfulness', 'rag_answer_relevancy', 'rag_context_precision', 'rag_context_recall']:
        ...
        if hasattr(importlib.import_module('kozmodb_evaluator.accuracy'), accuracy_function):
            accuracy_function = getattr(importlib.import_module('kozmodb_evaluator.accuracy'), accuracy_function)
        else:
            raise Exception(f"Could not retrieve accuracy function: {accuracy_function}")
