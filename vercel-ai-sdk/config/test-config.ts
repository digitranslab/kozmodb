...
export const testConfig = {
  ...
  fetchDeleteId: async function () {
    ...
    try {
      const response = await fetch('https://api.kozmodb.ai/v1/entities/', options);
  ...
  deleteUser: async function () {
    ...
    try {
      const response = await fetch(`https://api.kozmodb.ai/v1/entities/user/${this.deleteId}/`, options);
      if (!response.ok) {
