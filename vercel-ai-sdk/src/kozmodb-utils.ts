...
import { KozmodbConfigSettings } from './kozmodb-types';
...
const convertToKozmodbFormat = (messages: LanguageModelV1Prompt) => {
    try {
        ...
        console.error("Error in convertToKozmodbFormat:", error);
        return [];
    }
}

const searchInternalMemories = async (query: string, config?: KozmodbConfigSettings, top_k: number = 5) => {
    try {
        ...
        const response = await fetch('https://api.kozmodb.ai/v2/memories/search/', options);
        if (!response.ok) {
...
const addMemories = async (messages: LanguageModelV1Prompt, config?: KozmodbConfigSettings) => {
    try {
        let finalMessages: Array<Message> = [];
        if (typeof messages === "string") {
            finalMessages = [{ role: "user", content: messages }];
        } else {
            finalMessages = convertToKozmodbFormat(messages);
        }
...
const updateMemories = async (messages: Array<Message>, config?: KozmodbConfigSettings) => {
    try {
        ...
        const response = await fetch('https://api.kozmodb.ai/v1/memories/', options);
        if (!response.ok) {
            throw new Error(`HTTP error! status: ${response.status}`);
        }
        const data = await response.json();
...
const retrieveMemories = async (prompt: LanguageModelV1Prompt | string, config?: KozmodbConfigSettings) => {
    try {
...
const getMemories = async (prompt: LanguageModelV1Prompt | string, config?: KozmodbConfigSettings) => {
    try {
        const message = typeof prompt === 'string' ? prompt : flattenPrompt(prompt);
        const memories = await searchInternalMemories(message, config);
...
const searchMemories = async (prompt: LanguageModelV1Prompt | string, config?: KozmodbConfigSettings) => {
    try {
        const message = typeof prompt === 'string' ? prompt : flattenPrompt(prompt);
        const memories = await searchInternalMemories(message, config);
        return memories;
