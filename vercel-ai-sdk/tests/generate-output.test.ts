...
describe.each(testConfig.providers)('TESTS: Generate/Stream Text with model %s', (provider: Provider) => {
  ...
  let kozmodb: ReturnType<typeof testConfig.createTestClient>;
  jest.setTimeout(50000);
  
  beforeEach(() => {
    kozmodb = testConfig.createTestClient(provider);
  });
  ...
  it("should generate text using kozmodb model", async () => {
    const { text } = await generateText({
      model: kozmodb(provider.activeModel, {
        user_id: userId,
      }),
  ...
  it("should stream text using Kozmodb provider", async () => {
