...
# NOTE do not `import from kozmodb` here
...
def create_data_dir(path: Path) -> None:
    ...
    try:
        ...
        raise Exception("KozmoDB storage directory could not be created") from e
...
class Config:
    ...
    use_docker_env: bool = os.environ.get("KOZMODB_DOCKER_ENV", False) is not False
    ...
    def __new__(cls, *args, **kwargs) -> "Config":
        ...
        if self.storage_root_path is None:
            if isinstance(os.environ.get("KOZMODB_STORAGE_DIR"), str):
                self.storage_root_path = os.environ["KOZMODB_STORAGE_DIR"]
            elif "root" in self._user_config.get("paths", {}):
                self.storage_root_path = self.user_config["paths"]["root"]
            else:
                self.storage_root_path = os.path.join(user_data_dir("kozmodb", "kozmodb"), "var/")
        ...
        self._default_config = {
            ...
            "storage_db": (
                "sqlite:///"
                + str(self.storage_root_path / "kozmodb.sqlite3.db")
            ...
            "logging": {
                "handlers": {
                    "console": {
                        "enabled": True,
                        "formatter": "default",
                        "level": "INFO",  # KOZMODB_CONSOLE_LOG_LEVEL or KOZMODB_LOG_LEVEL (obsolete)
                    },
                    "file": {
                        "enabled": False,
                        "level": "INFO",  # KOZMODB_FILE_LOG_LEVEL
                        "filename": "app.log",
            ...
            "api": {
                ...
                "postgres": {"host": api_host, "port": "55432", "database": "kozmodb"},
                "litellm": {
    ...
    def prepare_env_config(self) -> None:
        ...
        if os.environ.get("KOZMODB_STORAGE_DIR", "") != "":
            self._env_config["paths"] = {"root": Path(os.environ["KOZMODB_STORAGE_DIR"])}
        # endregion

        # region vars: permanent storage disabled?
        if os.environ.get("KOZMODB_STORAGE_BACKUP_DISABLED", "").lower() in (
            "1",
            "true",
        ):
        ...
        if os.environ.get("KOZMODB_ML_QUEUE_TYPE", "").lower() == "redis":
            self._env_config["ml_task_queue"] = {
                "type": "redis",
                "host": os.environ.get("KOZMODB_ML_QUEUE_HOST", "localhost"),
                "port": int(os.environ.get("KOZMODB_ML_QUEUE_PORT", 6379)),
                "db": int(os.environ.get("KOZMODB_ML_QUEUE_DB", 0)),
                "username": os.environ.get("KOZMODB_ML_QUEUE_USERNAME"),
                "password": os.environ.get("KOZMODB_ML_QUEUE_PASSWORD"),
            }
        # endregion

        # region vars: username and password
        http_username = os.environ.get("KOZMODB_USERNAME")
        http_password = os.environ.get("KOZMODB_PASSWORD")

        if bool(http_username) != bool(http_password):
            raise ValueError(
                "Both KOZMODB_USERNAME and KOZMODB_PASSWORD must be set together and must be non-empty strings."
            )
        ...
        # region logging
        if os.environ.get("KOZMODB_LOG_LEVEL", "") != "":
            self._env_config["logging"]["handlers"]["console"]["level"] = os.environ["KOZMODB_LOG_LEVEL"]
            self._env_config["logging"]["handlers"]["console"]["enabled"] = True
        if os.environ.get("KOZMODB_CONSOLE_LOG_LEVEL", "") != "":
            self._env_config["logging"]["handlers"]["console"]["level"] = os.environ["KOZMODB_CONSOLE_LOG_LEVEL"]
            self._env_config["logging"]["handlers"]["console"]["enabled"] = True
        if os.environ.get("KOZMODB_FILE_LOG_LEVEL", "") != "":
            self._env_config["logging"]["handlers"]["file"]["level"] = os.environ["KOZMODB_FILE_LOG_LEVEL"]
            self._env_config["logging"]["handlers"]["file"]["enabled"] = True
        # endregion

        if os.environ.get("KOZMODB_DB_CON", "") != "":
            self._env_config["storage_db"] = os.environ["KOZMODB_DB_CON"]

        if os.environ.get("KOZMODB_DEFAULT_PROJECT", "") != "":
            self._env_config["default_project"] = os.environ["KOZMODB_DEFAULT_PROJECT"].lower()

        if os.environ.get("KOZMODB_DEFAULT_LLM_API_KEY", "") != "":
            self._env_config["default_llm"] = {"api_key": os.environ["KOZMODB_DEFAULT_LLM_API_KEY"]}
        if os.environ.get("KOZMODB_DEFAULT_EMBEDDING_MODEL_API_KEY", "") != "":
            self._env_config["default_embedding_model"] = {
                "api_key": os.environ["KOZMODB_DEFAULT_EMBEDDING_MODEL_API_KEY"]
            }
        if os.environ.get("KOZMODB_DEFAULT_RERANKING_MODEL_API_KEY", "") != "":
            self._env_config["default_reranking_model"] = {
                "api_key": os.environ["KOZMODB_DEFAULT_RERANKING_MODEL_API_KEY"]
            }

        # Reranker configuration from environment variables
        reranker_config = {}
        if os.environ.get("KOZMODB_RERANKER_N", "") != "":
            try:
                reranker_config["n"] = int(os.environ["KOZMODB_RERANKER_N"])
            except ValueError:
                raise ValueError(f"KOZMODB_RERANKER_N must be an integer, got: {os.environ['KOZMODB_RERANKER_N']}")

        if os.environ.get("KOZMODB_RERANKER_LOGPROBS", "") != "":
            logprobs_value = os.environ["KOZMODB_RERANKER_LOGPROBS"].lower()
            if logprobs_value in ("true", "1", "yes", "y"):
                ...
                raise ValueError(
                    f"KOZMODB_RERANKER_LOGPROBS must be a boolean value, got: {os.environ['KOZMODB_RERANKER_LOGPROBS']}"
                )

        if os.environ.get("KOZMODB_RERANKER_TOP_LOGPROBS", "") != "":
            try:
                reranker_config["top_logprobs"] = int(os.environ["KOZMODB_RERANKER_TOP_LOGPROBS"])
            except ValueError:
                raise ValueError(
                    f"KOZMODB_RERANKER_TOP_LOGPROBS must be an integer, got: {os.environ['KOZMODB_RERANKER_TOP_LOGPROBS']}"
                )

        if os.environ.get("KOZMODB_RERANKER_MAX_TOKENS", "") != "":
            try:
                reranker_config["max_tokens"] = int(os.environ["KOZMODB_RERANKER_MAX_TOKENS"])
            except ValueError:
                raise ValueError(
                    f"KOZMODB_RERANKER_MAX_TOKENS must be an integer, got: {os.environ['KOZMODB_RERANKER_MAX_TOKENS']}"
                )

        if os.environ.get("KOZMODB_RERANKER_VALID_CLASS_TOKENS", "") != "":
            try:
                reranker_config["valid_class_tokens"] = os.environ["KOZMODB_RERANKER_VALID_CLASS_TOKENS"].split(",")
            except ValueError:
                raise ValueError(
                    f"KOZMODB_RERANKER_VALID_CLASS_TOKENS must be a comma-separated list of strings, got: {os.environ['KOZMODB_RERANKER_VALID_CLASS_TOKENS']}"
                )
        ...
        if os.environ.get("KOZMODB_DATA_CATALOG_ENABLED", "").lower() in ("1", "true"):
            self._env_config["data_catalog"] = {"enabled": True}

        if os.environ.get("KOZMODB_NO_STUDIO", "").lower() in ("1", "true"):
    ...
    def fetch_user_config(self) -> bool:
        ...
        if self._user_config is None:
            ...
            if isinstance(os.environ.get("KOZMODB_CONFIG_PATH"), str):
                self.config_path = os.environ["KOZMODB_CONFIG_PATH"]
    ...
    def parse_cmd_args(self) -> None:
        ...
        # if it is not kozmodb run, then set args to empty
        if (sys.modules["__main__"].__package__ or "").lower() != "kozmodb" and os.environ.get(
            "KOZMODB_RUNTIME"
        ) != "1":
        ...
        parser = argparse.ArgumentParser(description="CL argument for kozmodb server")
        ...
        parser.add_argument(
            ...
            type=str,
            default=None,
            help="Project containing the agent (default: kozmodb)",
        )

        parser.add_argument("--project-name", type=str, default=None, help="KozmoDB project name")
