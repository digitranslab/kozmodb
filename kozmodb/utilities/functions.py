...
from kozmodb.utilities.fs import create_process_mark, delete_process_mark, set_process_mark
    ...
    if Config().use_docker_env:
        ...
        return textwrap.dedent(f"""\
            ...
            ({container_id} is the ID of this container):
                ...
                docker exec {container_id} pip install 'kozmodb[{handler_name}]'""")
    else:
        return textwrap.dedent(f"""\
            To install the {handler_name} handler, run the following in your terminal:

                pip install 'kozmodb[{handler_name}]'  # If you installed kozmodb via pip
                pip install '.[{handler_name}]'        # If you installed kozmodb from source""")
...
def init_lexer_parsers():
    ...
    from kozmodb_sql_parser.parser import KozmoDBParser

    return KozmoDBLexer(), KozmoDBParser()
