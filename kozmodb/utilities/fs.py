...
def get_tmp_dir() -> Path:
    return Path(tempfile.gettempdir()).joinpath("kozmodb")
...
def create_pid_file():
    """
    Create kozmodb process pid file. Check if previous process exists and is running
    """
...
def delete_pid_file():
    ...
    if pid != str(os.getpid()):
        logger.warning(f"Process id in PID file ({pid_file}) doesn't match kozmodb pid")
        return
