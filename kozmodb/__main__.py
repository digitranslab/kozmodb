...
from kozmodb.utilities.ps import is_pid_listen_port, get_child_pids
import kozmodb.interfaces.storage.db as db
from kozmodb.utilities.fs import clean_process_marks, clean_unlinked_process_marks, create_pid_file, delete_pid_file
from kozmodb.utilities.context import context as ctx
from kozmodb.utilities.auth import register_oauth_client, get_aws_meta_data
from kozmodb.utilities.sentry import sentry_sdk  # noqa: F401
from kozmodb.utilities.api_status import set_api_status
...
def clean_kozmodb_tmp_dir():
    """Clean the KozmoDB tmp dir at exit."""
...
if __name__ == "__main__":
    ...
    if not (sys.version_info[0] >= 3 and sys.version_info[1] >= 10):
        print(
            """
     KozmoDB requires Python >= 3.10 to run

     Once you have supported Python version installed you can start kozmodb as follows:
     ...
     2. install KozmoDB:
     pip3 install kozmodb

     3. Run KozmoDB
     python3 -m kozmodb

     More instructions in https://docs.kozmodb.com
         """
        )
        exit(1)

    if config.cmd_args.version:
        print(f"KozmoDB {kozmodb_version}")
        sys.exit(0)

    if config.cmd_args.update_gui:
        from kozmodb.api.http.initialize import initialize_static
    ...
    apis = os.getenv("KOZMODB_APIS") or config.cmd_args.api
    ...
    logger.info(f"Version: {kozmodb_version}")
    ...
    if not is_cloud:
        ...
        try:
            from kozmodb.migrations import migrate
    ...
    atexit.register(clean_kozmodb_tmp_dir)
