...
from kozmodb_sql_parser.ast.kozmodb import AlterDatabase
...
import kozmodb.utilities.profiler as profiler

from kozmodb.api.executor.sql_query.result_set import Column, ResultSet
from kozmodb.api.executor.sql_query import SQLQuery
from kozmodb.api.executor.data_types.answer import ExecuteAnswer
from kozmodb.api.mysql.mysql_proxy.libs.constants.mysql import (
    CHARSET_NUMBERS,
    SERVER_VARIABLES,
    TYPES,
)

from kozmodb.api.executor.exceptions import (
    ExecutorException,
    BadDbError,
    NotSupportedYet,
    WrongArgumentError,
...
from kozmodb.api.executor.utilities.functions import download_file
from kozmodb.api.executor.utilities.sql import query_df
from kozmodb.integrations.libs.const import (
    HANDLER_CONNECTION_ARG_TYPE,
    PREDICTOR_STATUS,
)
from kozmodb.integrations.libs.response import HandlerStatusResponse
from kozmodb.interfaces.chatbot.chatbot_controller import ChatBotController
...
from kozmodb.interfaces.model.functions import (
    get_model_record,
    get_model_records,
    get_predictor_integration,
)
from kozmodb.interfaces.query_context.context_controller import query_context_controller
from kozmodb.interfaces.triggers.triggers_controller import TriggersController
...
from kozmodb.utilities.functions import mark_process, resolve_model_identifier, get_handler_install_message
...
class ExecuteCommands:
    ...
    def execute_command(self, statement: ASTNode, database_name: str = None) -> ExecuteAnswer:
        ...
        if statement_type is CreateDatabase:
            ...
            if sql_category == "ml_engines":
                ...
                schema = database_name or "kozmodb"
                ...
                # SHOW FUNCTION STATUS WHERE Db = 'KOZMODB';
                # SHOW PROCEDURE STATUS WHERE Db = 'KOZMODB'
                # SHOW FUNCTION STATUS WHERE Db = 'KOZMODB' AND Name LIKE '%';
            ...
            # FIXME if have answer on that request, then DataGrip show warning '[S0022] Column 'Non_unique' not found.'
                ...
                # SHOW CREATE TABLE `KOZMODB`.`predictors`
    ...
    def exec_service_function(self, statement: Select, database_name: str) -> Optional[ExecuteAnswer]:
        ...
        If input query is a single line select without FROM
          and has function in targets that matches with one of the kozmodb service functions:
    ...
    def answer_evaluate_metric(self, statement, database_name):
        ...
        from kozmodb_evaluator.accuracy.general import evaluate_accuracy
    ...
    def answer_create_database(self, statement: CreateDatabase) -> ExecuteAnswer:
        ...
        engine = (statement.engine or "kozmodb").lower()
