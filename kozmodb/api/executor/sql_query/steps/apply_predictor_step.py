...
from kozmodb.utilities.cache import get_cache, dataframe_checksum

from .base import BaseStepCall


def get_preditor_alias(step, kozmodb_database):
    ...
    return (kozmodb_database, predictor_name, predictor_alias)
...
class ApplyPredictorStepCall(ApplyPredictorBaseCall):
    ...
    def call(self, step):
        ...
        # adding __kozmodb_row_id, use first table if exists
        if len(data.find_columns("__kozmodb_row_id")) == 0:
            table = data.get_tables()[0] if len(data.get_tables()) > 0 else None

            row_id_col = Column(
                name="__kozmodb_row_id",
        ...
        project_datanode = self.session.datahub.get(project_name)
        if len(data) == 0:
            columns_names = project_datanode.get_table_columns_names(predictor_name) + ["__kozmodb_row_id"]
