...
from kozmodb.api.executor.utilities.sql import query_df_with_type_infer_fallback
from kozmodb.api.executor.exceptions import NotSupportedYet
...
class JoinStepCall(BaseStepCall):
    ...
    def call(self, step):
        ...
        if right_data.is_prediction or left_data.is_prediction:
            ...
            l_row_ids = left_data.find_columns('__kozmodb_row_id')
            r_row_ids = right_data.find_columns('__kozmodb_row_id')
            ...
            if len(l_row_ids) == 0:
                if len(r_row_ids) == 0:
                    ...
                    left_data.set_column_values('__kozmodb_row_id', right_data.get_column_values(idx))
                    l_row_ids = left_data.find_columns('__kozmodb_row_id')
            elif len(r_row_ids) == 0:
                # copy from left to right
                idx = left_data.get_col_index(l_row_ids[0])
                right_data.set_column_values('__kozmodb_row_id', left_data.get_column_values(idx))
                r_row_ids = right_data.find_columns('__kozmodb_row_id')
        ...
        for col in data.find_columns('__kozmodb_row_id'):
            data.del_column(col)
