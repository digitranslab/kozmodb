...
from kozmodb.api.executor.planner import utils
...
from kozmodb.api.executor.planner.utils import (
    disambiguate_predictor_column_identifier,
    recursively_extract_column_values,
    query_traversal,
    filters_to_bin_op,
)
from kozmodb.api.executor.planner.plan_join import PlanJoin
from kozmodb.api.executor.planner.query_prepare import PreparedStatementPlanner
from kozmodb.utilities.config import config


default_project = config.get("default_project")

# This includes built-in KozmoDB SQL functions and functions to be executed via DuckDB consistently.
KOZMODB_SQL_FUNCTIONS = {"llm", "to_markdown", "hash"}
    ...
    def get_query_info(self, query):
        ...
        def find_objects(node, is_table, **kwargs):
            if isinstance(node, Function):
                if node.namespace is not None or node.op.lower() in KOZMODB_SQL_FUNCTIONS:
