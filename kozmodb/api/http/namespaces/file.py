...
from kozmodb.api.http.namespaces.configs.files import ns_conf
...
from kozmodb.utilities.security import is_private_url, clear_filename, validate_urls
from kozmodb.utilities.fs import safe_extract
from kozmodb.integrations.utilities.files.file_reader import FileProcessingError
...
@ns_conf.route("/<name>")
@ns_conf.param("name", "KozmoDB's name for file")
class File(Resource):
    ...
    def put(self, name: str):
        ...
        if kozmodb_file_name in existing_file_names:
            return http_error(
                400,
                "File already exists",
                f"File with name '{kozmodb_file_name}' already exists",
            )
        ...
        if lp.endswith((".zip", ".tar.gz")):
            ...
            kozmodb_file_name = files[0]
            if not os.path.isfile(file_path):
                os.rmdir(temp_dir_path)
                return http_error(400, "Wrong content.", "Archive must contain data file in root.")

        try:
            ca.file_controller.save_file(kozmodb_file_name, file_path, file_name=original_file_name)
