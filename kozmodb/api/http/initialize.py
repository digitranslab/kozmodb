...
from kozmodb.__about__ import __version__ as kozmodb_version
from kozmodb.api.http.gui import update_static
from kozmodb.api.http.utils import http_error
from kozmodb.api.http.namespaces.agents import ns_conf as agents_ns
from kozmodb.api.http.namespaces.analysis import ns_conf as analysis_ns
from kozmodb.api.http.namespaces.auth import ns_conf as auth_ns
from kozmodb.api.http.namespaces.chatbots import ns_conf as chatbots_ns
from kozmodb.api.http.namespaces.jobs import ns_conf as jobs_ns
from kozmodb.api.http.namespaces.config import ns_conf as conf_ns
from kozmodb.api.http.namespaces.databases import ns_conf as databases_ns
from kozmodb.api.http.namespaces.default import ns_conf as default_ns
from kozmodb.api.http.namespaces.file import ns_conf as file_ns
from kozmodb.api.http.namespaces.handlers import ns_conf as handlers_ns
from kozmodb.api.http.namespaces.knowledge_bases import ns_conf as knowledge_bases_ns
from kozmodb.api.http.namespaces.models import ns_conf as models_ns
from kozmodb.api.http.namespaces.projects import ns_conf as projects_ns
from kozmodb.api.http.namespaces.skills import ns_conf as skills_ns
from kozmodb.api.http.namespaces.sql import ns_conf as sql_ns
from kozmodb.api.http.namespaces.tab import ns_conf as tab_ns
from kozmodb.api.http.namespaces.tree import ns_conf as tree_ns
from kozmodb.api.http.namespaces.views import ns_conf as views_ns
from kozmodb.api.http.namespaces.util import ns_conf as utils_ns
from kozmodb.api.http.namespaces.webhooks import ns_conf as webhooks_ns
from kozmodb.interfaces.database.integrations import integration_controller
from kozmodb.interfaces.database.database import DatabaseController
from kozmodb.interfaces.file.file_controller import FileController
from kozmodb.interfaces.jobs.jobs_controller import JobsController
from kozmodb.interfaces.storage import db
from kozmodb.metrics.server import init_metrics
from kozmodb.utilities import log
from kozmodb.utilities.config import config
from kozmodb.utilities.context import context as ctx
from kozmodb.utilities.json_encoder import CustomJSONProvider
from kozmodb.utilities.ps import is_pid_listen_port, wait_func_is_true
from kozmodb.utilities.sentry import sentry_sdk  # noqa: F401
from kozmodb.utilities.otel import trace  # noqa: F401
from kozmodb.api.common.middleware import verify_pat
...
def get_last_compatible_gui_version() -> Version | bool:
    ...
    try:
        res = requests.get(
            "https://kozmodb-web-builds.s3.amazonaws.com/compatible-config.json",
            timeout=5,
        )
    ...
    current_kozmodb_lv = parse_version(kozmodb_version)

    try:
        gui_versions = {}
        max_kozmodb_lv = None
        max_gui_lv = None
        for el in versions["kozmodb"]:
            if el["kozmodb_version"] is None:
                gui_lv = parse_version(el["gui_version"])
            else:
                kozmodb_lv = parse_version(el["kozmodb_version"])
                gui_lv = parse_version(el["gui_version"])
                if kozmodb_lv.base_version not in gui_versions or gui_lv > gui_versions[kozmodb_lv.base_version]:
                    gui_versions[kozmodb_lv.base_version] = gui_lv
                if max_kozmodb_lv is None or max_kozmodb_lv < kozmodb_lv:
                    max_kozmodb_lv = kozmodb_lv
            if max_gui_lv is None or max_gui_lv < gui_lv:
                max_gui_lv = gui_lv

        all_kozmodb_lv = [parse_version(x) for x in gui_versions.keys()]
        all_kozmodb_lv.sort()

        if current_kozmodb_lv.base_version in gui_versions:
            gui_version_lv = gui_versions[current_kozmodb_lv.base_version]
        elif current_kozmodb_lv > all_kozmodb_lv[-1]:
            gui_version_lv = max_gui_lv
        else:
            lower_versions = {
                key: value for key, value in gui_versions.items() if parse_version(key) < current_kozmodb_lv
            }
            if len(lower_versions) == 0:
                gui_version_lv = gui_versions[all_kozmodb_lv[0].base_version]
            else:
...
def initialize_flask(config, init_static_thread):
    ...
    # region required for windows https://github.com/digitranslab/kozmodb/issues/2526
    ...
    app.config["SWAGGER_HOST"] = "http://localhost:8000/kozmodb"
