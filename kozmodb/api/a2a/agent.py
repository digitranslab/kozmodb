...
import requests
import httpx
from kozmodb.api.a2a.utils import to_serializable, convert_a2a_message_to_qa_format
from kozmodb.api.a2a.constants import DEFAULT_STREAM_TIMEOUT
...
class KozmoDBAgent:
    """An agent that communicates with KozmoDB over HTTP following the A2A protocol."""
    ...
    def __init__(
        ...
        logger.info(f"Initialized KozmoDB agent connector to {self.base_url}")

    def invoke(self, query, session_id) -> Dict[str, Any]:
        """Send a query to the KozmoDB agent using SQL API."""
        try:
            escaped_query = query.replace("'", "''")
            sql_query = f"SELECT * FROM {self.project_name}.{self.agent_name} WHERE question = '{escaped_query}'"
            logger.debug(f"Sending SQL query to KozmoDB: {sql_query[:100]}...")
            response = requests.post(self.sql_url, json={"query": sql_query}, headers=self.headers)
            response.raise_for_status()
            data = response.json()
            logger.debug(f"Received response from KozmoDB: {json.dumps(data)[:200]}...")
            if "data" in data and len(data["data"]) > 0:
                ...
                error_msg = "Error: No data returned from KozmoDB"
            ...
            error_msg = f"Error connecting to KozmoDB: {str(e)}"
            logger.error(error_msg)
            return {
                "content": error_msg,
                "parts": [{"type": "text", "text": error_msg}],
    ...
    async def streaming_invoke(self, messages, timeout=DEFAULT_STREAM_TIMEOUT):
        ...
        logger.debug(f"Sending streaming request to KozmoDB agent: {self.agent_name}")
    ...
    async def stream(
        ...
        """Stream responses from the KozmoDB agent (uses streaming API endpoint)."""
        try:
