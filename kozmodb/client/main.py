...
from kozmodb.memory.setup import get_user_id, setup_config
from kozmodb.memory.telemetry import capture_client_event
...
class MemoryClient:
    """Client for interacting with the Kozmodb API.

    This class provides methods to create, retrieve, search, and delete memories
    using the Kozmodb API.

    Attributes:
        api_key (str): The API key for authenticating with the Kozmodb API.
        host (str): The base URL for the Kozmodb API.
    ...
    def __init__(
        ...
        if not self.api_key:
            raise ValueError("Kozmodb API Key not provided. Please provide an API Key.")
        ...
        if client is not None:
            ...
            self.client.headers.update({"Authorization": f"Token {self.api_key}", "Kozmodb-User-ID": self.user_id})
        else:
            self.client = httpx.Client(
                base_url=self.host,
                headers={"Authorization": f"Token {self.api_key}", "Kozmodb-User-ID": self.user_id},
                timeout=300,
            )
    ...
    def add(self, messages: List[Dict[str, str]], **kwargs) -> Dict[str, Any]:
        ...
        if kwargs.get("output_format") != "v1.1":
            ...
            warnings.warn(
                ...
                "Check out the docs for more information: https://docs.kozmodb.ai/platform/quickstart#4-1-create-memories",
                DeprecationWarning,
                stacklevel=2,
            )
        kwargs["version"] = "v2"
    ...
    def chat(self):
        """Start a chat with the Kozmodb AI. (Not implemented)
...
class AsyncMemoryClient:
    """Asynchronous client for interacting with the Kozmodb API.
    ...
    def __init__(
        ...
        if client is not None:
            ...
            self.async_client.headers.update({"Authorization": f"Token {self.api_key}", "Kozmodb-User-ID": self.user_id})
        else:
