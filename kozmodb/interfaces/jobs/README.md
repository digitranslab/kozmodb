...
CREATE JOB [<project_name>.]<job name> [AS] (
   <kozmodb sql query>[; <kozmodb sql query>]
)
START { now | <date> }
END { now | <date> }
EVERY [<numer>] <period> 
...
create job proj1.retrain_rentals (
   retrain kozmodb.home_rentals_model
   using
      join_learn_process=true;

   insert into int1.rentals(
      select * from int2.table1
      join kozmodb.home_rentals_model
   )
) 
...
create job slice_predictions (
   insert into int1.`result_{{START_DATE}}`(
      select * from int2.table1
      join kozmodb.home_rentals_model2
   )
)
every hour
```
...
```sql
create job becalm (
   drop model kozmodb.home_rentals_model
) 
start '2023-04-01'
```
...
- QUERY - kozmodb sql that is going to be called on every run
...
1. disable: scheduler activity. By default, scheduled is always starting with start of kozmodb. 
...
Kozmodb node runs jobs scheduler process.
This process:
...
- tries to lock it
  ...
  - because jobs_history table has the constraint on job_id, start_at: only one kozmodb node will be able to create such record
