...
from kozmodb.interfaces.agents.kozmodb_database_agent import extract_essential


class _KozmoDBSQLParserToolInput(BaseModel):
    tool_input: str = Field("", description="A SQL query to validate.")


class KozmoDBSQLParserTool(BaseTool):
    name: str = "kozmodb_sql_parser_tool"
    description: str = "Parse a SQL query to check it is valid KozmoDB SQL."
    args_schema: Type[BaseModel] = _KozmoDBSQLParserToolInput
