...
from kozmodb_sql_parser.ast.base import ASTNode
from kozmodb_sql_parser.ast import Select, Star, Constant, Identifier, BinaryOperation
...
from kozmodb.interfaces.database.views import ViewController
...
class Project:
    ...
    def delete(self):
        if self.record.metadata_ and self.record.metadata_.get("is_default", False):
            raise Exception(
                ...
                "The default project can be changed in the config file or by setting the environment variable KOZMODB_DEFAULT_PROJECT."
            )
    ...
    def _get_model_data(predictor_record, integraion_record, with_secrets: bool = True):
        ...
        predictor_meta = {
            ...
            "kozmodb_version": predictor_record.kozmodb_version,
        ...
        match str(table["type"]).upper():
            ...
            case "AGENT":
                ...
                if agent is not None:
                    from kozmodb.interfaces.agents.constants import ASSISTANT_COLUMN, USER_COLUMN
