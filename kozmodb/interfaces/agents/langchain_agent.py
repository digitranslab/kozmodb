...
from kozmodb.integrations.libs.llm.utils import get_llm_config
from kozmodb.integrations.utilities.handler_utils import get_api_key
from kozmodb.integrations.utilities.rag.settings import DEFAULT_RAG_PROMPT_TEMPLATE
from kozmodb.interfaces.agents.event_dispatch_callback_handler import (
    EventDispatchCallbackHandler,
)
from kozmodb.interfaces.agents.constants import AGENT_CHUNK_POLLING_INTERVAL_SECONDS
from kozmodb.utilities import log
from kozmodb.utilities.context_executor import ContextThreadPoolExecutor
from kozmodb.interfaces.storage import db
from kozmodb.utilities.context import context as ctx

from .kozmodb_chat_model import ChatKozmodb
...
from kozmodb.interfaces.agents.constants import (
    OPEN_AI_CHAT_MODELS,
...
from kozmodb.interfaces.skills.skill_tool import skill_tool, SkillData
from langchain_anthropic import ChatAnthropic
from langchain_openai import ChatOpenAI

from kozmodb.utilities.langfuse import LangfuseClientWrapper
...
def create_chat_model(args: Dict):
    ...
    if args["provider"] == "kozmodb":
        return ChatKozmodb(**model_kwargs)
...
class LangchainAgent:
    ...
    def _initialize_args(self, llm_params: dict = None) -> dict:
        ...
        if self.agent.provider == "kozmodb":
            args["model_name"] = self.agent.model_name
    ...
    def create_agent(self, df: pd.DataFrame) -> AgentExecutor:
        ...
        agent_executor = initialize_agent(
            ...
            agent_kwargs={
                ...
                "prefix": KOZMODB_PREFIX,  # Override default "Assistant is a large language model..." text
    ...
    def _get_agent_callbacks(self, args: Dict) -> List:
        ...
        if self.langfuse_client_wrapper.trace is None:
            ...
            trace_name = "NativeTrace-KozmoDB-AgentExecutor"
