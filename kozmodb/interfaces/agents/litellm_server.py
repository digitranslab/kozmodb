...
from kozmodb.interfaces.agents.mcp_client_agent import create_mcp_agent

logger = log.getLogger(__name__)

app = FastAPI(title="KozmoDB MCP Agent LiteLLM API")
...
async def chat_completions(request: ChatCompletionRequest):
    ...
    if agent_wrapper is None:
        raise HTTPException(status_code=500, detail="Agent not initialized. Make sure KozmoDB server is running with MCP enabled: python -m kozmodb --api=mysql,mcp,http")
...
async def direct_sql(request: DirectSQLRequest, background_tasks: BackgroundTasks):
    ...
    if agent_wrapper is None and mcp_session is None:
        raise HTTPException(status_code=500, detail="No MCP session available. Make sure KozmoDB server is running with MCP enabled.")
...
async def test_mcp_connection():
    ...
    try:
        ...
        error_detail = f"Error connecting to MCP server: {str(e)}. Make sure KozmoDB server is running with MCP enabled: python -m kozmodb --api=mysql,mcp,http"
...
async def init_agent(agent_name: str, project_name: str, mcp_host: str, mcp_port: int):
    ...
    try:
        ...
        logger.info("Make sure KozmoDB server is running with MCP enabled: python -m kozmodb --api=mysql,mcp,http")
...
async def run_server_async(
    ...
    if not success:
        logger.error("Failed to initialize agent. Make sure KozmoDB server is running with MCP enabled.")
        return 1
...
if __name__ == "__main__":
    ...
    parser.add_argument("--project", type=str, default="kozmodb", help="Project containing the agent")
