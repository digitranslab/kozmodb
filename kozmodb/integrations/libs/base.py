...
from kozmodb.integrations.libs.response import HandlerResponse, HandlerStatusResponse, RESPONSE_TYPE
...
class BaseHandler:
    ...
    broader KozmoDB ecosystem via SQL commands.
    """
...
class BaseMLEngine(ArgProbeMixin):
    ...
    that KozmoDB core expects, instances of this class will be wrapped with the `BaseMLEngineExec` class defined
    in `libs/ml_exec_base`.

    Broadly speaking, the flow is as follows:
      - A SQL statement is sent to the KozmoDB executor
      - The statement is parsed, and a sequential plan is generated by `kozmodb_sql`
      ...
      - Any output produced by the ML engine is then formatted by the wrapper and passed back into the KozmoDB executor, which can then morph the data to comply with the original SQL query
    """  # noqa
