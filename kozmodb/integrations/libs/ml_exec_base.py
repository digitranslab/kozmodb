...
In particular, three big components are included:
    ...
    - `BaseMLEngineExec` class: this class wraps any object that inherits from `BaseMLEngine` and exposes some endpoints
      ...
      output and format it into the HandlerResponse instance that KozmoDB core expects.

    - `learn_process` method: handles async dispatch of the `learn` method in an engine, as well as registering all
      models inside of the internal KozmoDB registry.
...
from kozmodb.utilities.hooks import after_predict as after_predict_hook
from kozmodb.interfaces.model.functions import (
    get_model_record
)
from kozmodb.integrations.libs.const import PREDICTOR_STATUS
...
from kozmodb.utilities.functions import mark_process
import kozmodb.utilities.profiler as profiler
from kozmodb.utilities.ml_task_queue.producer import MLTaskProducer
from kozmodb.utilities.ml_task_queue.const import ML_TASK_TYPE
from kozmodb.integrations.libs.process_cache import process_cache, empty_callback, MLProcessException
...
class BaseMLEngineExec:
    ...
    def learn(
        ...
        predictor_record = db.Predictor(
            ...
            kozmodb_version=kozmodb_version,
            to_predict=target,
    ...
    def predict(self, model_name: str, df: pd.DataFrame, pred_format: str = 'dict',
        ...
        # mdb indexes
        if '__kozmodb_row_id' not in predictions.columns and '__kozmodb_row_id' in df.columns:
            predictions['__kozmodb_row_id'] = df['__kozmodb_row_id']
