...
class ReplicateHandler(BaseMLEngine):
    ...
    def predict(self, df: pd.DataFrame, args: Optional[Dict] = None) -> pd.DataFrame:
        ...
        # Check if '__kozmodb_row_id' column exists and drop it if present
        if '__kozmodb_row_id' in df.columns:
            df.drop(columns=['__kozmodb_row_id'], inplace=True)

        def get_data(conditions):
            # Run prediction using KozmoDB's replicate library
...
Use DESCRIBE PREDICTOR kozmodb.<model_name>.features; to know about available parameters. OR
