...
from kozmodb_sql_parser.ast import Insert
...
def get_exec_call_tool(llm, executor, model_kwargs) -> Callable:
    def mdb_exec_call_tool(query: str) -> str:
        try:
            ...
            data = f"kozmodb tool failed with error:\n{str(e)}"   # let the agent know
    ...
    def mdb_write_call(query: str) -> str:
        try:
            ...
            if isinstance(ast_query, Insert):
                ...
                return "kozmodb write tool executed successfully"
        except Exception as e:
            return f"kozmodb write tool failed with error:\n{str(e)}"
