...
This documentation describes the integration of KozmoDB with [Hugging Face Inference API](https://huggingface.co/inference-api/serverless).
The integration allows for the deployment of Hugging Face models through Inference API within KozmoDB, providing the models with access to data from various data sources.
...
2. To use Hugging Face Inference API within KozmoDB, install the required dependencies following [this instruction](https://docs.kozmodb.com/setup/self-hosted/docker#install-dependencies).
3. Obtain the API key for Hugging Face Inference API required to deploy and use Hugging Face models through Inference API within KozmoDB. Generate tokens in the `Settings -> Access Tokens` tab of the Hugging Face account.

## Setup

Create an AI engine from the [Hugging Face Inference API handler](https://github.com/digitranslab/kozmodb/tree/main/kozmodb/integrations/handlers/huggingface_api_handler).
...
```sql
CREATE MODEL kozmodb.hf_text_classifier
PREDICT sentiment
USING
  task = 'text-classification',
...
```sql
CREATE MODEL kozmodb.hf_fill_mask
PREDICT sequence
USING
  task = 'fill-mask',
  engine = 'hf_api_engine',
...
```sql
CREATE MODEL kozmodb.hf_summarizer
PREDICT summary
USING
  task = 'summarization',
  engine = 'hf_api_engine',
...
```sql
CREATE MODEL kozmodb.hf_text_generator
...
```sql
CREATE MODEL kozmodb.hf_question_answerer
PREDICT answer
USING
...
```sql
CREATE MODEL kozmodb.hf_sentence_similarity
PREDICT similarity
USING
...
```sql
CREATE MODEL kozmodb.hf_zero_shot_classifier
PREDICT label
USING
...
```sql
CREATE MODEL kozmodb.hf_image_classifier
PREDICT label
USING
...
```sql
CREATE MODEL kozmodb.hf_object_detector
PREDICT objects
USING
...
```sql
CREATE MODEL kozmodb.hf_speech_recognizer
...
```sql
CREATE MODEL kozmodb.hf_audio_classifier
PREDICT label
USING
...
Follow [this link](https://docs.kozmodb.com/sql/tutorials/hugging-face-inference-api-examples) to see more use case examples.
</Tip>
