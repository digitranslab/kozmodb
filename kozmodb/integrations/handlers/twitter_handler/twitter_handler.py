...
from kozmodb.integrations.libs.api_handler import APIHandler, APITable, FuncParser
from kozmodb.integrations.utilities.sql_utils import extract_comparison_conditions
from kozmodb.integrations.utilities.date_utils import parse_utc_date
...
class TweetsTable(APITable):
    ...
    def select(self, query: ast.Select) -> Response:
        ...
        if 'query' not in params:
            # search not works without query, use 'kozmodb'
            params['query'] = 'kozmodb'
    ...
    def insert(self, query: ast.Insert):
        ...
        for p in insert_params:
            if p not in self.handler.connection_args:
                raise Exception(f'To insert data into Twitter, you need to provide the following parameters when connecting it to KozmoDB: {insert_params}')  # noqa
