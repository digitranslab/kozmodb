...
from kozmodb.integrations.handlers.langchain_embedding_handler.vllm_embeddings import VLLMEmbeddings
from kozmodb.integrations.handlers.langchain_embedding_handler.fastapi_embeddings import FastAPIEmbeddings
...
class LangchainEmbeddingHandler(BaseMLEngine):
    """
    Bridge class to connect langchain.embeddings module to kozmoDB
    """
    ...
    def create(
        ...
        if "input_columns" not in user_args and df is not None:
            # ignore private columns starts with __kozmodb
            # ignore target column in the input dataframe
            user_args["input_columns"] = [
                col
                for col in df.columns.tolist()
                if not col.startswith("__kozmodb") and col != target
            ]
