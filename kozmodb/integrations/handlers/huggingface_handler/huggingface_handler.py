...
from kozmodb.integrations.handlers.huggingface_handler.settings import FINETUNE_MAP
...
class HuggingFaceHandler(BaseMLEngine):
    ...
    def create_validation(target, args=None, **kwargs):
        ...
        if "pytorch" not in metadata.tags:
            raise Exception(
                "Currently only PyTorch models are supported (https://huggingface.co/models?library=pytorch&sort=downloads). To request another library, please contact us on our community slack (https://kozmodb.com/joincommunity)."
            )
    ...
    def create(self, target, args=None, **kwargs):
        ...
        # Otherwise download it
            ...
            try:
                ...
                raise Exception(
                    "Error while downloading and setting up the model. Please try a different model. We're working on expanding the list of supported models, so we would appreciate it if you let us know about this in our community slack (https://kozmodb.com/joincommunity)."
                )  # noqa
        ####
