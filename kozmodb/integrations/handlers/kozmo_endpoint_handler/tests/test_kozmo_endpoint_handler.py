...
@pytest.mark.skipif(os.environ.get('MDB_TEST_KOZMO_ENDPOINT_API_KEY') is None, reason='Missing API key!')
class TestKozmoEndpoint(BaseMLAPITest):
    """
    Integration tests for Kozmo Endpoint engine.
    """
    ...
    def setup_method(self):
        """
        Setup test environment by creating a project and a Kozmo Endpoint engine.
        """
        ...
        self.run_sql(
            f"""
            CREATE ML_ENGINE kozmo_endpoint_engine
            FROM kozmo_endpoint
            USING
            kozmo_endpoint_api_key = '{self.get_api_key('MDB_TEST_KOZMO_ENDPOINT_API_KEY')}';
            """
        )
    ...
    def test_create_model_raises_exception_with_invalid_model_parameter(self):
        ...
        self.run_sql(
            f"""
            CREATE MODEL proj.test_kozmo_endpoint_invalid_parameter_model
            PREDICT answer
            USING
                engine='kozmo_endpoint_engine',
        ...
        with pytest.raises(Exception):
            self.wait_predictor("proj", "test_kozmo_endpoint_invalid_model")
    ...
    def test_create_model_raises_exception_with_unknown_model_argument(self):
        ...
        self.run_sql(
            f"""
            CREATE MODEL proj.test_kozmo_endpoint_unknown_argument_model
            PREDICT answer
            USING
                engine='kozmo_endpoint_engine',
                prompt_template='dummy_prompt_template',
        ...
        with pytest.raises(Exception):
            self.wait_predictor("proj", "test_kozmo_endpoint_unknown_argument_model")
    ...
    def test_select_runs_no_errors_on_chat_completion_question_answering_single(self):
        ...
        self.run_sql(
            f"""
            CREATE MODEL proj.test_kozmo_endpoint_single_qa
            PREDICT answer
            USING
                engine='kozmo_endpoint_engine',
        ...
        self.wait_predictor("proj", "test_kozmo_endpoint_single_qa")

        result_df = self.run_sql(
            """
            SELECT answer
            FROM proj.test_kozmo_endpoint_single_qa
    ...
    def test_select_runs_no_errors_on_chat_completion_question_answering_bulk(self, mock_postgres_handler):
        ...
        self.run_sql(
            f"""
            CREATE MODEL proj.test_kozmo_endpoint_bulk_qa
            PREDICT answer
            USING
                engine='kozmo_endpoint_engine',
                question_column='question',
        ...
        self.wait_predictor("proj", "test_kozmo_endpoint_bulk_qa")

        result_df = self.run_sql(
            """
            SELECT p.answer
            FROM pg.df as t
            JOIN proj.test_kozmo_endpoint_bulk_qa as p;
        """
        )
    ...
    def test_select_runs_no_errors_on_embeddings_completion_single(self):
        ...
        self.run_sql(
            f"""
            CREATE MODEL proj.test_kozmo_endpoint_single_embeddings
            PREDICT embeddings
            USING
                engine='kozmo_endpoint_engine',
        ...
        self.wait_predictor("proj", "test_kozmo_endpoint_single_embeddings")

        result_df = self.run_sql(
            """
            SELECT embeddings
            FROM proj.test_kozmo_endpoint_single_embeddings
            WHERE text = 'KozmoDB';
            """
        )
