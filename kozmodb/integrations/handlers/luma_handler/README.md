...
Luma handler for KozmoDB provides interfaces to connect to LUMA Events via APIs and pull event data into KozmoDB.
...
~~~~sql
SELECT * FROM kozmodb_luma.events;
~~~~

~~~~sql
SELECT * FROM kozmodb_luma.events where event_id="evt-HQ36IFDwncocuGy";
~~~~

Create an Event

~~~~sql
INSERT INTO kozmodb_luma.events (name, start_at, timezone, end_at, require_rsvp_approval, geo_address_json_type, geo_address_json_place_id, geo_address_json_description, geo_latitude, geo_longitude, meeting_url)
VALUES 
