...
from kozmodb.interfaces.storage.fs import FsStore
from kozmodb.integrations.handlers.lightwood_handler.lightwood_handler.lightwood_handler import LightwoodHandler
from kozmodb.interfaces.database.integrations import integration_controller
from kozmodb.integrations.utilities.test_utils import PG_HANDLER_NAME, PG_CONNECTION_DATA
...
os.environ['KOZMODB_STORAGE_DIR'] = os.environ.get('KOZMODB_STORAGE_DIR', temp_dir)
os.environ['KOZMODB_DB_CON'] = 'sqlite:///' + os.path.join(os.environ['KOZMODB_STORAGE_DIR'], 'kozmodb.sqlite3.db') + '?check_same_thread=False&timeout=30'

migrate.migrate_to_head()

# from kozmodb.integrations.handlers.lightwood_handler.lightwood_handler.utils import load_predictor
...
class LightwoodHandlerTest(unittest.TestCase):
    ...
    #                m.ma as forecast
    #         FROM kozmodb.{self.test_model_2} m JOIN {PG_HANDLER_NAME}.demo_data.house_sales t
    ...
    #     target = 'Traffic'
    #     oby = 'T'
    #     query = f"SELECT tb.{target} as predicted, ta.{target} as truth, ta.{oby} from kozmodb.{self.test_tsmodel_name_1} AS tb JOIN {self.sql_handler_name}.{self.data_table_name_2} AS ta ON 1=1 WHERE ta.{oby} > LATEST LIMIT 10"
    #     parsed = self.handler.parser(query, dialect=self.handler.dialect)
