...
env vars to contloll BYOM:
 - KOZMODB_BYOM_ENABLED - can BYOM be uysed or not. Locally enabled by default.
 - KOZMODB_BYOM_INHOUSE_ENABLED - enable or disable 'inhouse' BYOM usage. Locally enabled by default.
 - KOZMODB_BYOM_DEFAULT_TYPE - [inhouse|venv] default byom type. Locally it is 'venv' by default.
 - KOZMODB_BYOM_TYPE - [safe|unsafe] - obsolete, same as above.
"""
...
from kozmodb.utilities.fs import safe_extract
...
from kozmodb.integrations.utilities.utils import format_exception_error
...
class BYOMHandler(BaseMLEngine):
    ...
    def __init__(self, model_storage, engine_storage, **kwargs) -> None:
        ...
        if is_cloud is True:
            byom_enabled = os.environ.get('KOZMODB_BYOM_ENABLED', 'false').lower()
        ...
        try:
            ...
            if os.environ.get('KOZMODB_BYOM_DEFAULT_TYPE') is not None:
                self._default_byom_type = BYOM_TYPE[
                    os.environ.get('KOZMODB_BYOM_DEFAULT_TYPE').upper()
                ]
            else:
                env_var = os.environ.get('KOZMODB_BYOM_DEFAULT_TYPE')
                if env_var == 'SAVE':
                    self._default_byom_type = BYOM_TYPE['VENV']
                elif env_var == 'UNSAVE':
        ...
        env_var = os.environ.get('KOZMODB_BYOM_INHOUSE_ENABLED')
        if env_var is None:
...
class ModelWrapperSafe:
    ...
    def prepare_env(self, modules, engine_id, engine_version: int):
        try:
            ...
            if self.is_cloud:
                bese_env_path = Path(tempfile.gettempdir()) / 'kozmodb' / 'venv'
