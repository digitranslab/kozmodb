...
# - "kozmodb:cloud"        - This functions as a 'latest' tag for the cloud image
# - "kozmodb:v1.2.3-cloud" - For this specific version
...
function "get_tags" {
  ...
  result = [
    ...
    PUSH_TO_DOCKERHUB ? "kozmodb/${IMAGE}:${VERSION}${notequal(image, "bare") ? "-${image}" : ""}" : "",
    PUSH_TO_DOCKERHUB ? "kozmodb/${IMAGE}:${notequal(image, "bare") ? image : "latest"}" : ""
  ]
} 
...
### OUTPUT IMAGES ###
target "base" {
  dockerfile = "docker/kozmodb.Dockerfile"
