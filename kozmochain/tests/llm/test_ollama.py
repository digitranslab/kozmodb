...
from kozmochain.llm.ollama import OllamaLlm
...
def test_get_llm_model_answer(ollama_llm_config, mocker):
    mocker.patch("kozmochain.llm.ollama.Client.list", return_value={"models": [{"name": "llama2"}]})
    mocker.patch("kozmochain.llm.ollama.OllamaLlm._get_answer", return_value="Test answer")
...
def test_get_answer_mocked_ollama(ollama_llm_config, mocker):
    mocker.patch("kozmochain.llm.ollama.Client.list", return_value={"models": [{"name": "llama2"}]})
    mocked_ollama = mocker.patch("kozmochain.llm.ollama.Ollama")
...
def test_get_llm_model_answer_with_streaming(ollama_llm_config, mocker):
    ...
    mocker.patch("kozmochain.llm.ollama.Client.list", return_value={"models": [{"name": "llama2"}]})
    mocked_ollama_chat = mocker.patch("kozmochain.llm.ollama.OllamaLlm._get_answer", return_value="Test answer")
