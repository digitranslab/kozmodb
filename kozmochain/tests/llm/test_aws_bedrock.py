...
from kozmochain.llm.aws_bedrock import AWSBedrockLlm
...
def test_get_llm_model_answer(config, mocker):
    mocked_get_answer = mocker.patch("kozmochain.llm.aws_bedrock.AWSBedrockLlm._get_answer", return_value="Test answer")
...
def test_get_llm_model_answer_with_streaming(config, mocker):
    config.stream = True
    mocked_bedrock_chat = mocker.patch("kozmochain.llm.aws_bedrock.BedrockLLM")
