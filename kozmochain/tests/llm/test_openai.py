...
from kozmochain.config import BaseLlmConfig
...
def test_get_llm_model_answer(config, mocker):
    mocked_get_answer = mocker.patch("kozmochain.llm.openai.OpenAILlm._get_answer", return_value="Test answer")
...
def test_get_llm_model_answer_with_token_usage(config, mocker):
    ...
    mocked_get_answer = mocker.patch(
        "kozmochain.llm.openai.OpenAILlm._get_answer",
...
def test_get_llm_model_answer_with_streaming(config, mocker):
    config.stream = True
    mocked_openai_chat = mocker.patch("kozmochain.llm.openai.ChatOpenAI")
...
def test_get_llm_model_answer_with_http_client_proxies(env_config, mocker):
    ...
    config = BaseLlmConfig(
        ...
        system_prompt="System prompt",
        model="gpt-4o-mini",
        http_client_proxies="http://testproxy.kozmodb.net:8000",
    )
    ...
    mock_http_client.assert_called_once_with(proxies="http://testproxy.kozmodb.net:8000")
...
def test_get_llm_model_answer_with_http_async_client_proxies(env_config, mocker):
    ...
    config = BaseLlmConfig(
        ...
        system_prompt="System prompt",
        model="gpt-4o-mini",
        http_async_client_proxies={"http://": "http://testproxy.kozmodb.net:8000"},
    )
    ...
    mock_http_async_client.assert_called_once_with(proxies={"http://": "http://testproxy.kozmodb.net:8000"})
