...
from kozmochain.config import AppConfig
from kozmochain.config.vector_db.pinecone import PineconeDBConfig
from kozmochain.embedder.base import BaseEmbedder
from kozmochain.vectordb.weaviate import WeaviateDB
...
class TestWeaviateDb(unittest.TestCase):
    ...
    @patch("kozmochain.vectordb.weaviate.weaviate")
    def test_initialize(self, weaviate_mock):
            ...
            "classes": [
                {
                    ...
                    "properties": [
                        ...
                        {
                            "name": "metadata",
                            "dataType": ["Kozmochain_store_1536_metadata"],
                        },
                    ],
                },
                {
                    "class": "Kozmochain_store_1536_metadata",
                    "vectorizer": "none",
                    "properties": [
                        {
                            "name": "data_type",
        ...
        self.assertEqual(db.index_name, "Kozmochain_store_1536")
    ...
    def test_add(self, weaviate_mock):
        ...
        weaviate_client_batch_enter_mock.add_data_object.assert_any_call(
            data_object={"text": documents[0]}, class_name="Kozmochain_store_1536_metadata", vector=[1, 2, 3]
        )

        weaviate_client_batch_enter_mock.add_data_object.assert_any_call(
            data_object={"text": documents[0]},
            class_name="Kozmochain_store_1536_metadata",
            vector=[1, 2, 3],
        )
    ...
    def test_query_without_where(self, weaviate_mock):
        ...
        weaviate_client_query_mock.get.assert_called_once_with("Kozmochain_store_1536", ["text"])
    ...
    def test_query_with_where(self, weaviate_mock):
        ...
        weaviate_client_query_get_mock.with_where.assert_called_once_with(
            {"operator": "Equal", "path": ["metadata", "Kozmochain_store_1536_metadata", "doc_id"], "valueText": "123"}
        )
    ...
    def test_reset(self, weaviate_mock):
        ...
        weaviate_client_batch_mock.delete_objects.assert_called_once_with(
            "Kozmochain_store_1536", where={"path": ["identifier"], "operator": "Like", "valueText": ".*"}
        )
    ...
    def test_count(self, weaviate_mock):
        ...
        weaviate_client_query.aggregate.assert_called_once_with("Kozmochain_store_1536")
