...
from kozmochain import Client


class TestClient:
    @pytest.fixture
    def mock_requests_post(self, mocker):
        return mocker.patch("kozmochain.client.requests.post")
    ...
    def test_load_api_key_from_config(self, mocker):
        mocker.patch("kozmochain.Client.load_config", return_value={"api_key": "test_api_key"})
        client = Client()
        assert client.get() == "test_api_key"

    def test_load_invalid_api_key_from_config(self, mocker):
        mocker.patch("kozmochain.Client.load_config", return_value={})
        with pytest.raises(ValueError):
            Client()
