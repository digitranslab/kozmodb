...
from kozmochain.loaders.youtube_video import YoutubeVideoLoader
...
def test_load_data(youtube_video_loader):
    ...
    with patch("kozmochain.loaders.youtube_video.YoutubeLoader.from_youtube_url", return_value=mock_loader), patch(
        "kozmochain.loaders.youtube_video.YouTubeTranscriptApi.get_transcript", return_value=mock_transcript
    ):
...
def test_load_data_with_empty_doc(youtube_video_loader):
    ...
    with patch("kozmochain.loaders.youtube_video.YoutubeLoader.from_youtube_url", return_value=mock_loader):
