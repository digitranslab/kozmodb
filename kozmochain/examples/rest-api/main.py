...
app = FastAPI(
    title="Kozmochain REST API",
    description="This is the REST API for Kozmochain.",
    version="0.0.1",
    license_info={
        "name": "Apache 2.0",
        "url": "https://github.com/digitranslab/kozmochain/blob/main/LICENSE",
    },
)
...
async def create_app_using_default_config(app_id: str, config: UploadFile = None, db: Session = Depends(get_db)):
    ...
    If you don't provide a config file, Kozmochain will use the default config file\n
...
async def add_datasource_to_an_app(body: SourceApp, app_id: str, db: Session = Depends(get_db)):
    ...
    data_type: The data type of the source. Remove it if you want Kozmochain to detect it automatically.\n
    """
    try:
        if app_id is None:
            raise HTTPException(
...
# FIXME: The chat implementation of Kozmochain needs to be modified to work with the REST API.
# @app.post(
#     "/{app_id}/chat",
#     tags=["Apps"],
#     response_model=DefaultResponse,
...
async def deploy_app(body: DeployAppRequest, app_id: str, db: Session = Depends(get_db)):
    ...
    Kozmochain will use the API key previously used (if any).\n
    """
    try:
        if app_id is None:
            raise HTTPException(
        ...
        # this will save the api key in the kozmochain.db
