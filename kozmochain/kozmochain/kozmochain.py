...
from kozmochain.config import AddConfig, BaseLlmConfig, ChunkerConfig
from kozmochain.config.base_app_config import BaseAppConfig
from kozmochain.core.db.models import ChatHistory, DataSource
from kozmochain.data_formatter import DataFormatter
...
from kozmochain.models.data_type import (
    DataType,
    DirectDataType,
    IndirectDataType,
    SpecialDataType,
)
from kozmochain.utils.misc import detect_datatype, is_valid_json_string
...
class KozmoChain(JSONSerializable):
    def __init__(
        self,
        ...
        """
        Initializes the KozmoChain instance, sets up a vector DB client and
    ...
    def add(
        ...
        **kwargs: Optional[dict[str, Any]],
        ...
        try:
            DataType(source)
            logger.warning(
                f"""Starting from version v0.0.40, Kozmochain can automatically detect the data type. So, in the `add` method, the argument order has changed. You no longer need to specify '{source}' for the `source` argument. So the code snippet will be `.add("{data_type}", "{source}")`"""  # noqa #E501
            )
            logger.warning(
                "Kozmochain is swapping the arguments for you. This functionality might be deprecated in the future, so please adjust your code."  # noqa #E501
            )
        ...
        if data_type:
            try:
                ...
                logger.info(
                    f"Invalid data_type: '{data_type}', using `custom` instead.\n Check docs to pass the valid data type: `https://docs.digi-trans.org/data-sources/overview`"  # noqa: E501
                )
