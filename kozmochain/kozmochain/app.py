...
from kozmochain.client import Client
from kozmochain.config import AppConfig, CacheConfig, ChunkerConfig, KozmodbConfig
from kozmochain.core.db.database import get_session
from kozmochain.core.db.models import DataSource
from kozmochain.kozmochain import KozmoChain
from kozmochain.embedder.base import BaseEmbedder
from kozmochain.embedder.openai import OpenAIEmbedder
from kozmochain.evaluation.base import BaseMetric
from kozmochain.evaluation.metrics import (
    AnswerRelevance,
    ContextRelevance,
    Groundedness,
)
...
from kozmochain.llm.base import BaseLlm
from kozmochain.llm.openai import OpenAILlm
from kozmochain.telemetry.posthog import AnonymousTelemetry
from kozmochain.utils.evaluation import EvalData, EvalMetric
from kozmochain.utils.misc import validate_config
from kozmochain.vectordb.base import BaseVectorDB
from kozmochain.vectordb.chroma import ChromaDB
...
class App(KozmoChain):
    """
    KozmoChain App lets you create a LLM powered app for your unstructured
    ...
    def __init__(
        ...
        memory_config: KozmodbConfig = None,
    ...
    def _init_client(self):
        ...
        if config.get("api_key"):
            ...
            api_key = input(
                "🔑 Enter your Kozmochain API key. You can find the API key at https://app.digi-trans.org/settings/keys/ \n"  # noqa: E501
            )
    ...
    def _get_pipeline(self, id):
        ...
        print(
            f"🎉 Pipeline loaded successfully! Pipeline url: https://app.digi-trans.org/pipelines/{r.json()['id']}\n"  # noqa: E501
        )
        return r.json()
    ...
    def _create_pipeline(self):
        ...
        if r.status_code == 200:
            print(
                f"🎉🎉🎉 Existing pipeline found! View your pipeline: https://app.digi-trans.org/pipelines/{r.json()['id']}\n"  # noqa: E501
            )  # noqa: E501
        elif r.status_code == 201:
            print(
                f"🎉🎉🎉 Pipeline created successfully! View your pipeline: https://app.digi-trans.org/pipelines/{r.json()['id']}\n"  # noqa: E501
            )
        return r.json()
    ...
    def from_config(
        ...
        memory_config = KozmodbConfig(**memory_config_data) if memory_config_data else None
