...
<Tabs>
    <Tab title="docker">
        ```bash
        docker run --name slack-bot -e OPENAI_API_KEY=sk-xxx -e SLACK_BOT_TOKEN=xxx -p 8000:8000 kozmochain/slack-bot
        ```
    </Tab>
    <Tab title="python">
        ```bash
        pip install --upgrade "kozmochain[slack]"
        python3 -m kozmochain.bots.slack --port 8000
        ```
</Tab>
</Tabs>
