...
```bash
pip install --upgrade kozmochain
```
...
<Tabs>
    <Tab title="docker">
        ```bash
        docker run --name whatsapp-bot -e OPENAI_API_KEY=sk-xxx -p 8000:8000 kozmochain/whatsapp-bot
        ```
    </Tab>
    <Tab title="python">
        ```bash
        python -m kozmochain.bots.whatsapp --port 5000
        ```
    </Tab>
</Tabs>
