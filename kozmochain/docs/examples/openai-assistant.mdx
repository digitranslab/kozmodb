...
Kozmochain now supports [OpenAI Assistants API](https://platform.openai.com/docs/assistants/overview) which allows you to build AI assistants within your own applications. An Assistant has instructions and can leverage models, tools, and knowledge to respond to user queries.
...
Creating an OpenAI Assistant using Kozmochain is very simple 3 step process.
...
```python Initialize
from kozmochain.store.assistants import OpenAIAssistant
...
You can add any custom data source that is supported by Kozmochain. Else, you can directly pass the file path on your local system and Kozmochain propagates it to OpenAI Assistant.
```python Add data
