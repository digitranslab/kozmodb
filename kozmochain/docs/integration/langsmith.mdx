...
Kozmochain now supports integration with [LangSmith](https://www.langchain.com/langsmith).
...
2. Set the environment variables in your app so that kozmochain has context about it.
3. Just use kozmochain and everything will be logged to LangSmith, so that you can better test and monitor your application.
...
* Now create an app using Kozmochain and everything will be automatically visible in the LangSmith


```python
from kozmochain import App

# Initialize KozmoChain application.
app = App()
