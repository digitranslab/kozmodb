...
Get started with full-stack RAG applications using Kozmochain's easy-to-use CLI tool. Set up everything with just a few commands, whether you prefer Docker or not.
...
- Kozmochain python package (`pip install kozmochain`)
...
Before proceeding, make sure you have the Kozmochain package installed.

```bash
pip install kozmochain -U
```
...
1. Kozmochain fetches a full stack template (FastAPI backend, Next.JS frontend).
...
Check out the Kozmochain admin panel to see the document chunks for your RAG application.
