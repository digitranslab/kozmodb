...
<ParamField path="yaml_path" type="str" Optional>
    Kozmochain pipeline config yaml path to use. This will define the configuration of the AI Assistant (such as configuring the LLM, vector database, and embedding model)
</ParamField>
...
<ParamField path="collect_metrics" type="boolean" default="True">
  Anonymous telemetry (doesn't collect any user information or user's files). Used to improve the Kozmochain package utilization. Default is `True`.
</ParamField>
