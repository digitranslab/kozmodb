...
## Install Kozmochain with LanceDB

Install Kozmochain, LanceDB and  related dependencies using the following command:

```bash
pip install "kozmochain[lancedb]"
```
...
# create Kozmochain App and set config
...
```python main.py
from kozmochain import Pipeline as App

# config for Kozmochain App
config = {
  'llm': {
