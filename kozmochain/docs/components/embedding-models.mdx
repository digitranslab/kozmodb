...
Kozmochain supports several embedding models from the following providers:
...
* OpenAI announced two new embedding models: `text-embedding-3-small` and `text-embedding-3-large`. Kozmochain supports both these models. Below you can find YAML config for both:
...
Kozmochain supports Google's VertexAI embeddings model through a simple interface. You just have to pass the `model_name` in the config yaml and it would work out of the box.
...
* Cohere has few embedding models: `embed-english-v3.0`, `embed-multilingual-v3.0`, `embed-multilingual-light-v3.0`, `embed-english-v2.0`, `embed-english-light-v2.0` and `embed-multilingual-v2.0`. Kozmochain supports all these models. Below you can find YAML config for all:
