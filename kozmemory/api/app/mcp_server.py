"""
MCP Server for KozMemory with resilient memory client handling.

This module implements an MCP (Model Context Protocol) server that provides
memory operations for KozMemory. The memory client is initialized lazily
...
# Initialize MCP
mcp = FastMCP("kozmodb-mcp-server")
    ...
    try:
        ...
        try:
            ...
            if not app.is_active:
                return f"Error: App {app.name} is currently paused on KozMemory. Cannot create new memories."

            response = memory_client.add(text,
                                         user_id=uid,
                                         metadata={
                                            "source_app": "kozmemory",
...
def setup_mcp_server(app: FastAPI):
    ...
    mcp._mcp_server.name = f"kozmodb-mcp-server"
