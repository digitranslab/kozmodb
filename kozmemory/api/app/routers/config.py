...
def get_config_from_db(db: Session, key: str = "main"):
    ...
    if "kozmemory" not in config_value:
        config_value["kozmemory"] = default_config["kozmemory"]
    
    if "kozmodb" not in config_value:
        config_value["kozmodb"] = default_config["kozmodb"]
    else:
        # Ensure LLM config exists with defaults
        if "llm" not in config_value["kozmodb"] or config_value["kozmodb"]["llm"] is None:
            config_value["kozmodb"]["llm"] = default_config["kozmodb"]["llm"]
        
        # Ensure embedder config exists with defaults
        if "embedder" not in config_value["kozmodb"] or config_value["kozmodb"]["embedder"] is None:
            config_value["kozmodb"]["embedder"] = default_config["kozmodb"]["embedder"]
...
async def update_configuration(config: ConfigSchema, db: Session = Depends(get_db)):
    ...
    # Update kozmemory settings if provided
    if config.kozmemory is not None:
        if "kozmemory" not in updated_config:
            updated_config["kozmemory"] = {}
        updated_config["kozmemory"].update(config.kozmemory.dict(exclude_none=True))
    
    # Update kozmodb settings
    updated_config["kozmodb"] = config.kozmodb.dict(exclude_none=True)
...
@router.get("/kozmodb/llm", response_model=LLMProvider)
async def get_llm_configuration(db: Session = Depends(get_db)):
    """Get only the LLM configuration."""
    config = get_config_from_db(db)
    llm_config = config.get("kozmodb", {}).get("llm", {})
    return llm_config

@router.put("/kozmodb/llm", response_model=LLMProvider)
async def update_llm_configuration(llm_config: LLMProvider, db: Session = Depends(get_db)):
    """Update only the LLM configuration."""
    current_config = get_config_from_db(db)
    
    # Ensure kozmodb key exists
    if "kozmodb" not in current_config:
        current_config["kozmodb"] = {}
    
    # Update the LLM configuration
    current_config["kozmodb"]["llm"] = llm_config.dict(exclude_none=True)
    
    # Save the configuration to database
    save_config_to_db(db, current_config)
    reset_memory_client()
    return current_config["kozmodb"]["llm"]

@router.get("/kozmodb/embedder", response_model=EmbedderProvider)
async def get_embedder_configuration(db: Session = Depends(get_db)):
    """Get only the Embedder configuration."""
    config = get_config_from_db(db)
    embedder_config = config.get("kozmodb", {}).get("embedder", {})
    return embedder_config

@router.put("/kozmodb/embedder", response_model=EmbedderProvider)
async def update_embedder_configuration(embedder_config: EmbedderProvider, db: Session = Depends(get_db)):
    ...
    current_config["kozmodb"]["embedder"] = embedder_config.dict(exclude_none=True)
    
    # Save the configuration to database
    save_config_to_db(db, current_config)
    reset_memory_client()
    return current_config["kozmodb"]["embedder"]

@router.get("/kozmemory", response_model=KozMemoryConfig)
async def get_kozmemory_configuration(db: Session = Depends(get_db)):
    """Get only the KozMemory configuration."""
    config = get_config_from_db(db)
    kozmemory_config = config.get("kozmemory", {})
    return kozmemory_config

@router.put("/kozmemory", response_model=KozMemoryConfig)
async def update_kozmemory_configuration(kozmemory_config: KozMemoryConfig, db: Session = Depends(get_db)):
    """Update only the KozMemory configuration."""
    current_config = get_config_from_db(db)
    
    # Ensure kozmemory key exists
    if "kozmemory" not in current_config:
        current_config["kozmemory"] = {}
    
    # Update the KozMemory configuration
    current_config["kozmemory"].update(kozmemory_config.dict(exclude_none=True))
    
    # Save the configuration to database
    save_config_to_db(db, current_config)
    reset_memory_client()
    return current_config["kozmemory"] 