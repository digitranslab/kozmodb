...
interface UseConfigApiReturn {
  ...
  saveConfig: (config: { kozmemory?: KozMemoryConfig; kozmodb: KozmodbConfig }) => Promise<void>;
...
export const useConfig = (): UseConfigApiReturn => {
  ...
  const saveConfig = async (config: { kozmemory?: KozMemoryConfig; kozmodb: KozmodbConfig }) => {
    setIsLoading(true);
    setError(null);
  ...
  const saveLLMConfig = async (llmConfig: LLMProvider) => {
    ...
    try {
      const response = await axios.put(`${URL}/api/v1/config/kozmodb/llm`, llmConfig);
  ...
  const saveEmbedderConfig = async (embedderConfig: EmbedderProvider) => {
    ...
    try {
      const response = await axios.put(`${URL}/api/v1/config/kozmodb/embedder`, embedderConfig);
